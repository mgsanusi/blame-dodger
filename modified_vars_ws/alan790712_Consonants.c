...
int Main()
{
	...
	for (c = 1; c <= t; ++c) {
		...
		for (file = 0; file < temp1; ++file) {
			scanf("%d %d %d %d %d %d %d %d", &a[file].map,
			      &a[file].temp1, &a[file].w, &a[file].qn,
			      &a[file].s, &a[file].dd, &a[file].middle_char,
			      &a[file].ds);
			a[file].w += 500;
			a[file].qn += 500;
		}
