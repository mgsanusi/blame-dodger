...
int stack[2008];
int
comp (const void *k, const void *x)
{
  PAIR *p = (PAIR *) k;
  PAIR *q = (PAIR *) x;
  if (p->y > q->y)
...
Main ()
{
  ...
  for (t2 = 1; t2 <= x; t2++)
      ...
      for (seanxor = 0; seanxor < m; seanxor++)
	{
	  ...
	  tmp = e - o;
	  tmp2 = ((tmp * in) - ((tmp * (tmp - 1)) / 2)) % 1000002013;
      ...
      for (seanxor = 0; seanxor < x1; seanxor++)
	{
	  ...
	  if (p[seanxor].p > 0)
	    ...
	    {
	      outputFilePtr = 1;
	      while ((p[stack[deltaP - 1]].p + p[seanxor].p) <= 0)
		{
		  deltaP--;
		  ...
		  tmp4 =
		    (tmp4 +
		     (tmp2 * (p[stack[deltaP]].p % 1000002013))) % 1000002013;
		  p[stack[deltaP]].p = 0;
		  if (deltaP == 0)
		    {
		      outputFilePtr = 0;
	      ...
	      if (outputFilePtr)
		{
		  tmp = p[seanxor].y - p[stack[deltaP - 1]].y;
		  tmp2 = ((tmp * in) - ((tmp * (tmp - 1)) / 2)) % 1000002013;
		  tmp4 =
		    (tmp4 +
		     (tmp2 * ((-p[seanxor].p) % 1000002013))) % 1000002013;
