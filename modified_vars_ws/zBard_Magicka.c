...
int Main(void)
{
	...
	for (; no_combine > 0; no_combine--) {
	    scanf(" %s", combine_str);
	    reaction_arr[combine_str[0] - 'A'][combine_str[1] - 'A'] =
		combine_str[2] - 'A';
	    reaction_arr[combine_str[1] - 'A'][combine_str[0] - 'A'] =
		combine_str[2] - 'A';
	}
	scanf("%d", &num_destroy);
	for (; num_destroy > 0; num_destroy--) {
	    scanf(" %s", destroy_string);
	    destruction_array[destroy_string[0] - 'A'][destroy_string[1] -
						       'A'] = 1;
	    destruction_array[destroy_string[1] - 'A'][destroy_string[0] -
						       'A'] = 1;
	}
	scanf("%d", &no_str);
	scanf(" %s", input_str);
	for (bx = 0; bx < no_str; bx++) {
		...
		if (output_len != 0) {
			...
			flag = 1;
			new_element =
			    reaction_arr[new_element][former_element];
			output_len--;
			element_seen[former_element]--;
			continue;
		    }
		    for (v = 0; v < 26; v++) {
			if (destruction_array[new_element][v]
			    && element_seen[v]) {
			    flag = 2;
			    for (w = 0; w < 26; w++) {
				element_seen[w] = 0;
