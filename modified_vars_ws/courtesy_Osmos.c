...
int Main()
{
	...
	for (ishappy = 1; ishappy <= c; ishappy++) {
		...
		scanf("%d", &mutex_buf);
		for (cf_stat = 0; cf_stat < mutex_buf; cf_stat++) {
			scanf("%d %d %d %d %d %d %d %d", &b[cf_stat].p,
			      &b[cf_stat].mutex_buf, &b[cf_stat].c,
			      &b[cf_stat].p, &b[cf_stat].l, &b[cf_stat].dd,
			      &b[cf_stat].y, &b[cf_stat].cases);
		}
		sort();
		b = 0;
		while (1) {
			for (cf_stat = 0;;) {
				for (length2 = 2 * b[cf_stat].c;
				     length2 <= (2 * b[cf_stat].p); length2++) {
					if (length2 < 0) {
				...
				if ((((cf_stat + 1) < mutex_buf)
				     && b[cf_stat + 1].mutex_buf)
				    && (b[cf_stat + 1].p == b[cf_stat].p))
					cf_stat++;
				else
					break;
			}
			for (length2 = 0; length2 <= cf_stat; length2++) {
				for (p2 = 2 * b[length2].c;
				     p2 <= (2 * b[length2].p); p2++) {
					if (p2 < 0) {
