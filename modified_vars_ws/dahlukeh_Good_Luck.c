...
void
sort (int start, int end, struct range *file)
{
  if (start >= end)
    {
      return;
  ...
  for (i = 0; i < ((end - start) + 1); i++)
	...
	{
	  if (((typeSort == 0)
	       && ((file[place1].s < file[place2].s)
		   || ((file[place1].s == file[place2].s)
		       && (file[place1].t < file[place2].t))))
	      || ((typeSort == 1)
		  && ((file[place1].t > file[place2].t)
		      || ((file[place1].t == file[place2].t)
			  && (file[place1].s > file[place2].s)))))
...
findAmo (int s, int t)
{
  ...
  long long rslt =
    ((score * (score + 1ll)) / 2ll) -
    (((score - diff) * ((score + 1ll) - diff)) / 2ll);
  return rslt;
}

int
getSwapAmo (int a, int e1, int s2, int e2)
{
...
handlecase ()
{
  ...
  for (i = 0; i < nP; i++)
	...
	{
	  if ((((people[list].s > people[i].s)
		&& (people[list].s <= people[i].t))
	       && (people[list].t > people[i].t)) && (people[list].file > 0))
	    {
      ...
      if (ind != (-1))
	{
	  if (people[i].file > people[ind].file)
	    {
	      long long caseId =
		(long long) getSwapAmo (people[i].s, people[i].t,
					people[ind].s, people[ind].t);
