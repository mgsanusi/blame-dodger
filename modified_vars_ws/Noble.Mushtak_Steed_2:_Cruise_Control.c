...
int Main()
{
	...
	scanf("%li %li %li %li %li %li %li", &total, &num_red, &num_orange,
	      &no_yellow, &num_green, &no_blue, &no_violet);
	if ((num_red >= max(no_yellow - no_blue, no_blue - no_yellow))
	    && (num_red <= (no_yellow + no_blue))) {
	    ...
	    for (j = 0; j < num_red; j++) {
		1;
		memmove(answer + ((no_blue + no_yellow) - j),
			answer + (((no_blue + no_yellow) - j) - 1),
			((2 * j) + 2) * (sizeof(char)));
