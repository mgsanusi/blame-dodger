double
trarc (double re_why, double wp, double x)
{
  double d = sqrt ((x * x) - (wp * wp));
...
double
area (double re_why, double wp, double d, double names, double x)
{
  if (((d < re_why) || (names < wp))
      || (((re_why * re_why) + (wp * wp)) >= (x * x)))
    return 0;
  ...
  if (((re_why * re_why) + (names * names)) > (x * x))
	...
	{
	  return (trarc (re_why, wp, x) - trarc (d, wp, x)) - trarc (re_why,
								     names,
								     x);
	  ;
	}
	  ...
	  for (k = 0; k < 1100; k++)
	    {
	      ...
	      m =
		area (re_why, wp, ((re_why + nextlado) - f) - f,
		      ((wp + nextlado) - f) - f, (r - tmp) - f);
	      x += m;
	      if (m == 0)
		break;
	    }
