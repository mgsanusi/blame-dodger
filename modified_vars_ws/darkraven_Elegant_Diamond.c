...
int find(int k, int x, int r, int dep)
{
	...
	if ((k + (last[x] < last[r] ? last[x] : last[r])) >= p)
	    return 0;
	else if ((k + (last[x] < last[r] ? last[x] : last[r])) == (p - 1))
	    return s[dep][x][r];
	else
	    return 1000000000;
    }
    ...
    int res = 1000000000;
    tmp =
	find(k, x, (x + r) >> 1, dep - 1) + find(k, ((x + r) >> 1) + 1, r,
						 dep - 1);
    if (tmp < res)
	res = tmp;
    tmp =
	(find(k + 1, x, (x + r) >> 1, dep - 1) +
	 find(k + 1, ((x + r) >> 1) + 1, r, dep - 1)) + s[dep][x][r];
    if (tmp < res)
	res = tmp;
    len[k][x][r] = 1;
    v[k][x][r] = res;
...
int Main()
{
	...
	for (applauding = 1; applauding <= p; applauding++)
		...
		scanf("%d",
		      s[applauding][len] + ((len + (1 << applauding)) -
					    1));
