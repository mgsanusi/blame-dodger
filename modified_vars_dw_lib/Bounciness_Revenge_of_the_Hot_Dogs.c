...
int valid(int produce, int data, int pos)
{
  int first_iteration;
    ...
    for (x2 = 0; x2 < pos; x2++)
  {
    ...
    ansx += ((double) grid[a + produce][x2 + data]) * ((((double) (produce + a)) + 0.5) - data_style_css);
    ansy += ((double) grid[a + produce][x2 + data]) * ((((double) (data + x2)) + 0.5) - n);
  }


  if ((new_fabs(ansx) < 0.00000001) && (new_fabs(ansy) < 0.000000001))
    return 1;
