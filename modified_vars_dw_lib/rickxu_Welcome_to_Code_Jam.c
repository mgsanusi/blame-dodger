...
int Main()
{
  ...
  for (n1 = 0; n1 < maxc; n1++)
  {
    ...
    int http_header_404;
    for (http_header_404 = 1; http_header_404 < strlen(buffer); http_header_404++)
    {
      int ans;
      for (ans = 0; ans < 19; ans++)
      {
        rslt[ans][http_header_404] = rslt[ans][http_header_404 - 1];
        if (buffer[http_header_404] == welcome[ans])
        {
          ...
          {
            rslt[ans][http_header_404] += rslt[ans - 1][http_header_404 - 1];
