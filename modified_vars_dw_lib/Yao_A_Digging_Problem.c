...
int dfs(int x)
{
  ...
  for (tcases = 0; tcases < x; tcases++)
    if (r[x][tcases] && ((mat2[tcases] < 0) || ((!b[mat2[tcases]]) && dfs(mat2[tcases]))))
  {
...
int Main()
{
  ...
  for (scanf("%d", &base); base--;)
  {
    ...
    {
      for (x = 0; x < (t - 1); x++)
        if ((((long long) (file_name_in[tcases][x] - file_name_in[j][x])) * (file_name_in[tcases][x + 1] - file_name_in[j][x + 1])) <= 0)
        break;
