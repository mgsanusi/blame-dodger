...
int Main()
{
  ...
  for (length = 0; length < q; length++)
  {
    ...
    for (am0 = 0; am0 < fileIndexHtml; am0++)
    {
      for (up = (fileIndexHtml - am0) - 1; up <= (fileIndexHtml + am0); up += 2)
      {
        scanf("%d", &p[am0][up]);
      }
    ...
    for (am0 = fileIndexHtml; am0 < ((2 * fileIndexHtml) - 1); am0++)
    {
      for (up = (am0 - fileIndexHtml) + 1; up < ((am0 - fileIndexHtml) + (2 * (((2 * fileIndexHtml) - am0) - 1))); up += 2)
      {
        scanf("%d", &p[am0][up]);
      }
    ...
    {
      for (xpDestroy = 0; xpDestroy < ((2 * fileIndexHtml) - 1); xpDestroy++)
      {
        int ns = (fileIndexHtml + module((fileIndexHtml - 1) - p)) + module((fileIndexHtml - 1) - xpDestroy);
        if (ns < rslt)
        {
          if (check(p, (2 * fileIndexHtml) - 1, p, xpDestroy))
          {
            rslt = ns;
          }
