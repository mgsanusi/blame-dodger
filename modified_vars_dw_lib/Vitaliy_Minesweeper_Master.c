...
void clear(void)
{
  ...
  int x2;
  new_assert(scanf("%d\n", &x2) == 1);
  new_assert((x2 <= 100) && (x2 >= 1));
  int uncompleted;
  for (uncompleted = 0; uncompleted < x2; uncompleted++)
  {
    new_assert(scanf("%s\n", s[uncompleted]) == 1);
    new_assert(strlen(s[uncompleted]) > 0);
  }
  ...
  {
    if ((uncompleted == 0) || (s[0][uncompleted] != s[0][uncompleted - 1]))
    {
      x[w] = s[0][uncompleted];
      cou[0][w] = 1;
      w++;
  ...
  for (j = 1; j < x2; j++)
  {
    ...
    {
      if ((uncompleted == 0) || (s[j][uncompleted] != s[j][uncompleted - 1]))
      {
