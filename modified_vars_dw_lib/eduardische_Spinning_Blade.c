...
int Main()
{
  ...
  for (i_t = 0; i_t < k; i_t++)
  {
    ...
    {
      scanf("%d %d %d", &data[mask].c, &data[mask].m, &data[mask].rr);
    }
    ...
    for (mask = 0; mask < n; mask++)
    {
      if (data[mask].c > pos)
      {
        ...
        board[opt].t = ((double) board[opt].y2) / ((double) board[opt].rr);
        board[opt].c = ((double) board[opt].y2) / ((double) (board[opt].rr + (i2 - ah_validate)));
        pos = data[mask].c;
        opt++;
      }
    ...
    for (mask = 0; mask < opt; mask++)
    {
      ...
      {
        double s2 = rem * ((double) (board[mask].rr + (i2 - ah_validate)));
