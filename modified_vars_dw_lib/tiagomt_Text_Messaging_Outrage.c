...
int Main(void)
{
  ...
  int caseno;
  int lb;
  for (scanf("%d", &count), sizeCount = 1; sizeCount <= count; sizeCount++)
  {
    ...
    for (caseno = 0; caseno < f; caseno++)
    {
      ...
      start[caseno % r] = ((((long long) x) * start[caseno % r]) + (((long long) t) * (caseno + 1))) % mutexBuf;
    }
    ...
    for (caseno = 1; caseno < f; caseno++)
    {
      ...
      for (lb = 0; lb < caseno; lb++)
      {
        if (v[lb] < v[caseno])
        {
          w[caseno] = (w[caseno] + w[lb]) % 1000000007;
        }
