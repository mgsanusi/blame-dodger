...
void handle_subject(void)
{
  ...
  for (walk = 1; walk < no_l; walk++)
  {
    ...
    area += 0.5 * ((lower_p[walk].x - lower_p[walk - 1].x) * abs_f(lower_p[walk - 1].k - lower_p[walk].k));
  }
  ...
  for (walk = 1; walk < no_u; walk++)
  {
    ...
    area += 0.5 * ((upper_p[walk].x - upper_p[walk - 1].x) * abs_f(upper_p[walk - 1].k - upper_p[walk].k));
  }
...
double calc_ar(double place)
{
  ...
  double dy = (lower_p[walk].k - lower_p[walk - 1].k) / (lower_p[walk].x - lower_p[walk - 1].x);
  double k = lower_p[walk - 1].k + (dy * (place - lower_p[walk - 1].x));
  l = max(lower_p[walk - 1].k, k) + 1000.0;
  area -= l * (place - lower_p[walk - 1].x);
  area += 0.5 * ((place - lower_p[walk - 1].x) * abs_f(lower_p[walk - 1].k - k));
  ...
  dy = (upper_p[walk].k - upper_p[walk - 1].k) / (upper_p[walk].x - upper_p[walk - 1].x);
  k = upper_p[walk - 1].k + (dy * (place - upper_p[walk - 1].x));
  l = 1000.0 - min(upper_p[walk - 1].k, k);
  area -= l * (place - upper_p[walk - 1].x);
  area += 0.5 * ((place - upper_p[walk - 1].x) * abs_f(upper_p[walk - 1].k - k));
  return area;
}
