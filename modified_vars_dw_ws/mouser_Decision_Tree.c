...
void reverse(char *s, int length)
{
	...
	for (updateNumButtonsPressed = 0;
	     updateNumButtonsPressed < (length / 2);
...
void increase(void)
{
	...
	for (updateNumButtonsPressed = length - 2; updateNumButtonsPressed >= 0;
	     updateNumButtonsPressed--) {
		for (ebackspace = length - 1;
		     ebackspace > updateNumButtonsPressed; ebackspace--) {
			if (num[updateNumButtonsPressed] < num[ebackspace]) {
				swap(&num[updateNumButtonsPressed],
				     &num[ebackspace]);
