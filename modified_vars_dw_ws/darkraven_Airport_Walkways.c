...
int Main()
{
	...
	for (heapn = 0; heapn < dy; heapn++) {
	    scanf("%d%d%d", &walk[heapn].x2, &walk[heapn].l,
		  &walk[heapn].d);
	    plain -= walk[heapn].l - walk[heapn].x2;
	}
	qsort(walk, dy, sizeof(struct A), cmp);
	double result = 0;
	...
	for (heapn = 0; heapn < dy; heapn++) {
	    double time =
		((double) (walk[heapn].l - walk[heapn].x2)) /
		((double) (cl_destroy + walk[heapn].d));
	    if (time > t) {
		time =
		    t +
		    ((((double) (walk[heapn].l - walk[heapn].x2)) -
		      ((cl_destroy + walk[heapn].d) * t)) / ((double) (m +
								       walk
								       [heapn].
								       d)));
		t = 0;
