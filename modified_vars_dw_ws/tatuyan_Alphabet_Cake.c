...
int
comp (const void *c, const void *t)
{
  int first_iteration;
...
clear (int cases)
{
  ...
  qsort (gain, drop, sizeof (gain[0]), comp);
  for (lf = 0; lf < drop; lf++)
    {
      if ((lf != 0) && (gain[lf].tc > gain[lf - 1].tc))
	{
	  for (ah_cred = lf - 1; ah_cred >= 0; ah_cred--)
		...
		tmp =
		  ((double) (gain[ah_cred].k - gain[lf].k)) / (gain[lf].tc -
							       gain[ah_cred].
							       tc);
	      if (tmp <= time[ah_cred])
		break;
	...
	time[lf] = (dtime[lf] = ((double) (iy - gain[lf].k)) / gain[lf].tc);
    }
