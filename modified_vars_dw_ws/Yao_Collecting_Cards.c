...
int m;
long long
caltime (long long n, long long s, long long w, long long l)
{
  int first_iteration;
  long long c = s + w;
  long long t1 = l % c;
...
Main ()
{
  ...
  for (scanf ("%d", &n); n--;)
	...
	for (first = 0; first < m; first++)
	  {
	    scanf ("%d %d %d", &s[enqcnt][first], &w[enqcnt][first],
		   &l[enqcnt][first]);
	    cases[(enqcnt * m) + first][0] = (cases[(enqcnt * m) + first][1] =
					      (cases[(enqcnt * m) + first][2]
					       =
					       (cases[(enqcnt * m) + first][3]
						= 1000000000000000000LL)));
	    a[(enqcnt * m) + first][0] = (a[(enqcnt * m) + first][1] =
					  (a[(enqcnt * m) + first][2] =
					   (a[(enqcnt * m) + first][3] = 0)));
	  }
      cases[0][0] = 0;
      for (enqcnt = 0; enqcnt < ((n * m) * 4); enqcnt++)
	{
	  for (t1 = (t2 = -1), first = 0; first < (n * m); first++)
	    for (k = 0; k < 4; k++)
	      if ((!a[first][k])
		  && ((t1 < 0) || (cases[first][k] < cases[t1][t2])))
		t1 = first, t2 = k;
	  a[t1][t2] = 1;
	  if ((t1 == ((n * m) - 1)) && (t2 == 3))
	    break;
	  ...
	  if (t2 == 0)
		...
		cases[t1 - 1][1] = cases[t1][t2] + 2;
	      tt =
		caltime (cases[t1][t2], s[add][fp], w[add][fp],
			 l[add][fp]) + 1;
	      if (tt < cases[t1][2])
		cases[t1][2] = tt;
	      tt =
		caltime (cases[t1][t2], w[add][fp], s[add][fp],
			 l[add][fp] + s[add][fp]) + 1;
	      if (tt < cases[t1][1])
		cases[t1][1] = tt;
	    }
