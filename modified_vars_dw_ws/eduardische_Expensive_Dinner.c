...
int Main()
{
	...
	for (length = 0; length < n; length++) {
		...
		n[length][zip] =
		    (((((long long int) length) *
		       ((long long int) m[length][zip])) + get_value(n,
								     length
								     - 1,
								     zip))
		     + get_value(n, length, zip - 1)) - get_value(n,
								  length -
								  1,
								  zip - 1);
		map[length][zip] =
		    ((((long long int) m[length][zip]) +
		      get_value(map, length - 1, zip)) + get_value(map,
								   length,
								   zip -
								   1)) -
		...
		lasti[length][zip] =
		    (((((long long int) zip) *
		       ((long long int) m[length][zip])) + get_value(lasti,
								     length
								     - 1,
								     zip))
		     + get_value(lasti, length,
				 zip - 1)) - get_value(lasti, length - 1,
						       zip - 1);
		oowp[length][zip] =
		    ((((long long int) m[length][zip]) +
		      get_value(oowp, length - 1, zip)) + get_value(oowp,
								    length,
								    zip -
								    1)) -
	...
	while (s >= 3) {
		...
		for (zip = 0; ((zip + s) - 1) < ca; zip++) {
		    long long int n =
			((get_value(n, (length + s) - 1, (zip + s) - 1) -
			...
			((get_value(map, (length + s) - 1, (zip + s) - 1) -
			...
			get_value(map, length - 1, zip - 1);
		    long long int o_won =
			((get_value(lasti, (length + s) - 1, (zip + s) - 1)
		    ...
		    long long int loss =
			((get_value(oowp, (length + s) - 1, (zip + s) - 1)
		    ...
		    o_won -=
			((long long int) t) * ((long long int) m[x][t]);
