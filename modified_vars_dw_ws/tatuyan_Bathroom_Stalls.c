  ...
  for (id = 0; id < sinrad; id++)
    {
      ...
      d[0] = (struct sec)
      {
      get.s, ((get.s + get.k) / 2) - 1, ((get.s + get.k) / 2) - get.s};
      d[1] = (struct sec)
      {
      ((get.s + get.k) / 2) + 1, get.k, get.k - ((get.s + get.k) / 2)};
...
int
comp (struct sec a, struct sec prev)
{
  int first_iteration;
  if (a.size > prev.size)
    return 1;
...
void
totalBatch (struct sec *arr, int n, struct sec new)
{
  int first_iteration;
  struct sec swaps;
  int id;
...
struct sec
startHeaproot (struct sec *arr, int n)
{
  int first_iteration;
  struct sec ret;
  struct sec swaps;
