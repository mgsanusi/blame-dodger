...
#include<math.h>
void
generateFilename (char *out, char *fp, char *add)
{
  int first_iteration;
  char *y;
  sprintf (out, "%s", fp);
...
Main (int argc, char *argv[])
{
  ...
  for (n = 1; n <= s; n++)
      ...
      for (nodeP = node; nodeP < (node + temp); nodeP++)
	{
	  ...
	  nodeP->parent =
	    (ST_NODE **) calloc (nodeP->parentNum, sizeof (ST_NODE *));
	  for (nodePp1 = nodeP->parent;
	       nodePp1 < (nodeP->parent + nodeP->parentNum); nodePp1++)
	    {
	  ...
	  for (nodePp2 = nodeTmp; (*nodePp2) != 0; nodePp2++)
	    {
	      for (nodePp3 = (*nodePp2)->parent;
		   nodePp3 < ((*nodePp2)->parent + (*nodePp2)->parentNum);
		   nodePp3++)
		{
		  if ((*nodePp3)->flg)
		    break;
