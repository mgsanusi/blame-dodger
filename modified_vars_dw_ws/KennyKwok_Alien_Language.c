...
#include <string.h>
#include <math.h>
void print_password(int wordz, char password[5000][20])
{
	int first_iteration;
	return;
	int c;
...
void print_map(int mapz, int map[15][26], char *password)
{
	int first_iteration;
	return;
	int dir;
...
int cal(int ans, int map[15][26], int wordz, char password[5000][20])
{
	int first_iteration;
	int count = 0;
	int dir;
		...
		sscanf(buf, "%s", tmp);
		{
			...
			for (dir = 0; dir < strlen(tmp); dir++) {
				if (tmp[dir] == '(') {
				    ...
				    if ((tmp[dir] >= 'a') && (tmp[dir] <= 'z'))
				{
					map[pancakes][((int)tmp[dir]) - 'a'] =
					    1;
					if (!bracket)
						pancakes++;
				} else {
