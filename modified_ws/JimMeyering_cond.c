...
static struct gl_waitqueue_element *gl_waitqueue_add(gl_waitqueue_t * wq)
{
    ...
    elt = (struct gl_waitqueue_element *)
...
int glthread_cond_init_func(gl_cond_t * cond)
{
...
int glthread_cond_wait_func(gl_cond_t * cond, gl_lock_t * lock)
{
    if (!cond->guard.done) {
...
int
glthread_cond_timedwait_func(gl_cond_t * cond, gl_lock_t * lock,
			     struct timespec *abstime)
{
...
int glthread_cond_signal_func(gl_cond_t * cond)
{
    if (!cond->guard.done)
	return EINVAL;
    EnterCriticalSection(&cond->lock);
...
int glthread_cond_broadcast_func(gl_cond_t * cond)
{
    if (!cond->guard.done)
	return EINVAL;
    EnterCriticalSection(&cond->lock);
...
int glthread_cond_destroy_func(gl_cond_t * cond)
{
    if (!cond->guard.done)
	return EINVAL;
    if (cond->waiters.wq_list.wql_next != &cond->waiters.wq_list)
