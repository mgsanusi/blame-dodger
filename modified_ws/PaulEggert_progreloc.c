				...
				const char *orig_installdir)
{
	   ...
	   Remove the ".bin" here.  */
	{
		...
		if (argv0_len > 4 + exeext_len)
			if (memcmp
			    (argv0 + argv0_len - exeext_len - 4, ".bin",
			     4) == 0) {
				...
				{
					char *shorter =
					    (char *)xmalloc(argv0_len - 4 + 1);
#ifdef NO_XMALLOC
					if (shorter != NULL)
#endif
					{
						memcpy(shorter, argv0,
						       argv0_len - exeext_len -
						       4);
						if (sizeof(EXEEXT) > sizeof(""))
							memcpy(shorter +
							       argv0_len -
