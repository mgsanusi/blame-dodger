...
int main(int argc, char **argv)
{
    ...
    int test_cases, num_buttons, i, j, k, which_robot[MAX_CASES],
	which_button[MAX_CASES];
    int next_button_blue, next_button_orange, pos_blue, pos_orange,
	num_buttons_pressed, steps;
    FILE *fp_in, *fp_out;
...
//                              printf("next blue %d\n", next_button_blue);
		...
		for (j = num_buttons_pressed + 1; j < num_buttons; j++)
			...
			break;
		next_button_orange =
		    (j < num_buttons) ? which_button[j] : 1;
