...
int main(int argc, char **argv)
{
	...
	/* Test writing to a nonexistent program traps sooner or later.  */
	{
		...
		f = pipe_filter_gi_create("pipe-filter-test", path[0], path,
					  true, false, prepare_read,
					  ignore_done_read, NULL);
		small_nap();
	...
	/* Now test asynchronous I/O.  */
	{
		path[0] = argv[1];
		f = pipe_filter_gi_create("pipe-filter-test", path[0], path,
					  false, true, prepare_read,
					  output_done_read, NULL);
