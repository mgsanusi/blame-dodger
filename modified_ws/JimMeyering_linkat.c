...
#else
static int link_immediate(char const *file1, char const *file2)
{
...
#else
static int link_follow(char const *file1, char const *file2)
{
...
static int solaris_optimized_link_follow(char const *file1, char const *file2)
{
	if (__xpg4 != 0)
		return link(file1, file2);
...
int linkat(int fd1, char const *file1, int fd2, char const *file2, int flag)
{
...
int rpl_linkat(int fd1, char const *file1, int fd2, char const *file2, int flag)
{
	...
	/* Reject trailing slashes on non-directories.  */
	{
		...
		if ((len1 && file1[len1 - 1] == '/')
			...
			struct stat st;
			if (fstatat
			    (fd1, file1, &st, flag ? 0 : AT_SYMLINK_NOFOLLOW))
				return -1;
