...
#include "uniconv.h"
static int is_utf8_encoding(const char *encoding)
{
...
static void ascii_grapheme_breaks(const char *s, size_t n, char *p)
{
	size_t i;
	p[0] = 1;
	for (i = 1; i < n; i++) {
...
void ulc_grapheme_breaks(const char *s, size_t n, char *p)
{
	if (n > 0) {
		...
		if (is_utf8_encoding(encoding))
			...
			if (offsets != NULL) {
				uint8_t *t;
				size_t m;
				t = u8_conv_from_encoding(encoding,
							  iconveh_question_mark,
							  s, n, offsets, NULL,
							  &m);
				if (t != NULL) {
					char *q =
					    (char *)(m > 0 ? malloc(m) : NULL);
					if (m == 0 || q != NULL) {
						size_t i;
