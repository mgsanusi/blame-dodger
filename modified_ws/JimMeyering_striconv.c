...
char *str_cd_iconv(const char *src, iconv_t cd)
{
	...
	/* Do the conversion.  */
	{
     ...
     || !(((__GLIBC__ == 2 && __GLIBC_MINOR__ <= 1) && !defined __UCLIBC__) \
		...
		for (;;) {
			...
			size_t res =
			    iconv(cd, NULL, NULL, &outptr, &outbytes_remaining);
			if (res == (size_t) (-1)) {
				if (errno == E2BIG) {
					size_t used = outptr - result;
					size_t newsize = result_size * 2;
...
#endif
char *str_iconv(const char *src, const char *from_codeset,
