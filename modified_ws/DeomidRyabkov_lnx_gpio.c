...
bool
gpio_helper (int gpio_no, const char *file_name_templ, const char *val)
{
  int fd, res;
  char buf[50];
...
/* 0 = IN, 1 = OUT */
bool
gpio_set_direction (int gpio_no, enum mgos_gpio_mode d)
{
...
bool
gpio_set_value (int gpio_no, bool value)
{
...
bool
gpio_set_edge (int gpio_no, enum mgos_gpio_int_mode edge)
{
  ...
  return gpio_helper (gpio_no, "/sys/class/gpio/gpio%d/edge",
		      edge_names[edge]);
}
...
void
mgos_gpio_hal_int_done (int pin)
{
  (void) pin;
}

void
gpio_remove_handler (int gpio_no)
{
...
bool
mgos_gpio_hal_set_int_mode (int gpio_no, enum mgos_gpio_int_mode mode)
{
  int fd, res;
  char tmp, buf[50];
...
/* HAL functions */
bool
mgos_gpio_set_mode (int pin, enum mgos_gpio_mode mode)
{
...
bool
mgos_gpio_set_pull (int pin, enum mgos_gpio_pull_type pull)
{
...
void
mgos_gpio_write (int pin, bool level)
{
  gpio_set_value (pin, level);
}

bool
mgos_gpio_read (int pin)
{
  return gpio_get_value (pin);
}

bool
mgos_gpio_toggle (int pin)
{
  /* TODO */
  (void) pin;
  return false;
