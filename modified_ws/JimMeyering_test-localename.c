...
static void test_locale_name(void)
{
	...
	for (i = 0; i < SIZEOF(categories); i++) {
		...
		if (locale != NULL) {
			...
			const char *name_j =
			    gl_locale_name(categories[j].cat,
					   categories[j].string);
			if (j == i)
...
static void test_locale_name_thread(void)
{
	...
	for (j = 0; j < SIZEOF(choices); j++) {
		...
		for (i = 0; i < SIZEOF(categories); i++) {
		    unsaved_names[j][i] =
			gl_locale_name_thread(categories[i].cat,
					      categories[i].string);
	...
	for (j = 0; j < SIZEOF(choices); j++)
		...
		unsigned int i;
		for (i = 0; i < SIZEOF(categories); i++)
		    ASSERT(strcmp(unsaved_names[j][i], saved_names[j][i])
			   == 0);
	    }
...
static void test_locale_name_posix(void)
{
	...
	name = gl_locale_name_posix(LC_CTYPE, "LC_CTYPE");
	ASSERT(name == NULL
	       || strcmp(name, gl_locale_name_default()) == 0);
	name = gl_locale_name_posix(LC_MESSAGES, "LC_MESSAGES");
	ASSERT(strcmp(name, "fr_FR.UTF-8") == 0);
    }
#if HAVE_NEWLOCALE
