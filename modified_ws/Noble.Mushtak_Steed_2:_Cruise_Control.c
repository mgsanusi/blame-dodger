...
main ()
{
  ...
  {
    scanf ("%li %li %li %li %li %li %li", &total, &numRed, &numOrange,
	   &numYellow, &numGreen, &numBlue, &numViolet);
    if ((numRed >= max (numYellow - numBlue, numBlue - numYellow))
	&& (numRed <= numYellow + numBlue))
      {
	if (numYellow < numBlue)
	  {
	    REPEAT (i, numYellow) answer[2 * i] = 'Y', answer[2 * i + 1] =
	      'B';
	...
	{
	  PRINTF ("A %s\n", answer);
	  memmove (answer + (numBlue + numYellow - i),
		   answer + (numBlue + numYellow - i - 1),
		   (2 * i + 2) * sizeof (char));
