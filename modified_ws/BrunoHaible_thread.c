...
static void do_init_self_key(void)
{
	self_key = TlsAlloc();
...
static void init_self_key(void)
{
	gl_once_define(static, once) gl_once(once, do_init_self_key);
}
...
static inline HANDLE get_current_thread_handle(void)
{
	HANDLE this_handle;
...
gl_thread_t gl_thread_self_func(void)
{
	gl_thread_t thread;
	if (self_key == (DWORD) - 1)
		init_self_key();
	...
	if (thread == NULL) {
		...
		for (;;) {
			thread = (struct gl_thread_struct *)
...
static unsigned int WINAPI wrapper_func(void *varg)
{
...
glthread_create_func(gl_thread_t * threadp, void *(*func) (void *), void *arg)
{
	...
	thread->arg = arg;
	{
		...
		thread_handle = (HANDLE)
		    _beginthreadex(NULL, 100000, wrapper_func, thread, 0,
				   &thread_id);
...
int glthread_join_func(gl_thread_t thread, void **retvalp)
{
	if (thread == NULL)
		return EINVAL;
...
int gl_thread_exit_func(void *retval)
{
