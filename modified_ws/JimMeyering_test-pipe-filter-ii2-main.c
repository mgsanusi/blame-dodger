...
main (int argc, char **argv)
{
  ...
  /* Test writing to a nonexistent program traps sooner or later.  */
  {
    ...
    rc =
      pipe_filter_ii_execute ("pipe-filter-test", path[0], path, true, false,
			      prepare_write, done_write, prepare_read,
			      ignore_done_read, &l);
    ASSERT (rc == 127 || rc == -1);
  ...
  /* Test returning the exit status.  */
  {
    ...
    path[0] = argv[1];
    rc =
      pipe_filter_ii_execute ("pipe-filter-test", path[0], path, false, false,
			      prepare_write, done_write, prepare_read,
			      ignore_done_read, &l);
    ASSERT (rc == 1);
  ...
  /* Now test asynchronous I/O.  */
  {
    ...
    path[0] = argv[1];
    rc =
      pipe_filter_ii_execute ("pipe-filter-test", path[0], path, false, true,
			      prepare_write, done_write, prepare_read,
			      output_done_read, &l);
    ASSERT (rc == 0);
