...
int main()
{
	...
	for (h = 0; h < SIZEOF(handlers); h++) {
		...
		uint16_t *result =
		    u16_strconv_from_encoding(input, "ISO-8859-1", handler);
		ASSERT(result != NULL);
	...
	for (h = 0; h < SIZEOF(handlers); h++) {
		...
		uint16_t *result =
		    u16_strconv_from_encoding(input, "ISO-8859-2", handler);
		ASSERT(result != NULL);
		ASSERT(u16_strcmp(result, expected) == 0);
		free(result);
	}
	...
	for (h = 0; h < SIZEOF(handlers); h++) {
		...
		uint16_t *result =
		    u16_strconv_from_encoding(input, "autodetect_jp", handler);
		ASSERT(result != NULL);
		ASSERT(u16_strcmp(result, expected) == 0);
		free(result);
	}
