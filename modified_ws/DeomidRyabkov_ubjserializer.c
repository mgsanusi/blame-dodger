...
static struct visit *
push_visit (struct mbuf *stack, ub_val_t obj)
{
  struct visit *res;
...
struct prop *
ub_next_prop (ub_val_t obj, struct prop *p)
{
...
static void
ub_render_cont (struct ub_ctx *ctx)
{
...
void
ub_render (struct ub_ctx *ctx, ub_val_t root, ub_cb_t cb, void *user_data)
{
  ctx->cb = cb;
...
void
ub_bin_send (struct ub_ctx *ctx, void *d, size_t n)
{
  if (n > ctx->bytes_left)
    {
      n = ctx->bytes_left;
...
static void *
ub_alloc (struct ub_ctx *ctx, size_t size)
{
...
struct ub_str *
create_ub_str (struct ub_ctx *ctx, const struct mg_str s)
{
...
ub_val_t
ub_create_object (struct ub_ctx * ctx)
{
...
ub_val_t
ub_create_array (struct ub_ctx * ctx)
{
...
ub_val_t
ub_create_bin (struct ub_ctx * ctx, size_t n, ub_bin_cb_t cb, void *user_data)
{
...
ub_val_t
ub_create_string (struct ub_ctx * ctx, const struct mg_str str)
{
...
ub_val_t
ub_create_cstring (struct ub_ctx * ctx, const char *s)
{
  return ub_create_string (ctx, mg_mk_str (s));
}

ub_val_t
ub_create_undefined (void)
{
...
int
ub_is_bin (ub_val_t v)
{
  return v.kind == UBJSON_TYPE_BIN;
}

int
ub_is_undefined (ub_val_t v)
{
  return v.kind == UBJSON_TYPE_UNDEFINED;
}

ub_val_t
ub_get (ub_val_t o, const char *name)
{
...
void
ub_add_prop (struct ub_ctx *ctx, ub_val_t obj, const char *name, ub_val_t val)
{
  assert (obj.kind == UBJSON_TYPE_OBJECT);
...
static ub_val_t
ub_array_get (ub_val_t a, size_t idx)
{
  assert (a.kind == UBJSON_TYPE_ARRAY);
  assert ((idx * sizeof (ub_val_t)) < a.val.a->elems.len);
  ub_val_t res;
  memcpy (&res, a.val.a->elems.buf + (idx * sizeof (ub_val_t)),
	  sizeof (ub_val_t));
  return res;
}

void
ub_array_push (struct ub_ctx *ctx, ub_val_t a, ub_val_t val)
{
...
static void
ub_call_cb (struct ub_ctx *ctx, int end)
{
...
#ifdef UBJ_MAIN			/* test */
static void
demo_emit (char *d, size_t l, int end, void *user_data)
{
...
static void
demo_bin (struct ub_ctx *ctx, void *user_data)
{
