...
/* For digest auth. */
void
mg_hash_md5_v (size_t num_msgs, const uint8_t * msgs[],
	       const size_t * msg_lens, uint8_t * digest)
{
...
int
mbedtls_internal_aes_encrypt (mbedtls_aes_context * ctx,
...
int
mbedtls_internal_aes_decrypt (mbedtls_aes_context * ctx,
			      const unsigned char input[16],
			      unsigned char output[16])
{
...
int
atcac_sw_sha2_256 (const uint8_t * data, size_t data_size, uint8_t digest[32])
{
