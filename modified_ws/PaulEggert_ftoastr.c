		  ...
		  int width, int prec, FLOAT x)
{
  char width_0_buffer[LENGTH == 1 ? FLT_BUFSIZE_BOUND
		      : LENGTH == 2 ? DBL_BUFSIZE_BOUND : LDBL_BUFSIZE_BOUND];
  int n = width;
...
FTOASTR (char *buf, size_t bufsize, int flags, int width, FLOAT x)
{
  ...
  *p = '-';
  p += (flags & FTOASTR_LEFT_JUSTIFY) != 0;
  *p = '+';
  p += (flags & FTOASTR_ALWAYS_SIGNED) != 0;
  *p = ' ';
  p += (flags & FTOASTR_SPACE_POSITIVE) != 0;
  *p = '0';
  p += (flags & FTOASTR_ZERO_PAD) != 0;
  *p++ = '*';
  *p++ = '.';
  *p++ = '*';
  *p = 'L';
