...
IRAM NOINSTR static uint8_t read_unaligned_byte(uint8_t * addr)
{
	...
	if (addr >= (uint8_t *) MMAP_BASE && addr < (uint8_t *) MMAP_END) {
		...
		int block =
		    ((uintptr_t) addr & MMAP_ADDR_MASK) / SPIFFS_PAGE_DATA_SIZE;
		int off =
		    ((uintptr_t) addr & MMAP_ADDR_MASK) % SPIFFS_PAGE_DATA_SIZE;
...
void *mmap(void *addr, size_t len, int prot, int flags, int fd, off_t offset)
{
	...
	desc->base = MMAP_ADDR_FROM_DESC(desc);
	{
		...
		if (t != (int32_t) len) {
			LOG(LL_ERROR,
			    ("Spiffs dummy read failed: expected len: %d, actual: %d",
			     len, t));
			return NULL;
		}
		/*
		...
		if (t != 0) {
			LOG(LL_ERROR,
			    ("Failed to close descr after dummy read: %d", t));
			return NULL;
		}
	}
	return desc->base;
	...
	for (i = 0; i < descs_cnt; i++) {
		if (mmap_descs[i].blocks) {
			for (j = 0; j < mmap_descs[i].pages; j++) {
				...
				uint32_t page =
				    SPIFFS_PADDR_TO_PAGE(fs, addr - fbase);
				if (page == src_pix) {
					int delta = (int)dst_pix - (int)src_pix;
					mmap_descs[i].blocks[j] +=
					    delta * LOG_PAGE_SIZE;
				}
			}
		}
...
int esp_spiffs_dummy_read(spiffs * fs, u32_t addr, u32_t size, u8_t * dst)
{
	...
	if (dst >= DUMMY_MMAP_BUFFER_START && dst < DUMMY_MMAP_BUFFER_END) {
		if ((addr -
		     SPIFFS_PAGE_HEADER_SIZE) % SPIFFS_CFG_LOG_PAGE_SZ(fs) ==
		    0) {
			addr &= MMAP_ADDR_MASK;
