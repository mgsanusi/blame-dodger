...
main ()
{
  int t, n, m, p, o, e, i, j, k, tmp, trip[2005], man[2005], cur, loss, ans,
    table[105];
  int stack[2005], top;
  scanf ("%d", &t);
  for (k = 0; k < t; k++)
      ...
      for (i = 0; i < 2 * m; i++)
	{
	  for (j = 2 * m - 1; j > i; j--)
	      ...
	      if (abs (trip[j]) < abs (trip[j - 1]))
		{
	      ...
	      else if (abs (trip[j]) == abs (trip[j - 1])
		       && trip[j] > trip[j - 1])
		{
		  tmp = trip[j];
		  trip[j] = trip[j - 1];
	  ...
	  if (trip[i] < 0)
	      ...
	      while (man[i])
		{
		  if (man[i] >= man[stack[top - 1]])
			...
			(loss +
			 table[-trip[i] -
			       trip[stack[top - 1]]] * man[stack[top -
								 1]]) %
			1000002013;
		      top--;
		    }
			...
			(loss +
			 table[-trip[i] -
			       trip[stack[top - 1]]] * man[i]) % 1000002013;
