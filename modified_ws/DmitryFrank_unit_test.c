	...
	for (stash_size = 0; stash_size <= 3; stash_size++) {
		...
		for (seg_size = 1; seg_size <= 16; seg_size <<= 1) {
			...
			struct segstack_opt opt = {
				.cell_size = 4,.seg_size =
				    seg_size,.stash_segs = stash_size,
			};
			...
			for (i = 0; i <= maxsize; i++) {
				ASSERT_EQ(*(int32_t *) segstack_at(&ss, i),
					  f(i));
				ASSERT_EQ(num_segs(&ss),
					  (maxsize / seg_size + 1));
			}
			...
			for (back = 1; back < maxback; back++) {
				for (i = maxsize; i >= (maxsize - back); i--) {
					const char *t =
					    test_pop(&ss, i, seg_size,
						     max_segs_num, stash_size);
					if (t != NULL) {
						return t;
					}
			...
			for (i = maxsize - dropsize; i >= 0; i--) {
				const char *t =
				    test_pop(&ss, i, seg_size, max_segs_num,
					     stash_size);
				if (t != NULL) {
					return t;
				}
