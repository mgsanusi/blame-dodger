...
IRAM NOINSTR void
__wrap_user_fatal_exception_handler (uint32_t cause)
{
/*
 ...
 */
...
#endif
  uint32_t a14, a15;
  __asm volatile ("mov.n %0, a14\n" "mov.n %1, a15":"=a" (a14), "=a" (a15));
  /* We identify the frame by searching for the exit pointer (_xt_user_exit). */
  uint32_t *sp = &cause;
  while (*sp != (uint32_t) _xt_user_exit)
    sp++;
  ...
  if (cause == EXCCAUSE_LOAD_PROHIBITED && (void *) vaddr >= MMAP_BASE)
    {
      int pc_inc =
	esp_mmap_exception_handler (vaddr, (uint8_t *) f->pc,
				    (long *) &f->a[2]);
      if (pc_inc > 0)
	{
	  f->pc += pc_inc;
...
void
esp_system_restart_low_level (void)
{
