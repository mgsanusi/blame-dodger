...
enum mg_ssl_if_result mg_ssl_if_conn_init(struct mg_connection *nc,
					  const struct mg_ssl_if_conn_params
					  *params, const char **err_msg)
{
...
bool pem_to_der(const char *pem_file, const char *der_file)
{
	...
	if (pf == NULL)
		goto clean;
	remove(der_file);
	fs_slfs_set_new_file_size(der_file +
				  MG_SSL_IF_SIMPLELINK_SLFS_PREFIX_LEN, 2048);
	df = fopen(der_file, "w");
	if (df == NULL)
		goto clean;
	while (1) {
		...
		if (writing) {
			...
			while (!isspace((unsigned int)pem_buf[l]))
				l++;
			int der_len = 0;
			cs_base64_decode((const unsigned char *)pem_buf,
					 sizeof(pem_buf), der_buf, &der_len);
			if (der_len <= 0)
				break;
...
static char *sl_pem2der(const char *pem_file)
{
	...
	int l =
	    mg_asprintf(&der_file, 0,
			MG_SSL_IF_SIMPLELINK_SLFS_PREFIX "%.*s.der",
	...
	if (ctx != NULL) {
		...
		if (ctx->ssl_cert != NULL && ctx->ssl_key != NULL) {
			...
			if (ssl_cert != NULL && ssl_key != NULL) {
				...
				LOG(LL_INFO,
				    ("CERTIFICATE_FILE_NAME %s -> %d", ssl_cert,
				     err));
				err =
				...
				LOG(LL_INFO,
				    ("PRIVATE_KEY_FILE_NAME %s -> %d", ssl_key,
				     err));
			} else {
				err = -1;
			}
		...
		if (ctx->ssl_ca_cert != NULL) {
			if (ctx->ssl_ca_cert[0] != '\0') {
				char *ssl_ca_cert =
				    sl_pem2der(ctx->ssl_ca_cert);
				if (ssl_ca_cert != NULL) {
					err =
					    sl_SetSockOpt(nc->sock,
							  SL_SOL_SOCKET,
		...
		if (ctx->ssl_server_name != NULL) {
			...
			DBG(("DOMAIN_NAME_VERIFICATION %s -> %d",
			     ctx->ssl_server_name, err));
