...
int main(int argc, char *argv[])
{
	...
	/* Compare the access permissions of the two files, including ACLs.  */
	{
		...
		if (statbuf1.st_mode != statbuf2.st_mode) {
			fprintf(stderr,
				...
				file1, file2, (unsigned int)statbuf1.st_mode,
	...
	}
	{
		...
		if (count1 != count2) {
			...
			int i;
			if (count1 > 0
			    && acl(file1, GETACL, count1, entries1) < count1) {
				fprintf(stderr,
			...
			if (count2 > 0
			    && acl(file2, GETACL, count2, entries2) < count1) {
				fprintf(stderr,
					"error retrieving the ACLs of file %s\n",
					file2);
				fflush(stderr);
			...
			for (i = 0; i < count1; i++) {
				...
				if (entries1[i].a_perm != entries2[i].a_perm) {
					fprintf(stderr,
						...
						file1, file2, i,
						(unsigned int)entries1[i].
						a_perm,
						(unsigned int)entries2[i].
						a_perm);
					return 1;
				}
			}
			...
			for (i = 0; i < count1; i++) {
				...
				if (entries1[i].a_who != entries2[i].a_who) {
					fprintf(stderr,
						...
						file1, file2, i,
						(int)entries1[i].a_who,
						(int)entries2[i].a_who);
					return 1;
				}
				if (entries1[i].a_access_mask !=
				    entries2[i].a_access_mask) {
					fprintf(stderr,
		...
		if (count1 != count2) {
			...
			struct acl_entry *entries1 =
			    XNMALLOC(count1, struct acl_entry);
			struct acl_entry *entries2 =
			    XNMALLOC(count2, struct acl_entry);
			int i;
			...
			for (i = 0; i < count1; i++) {
				...
				if (entries1[i].mode != entries2[i].mode) {
					fprintf(stderr,
						"files %s and %s: different ACL entry #%d: different permissions %03o and %03o\n",
						file1, file2, i,
						(unsigned int)entries1[i].mode,
						(unsigned int)entries2[i].mode);
					return 1;
				}
			}
		}
#if HAVE_ACLV_H			/* HP-UX >= 11.11 */
		{
			...
			count1 =
			    acl((char *)file1, ACL_CNT, NACLVENTRIES,
				dummy_entries);
			if (count1 < 0
			    && (errno == ENOSYS || errno == EOPNOTSUPP
				|| errno == EINVAL))
				count1 = 0;
			count2 =
			    acl((char *)file2, ACL_CNT, NACLVENTRIES,
				dummy_entries);
			if (count2 < 0
			    && (errno == ENOSYS || errno == EOPNOTSUPP
				|| errno == EINVAL))
		...
		if (count1 != count2) {
			...
			int i;
			if (acl((char *)file1, ACL_GET, count1, entries1) <
			    count1) {
				fprintf(stderr,
					"error retrieving the ACLs of file %s\n",
					file1);
			...
			if (acl((char *)file2, ACL_GET, count2, entries2) <
			    count1) {
				fprintf(stderr,
					"error retrieving the ACLs of file %s\n",
					file2);
		...
		if (aclx_get(file1, 0, &type1, acl1, &aclsize1, &mode1) < 0) {
		    ...
		    if (aclx_printStr
			(text1, &textsize1, acl1, aclsize1, type1, file1,
			 0) < 0) {
			fprintf(stderr,
		...
		if (aclx_get(file2, 0, &type2, acl2, &aclsize2, &mode2) < 0) {
		    ...
		    if (aclx_printStr
			(text2, &textsize2, acl2, aclsize2, type2, file2,
			 0) < 0) {
			fprintf(stderr,
				"cannot convert the ACLs of file %s to text\n",
				file2);
		...
		if (acl1.a.u_access != acl2.a.u_access
			...
			fprintf(stderr,
				...
				file1, file2, acl1.a.u_access, acl1.a.g_access,
				acl1.a.o_access, acl2.a.u_access,
				acl2.a.g_access, acl2.a.o_access);
			return 1;
		}
