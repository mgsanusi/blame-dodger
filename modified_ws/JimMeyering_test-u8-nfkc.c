      ...
      const uint8_t * expected, size_t expected_length)
{
    ...
    result =
	u8_normalize(UNINORM_NFKC, input, input_length, NULL, &length);
    if (!(result != NULL))
	return 1;
	...
	result =
	    u8_normalize(UNINORM_NFKC, input, input_length, preallocated,
			 &length);
	if (!(result != NULL))
	    return 4;
...
void test_u8_nfkc(void)
{
	...
	ASSERT(check(input, SIZEOF(input), input, SIZEOF(input)) == 0);
	ASSERT(check(decomposed, SIZEOF(decomposed), input, SIZEOF(input))
	       == 0);
    }
...
0x84, 0xD9,
	    0x84, 0xD9, 0x87, 0x20, 0xD8, 0xB9, 0xD9, 0x84, 0xD9, 0x8A,
		0xD9, 0x87,
...
'.',
	    ' ', 0xD0, 0x97, 0xD0, 0xB4, 0xD1, 0x80, 0xD0, 0xB0, 0xD0,
		0xB2, 0xD1,
	    0x81, 0xD1, 0x82, 0xD0, 0xB2, 0xD1, 0x83, 0xD0, 0xB9,
	    0xD1, 0x82, 0xD0, 0xB5, '!', ' ', 'x', '=', '(', '-', 'b',
		0xC2, 0xB1,
	    's', 'q', 'r', 't', '(', 'b', 0xC2, 0xB2, '-', '4', 'a', 'c',
		')', ')',
	    '/', '(', '2', 'a', ')', ' ', ' ', 0xE6, 0x97, 0xA5, 0xE6,
		0x9C, 0xAC,
	...
	static const uint8_t decomposed[] =
	    { 'G', 'r', 0x75, 0xCC, 0x88, 0xC3, 0x9F, ' ', 'G', 'o', 't',
't', '.',
	    ' ', 0xD0, 0x97, 0xD0, 0xB4, 0xD1, 0x80, 0xD0, 0xB0, 0xD0,
		0xB2, 0xD1,
	    0x81, 0xD1, 0x82, 0xD0, 0xB2, 0xD1, 0x83, 0xD0, 0xB8, 0xCC,
		0x86,
	    0xD1, 0x82, 0xD0, 0xB5, '!', ' ', 'x', '=', '(', '-', 'b',
		0xC2, 0xB1,
...
'.',
	...
	for (pass = 0; pass < 3; pass++) {
	    ...
	    size_t m = 100000;
	    uint8_t *input =
		(uint8_t *) malloc(2 * (2 * m - 1) * sizeof(uint8_t));
	    if (input != NULL) {
		...
		for (; repeat > 0; repeat--) {
		    ASSERT(check(input, 2 * m - 1, expected, 2 * m - 2) ==
			   0);
		    ASSERT(check(expected, 2 * m - 2, expected, 2 * m - 2)
			   == 0);
		}
		free(input);
	    }
