...
static inline int leapyear(long_int year)
{
...
static struct tm *ranged_convert(struct tm
				 *(*convert) (const time_t *, struct tm *),
		  ...
		  time_t * offset)
{
	...
	int approx_requested_biennia =
		...
		? 0 : SHR(sec, ALOG2_SECONDS_PER_BIENNIUM)));
	...
	time_t overflow_threshold =
	    (time_t_max / 3 -
	     time_t_min / 3) >> ALOG2_SECONDS_PER_BIENNIUM;
    ...
    for (t = t1 = t2 = t0, dst2 = 0;
	 (gt = guess_time_tm(year, yday, hour, min, sec, &t,
	  ...
	  t != gt); t1 = t2, t2 = t, t = gt, dst2 = tm.tm_isdst != 0)
	if (t == t1 && t != t2
	    && (tm.tm_isdst < 0 || (isdst < 0 ? dst2 <= (tm.tm_isdst != 0)
	...
	for (delta = stride; delta < delta_bound; delta += stride)
		...
		if (time_t_int_add_ok(t, delta * direction)) {
			...
			t = guess_time_tm(year, yday, hour, min, sec, &ot,
					  &otm);
			ranged_convert(convert, &t, &tm);
			goto offset_found;
		    }
