...
int main()
{
	...
	u32_possible_linebreaks(NULL, 0, "GB18030", NULL);
	{
		...
		};
		{
			...
			size_t i;
			u32_possible_linebreaks(input, SIZEOF(input), "GB18030",
						p);
			for (i = 0; i < 61; i++) {
		...
		}
		{
			char *p = (char *)malloc(SIZEOF(input));
			size_t i;
			u32_possible_linebreaks(input, SIZEOF(input), "GB2312",
						p);
			for (i = 0; i < 61; i++) {
				ASSERT(p[i] == (i == 60 ? UC_BREAK_MANDATORY :
						i == 5
	   ...
	   regular spaces (rule LB8 in Unicode TR#14 revision 26).  */
	{
		...
		for (i = 0; i < 4; i++) {
			ASSERT(p[i] ==
			       (i ==
				3 ? UC_BREAK_POSSIBLE : UC_BREAK_PROHIBITED));
		}
		free(p);
	}
	return 0;
