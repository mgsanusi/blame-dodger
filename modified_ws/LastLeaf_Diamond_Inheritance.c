...
long long accountA[N_MAX], accountB[N_MAX], f[N_MAX][N_MAX],
    sumA[N_MAX][N_MAX], sumB[N_MAX][N_MAX];
int main()
{
	...
	for (i = 1; i <= n; i++)
		...
		sumA[i][j] =
		    typeA[i] ==
		    j ? sumA[i - 1][j] + accountA[i] : sumA[i - 1][j];
	for (i = 1; i <= m; i++)
	    for (j = 1; j <= 100; j++)
		sumB[i][j] =
		    typeB[i] ==
		    j ? sumB[i - 1][j] + accountB[i] : sumB[i - 1][j];
	for (i = 1; i <= n; i++)
	    for (j = 1; j <= m; j++) {
		f[i][j] =
