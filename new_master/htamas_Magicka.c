...
int main(int argc, char **argv) {
  ...
  char comb[36][3]; int ncomb;
  char opp[28][2]; int nopp;
  char base[100];
  char elem[100]; int ne;
  scanf("%d",&T);
  for(tc=1;tc<=T;++tc) {
    scanf("%d",&C);
    for(i=0;i<C;++i) {
    ...
    for(i=0;i<N;++i) {
      ...
      while(ne) {
        for(j=0;j<C;++j) {
          if(((elem[ne-1] == comb[j][0])&&(elem[ne] == comb[j][1]))||
             ((elem[ne-1] == comb[j][1])&&(elem[ne] == comb[j][0]))) {
            --ne;
            elem[ne] = comb[j][2];
            break;              
          }
      ...
      for(j=0;j<D;++j) {
        for(k=0;k<ne;++k) {
          if(((elem[k] == opp[j][0])&&(elem[ne] == opp[j][1]))||
             ((elem[k] == opp[j][1])&&(elem[ne] == opp[j][0]))) {
            ne=-1;
            break;
          }
        }
