...
int ac[100][100];
double good[100][100];
double count(int h, double k, int num, int c1, int f2, int ti)
{
...
int Main()
{
  ...
  for (_ = 0; _ < nz; _++)
  {
    ...
    while (1)
    {
      ...
      if (e != 0)
      {
        double diff_cnt = count(h, k, f[e][y], p[e][y], f[e - 1][y], p[e - 1][y]);
      ...
      if (y != 0)
      {
        double diff_cnt = count(h, k, f[e][y], p[e][y], f[e][y - 1], p[e][y - 1]);
