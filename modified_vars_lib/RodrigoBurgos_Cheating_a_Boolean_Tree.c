...
{
  if (((((di < 0) || (di >= n)) || (y < 0)) || (y >= k)) || (mapa[di][y] == '#'))
    return 1;
...
void insert(int pos, int *nq, int ep, int caso, int flag, int amo, int di, int y, int length)
{
  if ((((di < 0) || (y < 0)) || (di >= n)) || (y >= k))
    return;
  ...
  for (scanf("%d", &casos), set = 1; set <= casos; set++)
  {
    ...
    for (len = 0;; len++)
    {
      ...
      for (pos = 0; pos < nq[len % 2]; pos++)
      {
        ...
        for (dir = 0; dir < 4; dir++)
        {
          insert(len % 2, &nq[len % 2], position[di][y][dir][0], position[di][y][dir][1], flag, amo, di, y, len);
          insert(len % 2, &nq[len % 2], ep, caso, position[di][y][dir][0], position[di][y][dir][1], di, y, len);
        }

        if (((di == ep) && (y == caso)) && (flag != 16))
        {
          insert((len + 1) % 2, &nq[(len + 1) % 2], ep, caso, flag, amo, flag, amo, len + 1);
        }

        if (((di == flag) && (y == amo)) && (ep != 16))
        {
          insert((len + 1) % 2, &nq[(len + 1) % 2], ep, caso, flag, amo, ep, caso, len + 1);
        }

        for (dir = 0; dir < 4; dir++)
          insert((len + 1) % 2, &nq[(len + 1) % 2], ep, caso, flag, amo, di + value[dir], y + dy[dir], len + 1);
