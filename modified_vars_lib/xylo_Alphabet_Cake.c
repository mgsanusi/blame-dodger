...
int64_t combatDuration(int spend, int ad, int hk, int wp, int buf, int d, int nDebuffs, int nBuff)
{
  int dragonHp = spend;
  int knightHp = hk;
...
void clearTestCase(int caseIdx, int spend, int ad, int hk, int wp, int buf, int d)
{
  ...
  if (buf > 0)
  {
    ...
    {
      int64_t a;
      a = combatDuration(spend, ad, hk, wp, buf, d, nDebuffs, i);
      if (a < n)
      {
        nBuff = i;
        n = a;
