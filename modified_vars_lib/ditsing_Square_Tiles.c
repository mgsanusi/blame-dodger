...
int Main()
{
  ...
  for (cases = 0; cases != tcases; cases++)
  {
    ...
    {
      scanf("%d %d %d", &segs[space].s, &segs[space].count, &segs[space].spd);
    }

    new_qsort(segs, now, sizeof(struct seg), segsCmp);
    int former = 0;
    int count = 0;
    for (space = 0; space < now; space++)
    {
    ...
    for (space = 0; (space < count) && (sum > 1e-10); space++)
    {
      ...
      {
        time += sum;
        time += (line[space].count - (sum * (line[space].s + r))) / ((double) line[space].s);
        sum = 0.0;
      }
