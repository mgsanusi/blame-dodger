...
void handle_case(void)
{
  ...
  {
    scanf("%d %d %d ", &deck[case_count].rp, &deck[case_count].array, &deck[case_count]._p);
  }
...
int find_good(int num_draw)
{
  ...
  while (turn > 0)
  {
    turn--;
    to_play = -1;
    for (case_count = 0; (case_count < (num_begin + draw)) && (case_count < (num_begin + num_deck)); case_count++)
    {
      if (accept_in_hand[case_count] == 1)
      {
        ...
        if (deck[case_count]._p > 0)
        {
          ...
          if (deck[to_play]._p == 0)
        {
          if ((deck[case_count].rp > 0) && ((num_draw - draw) > 0))
          {
            if ((deck[case_count].array > deck[to_play].array) || (deck[to_play].rp == 0))
            {
              to_play = case_count;
            }
            ...
            if (!((deck[to_play].rp > 0) && ((num_draw - draw) > 0)))
          {
