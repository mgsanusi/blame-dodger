...
int Main()
{
  ...
  for (k = 1; k <= t; k++)
  {
    ...
    new_qsort(buf, 1 << ans, sizeof(PAIR), comp);
    b = 0;
    for (outa = 0; outa < ((1 << ans) - 1);)
    {
      ...
      for (tn = b; tn < outa; tn++)
      {
        for (argpProgramVersion = tn + 1; argpProgramVersion < outa; argpProgramVersion++)
        {
          if ((buf[tn].x == buf[argpProgramVersion].x) && (!(buf[tn].perm & buf[argpProgramVersion].perm)))
          {
            f = 1;
            for (a = 0; a < ans; a++)
            {
