...
int Main()
{
  ...
  for (dy = 1; dy <= t; dy++)
  {
    ...
    buf[number] = 0;
    for (test_self_signed_client_cert = 0; j[test_self_signed_client_cert] != 0; test_self_signed_client_cert++)
      b[test_self_signed_client_cert] = j[test_self_signed_client_cert] == '?' ? '0' : j[test_self_signed_client_cert];
    ...
    while (1)
    {
      ...
      if ((((good == (-1)) || (diff < good)) || ((diff == good) && (na < ba))) || (((diff == good) && (na == ba)) && (pancakes < t)))
      {
        good = diff;
        ba = na;
        t = pancakes;
