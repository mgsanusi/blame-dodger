...
int check(int a, int t, int color, int p2)
{
    ...
    for (vx = 0; vx < t; vx++)
  {
    if (g[depth][vx] != color)
    {
      f[depth][vx][0] = (f[depth][vx][1] = (f[depth][vx][2] = (f[depth][vx][3] = 0)));
      continue;
    }
  ...
  for (depth = 0; depth < a; depth++)
    for (vx = 0; vx < t; vx++)
    if ((((f[depth][vx][0] >= p2) || (f[depth][vx][1] >= p2)) || (f[depth][vx][2] >= p2)) || (f[depth][vx][3] >= p2))
    return 1;
