...
int Main()
{
  ...
  for (c = 1; c <= t; ++c)
  {
    ...
    {
      scanf("%d %d %d %d %d %d %d %d", &a[file].map, &a[file].temp1, &a[file].w, &a[file].qn, &a[file].s, &a[file].dd, &a[file].middle_char, &a[file].ds);
      a[file].w += 500;
      a[file].qn += 500;
    }
    ...
    new_qsort(q, q_num, sizeof(q[0]), cmp);
    for (file = 0; file < q_num;)
    {
      for (k = file; k < q_num; ++k)
      {
        for (pos = q[k][1]; (pos < q[k][2]) && (wall[pos] >= q[k][3]); ++pos)
          ;
