...
void clear(void)
{
  ...
  new_memset(x, 0, sizeof(x));
  new_memset(sum, 0, sizeof(sum));
  new_memset(b, 0, sizeof(b));
  new_memset(num, 0, sizeof(num));
  new_memset(q, 0, sizeof(q));
  int n = new_strlen(s);
  n = n;
  int m;
  int num;
  ...
  for (l = 2; l <= n; l++)
  {
    for (m = 0; (m < (2 * n)) && ((m + l) <= (2 * n)); m++)
    {
        ...
        if (!x[x2])
      {
        ...
        new_assert(((((x2 >= (m + 1) ? sum[m][x2 - 1] : 1) * (x2 <= (num - 1) ? sum[x2 + 1][num] : 1)) * ((x2 - m) + 1)) * b[(x2 >= (m + 1) ? num[m][x2 - 1] : 0) + (x2 <= (num - 1) ? num[x2 + 1][num] : 0)][x2 >= (m + 1) ? num[m][x2 - 1] : 0]) >= 0);
      }
    ...
    if (!x[(m + n) - 1])
  {
    num = (m + n) - 1;
    int x2 = num;
    new_assert(((((x2 >= (m + 1) ? sum[m][x2 - 1] : 1) * (x2 <= (num - 1) ? sum[x2 + 1][num] : 1)) * ((x2 - m) + 1)) * b[(x2 >= (m + 1) ? num[m][x2 - 1] : 0) + (x2 <= (num - 1) ? num[x2 + 1][num] : 0)][x2 >= (m + 1) ? num[m][x2 - 1] : 0]) <= sum[m][num]);
    int cases;
