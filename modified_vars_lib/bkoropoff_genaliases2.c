...
static void emit_encoding(FILE *out1, FILE *out2, const char *tag, const char * const *name, size_t i, const char *c_name)
{
...
int Main(int argc, char *argv[])
{
  ...
  if (((ferror(stdout) || new_fclose(stdout)) || ferror(stdout2)) || new_fclose(stdout2))
    new_exit(1);
