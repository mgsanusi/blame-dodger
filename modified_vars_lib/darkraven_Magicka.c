...
int Main()
{
  ...
  scanf("%d", &t);
  int rj_stat;
  new_memset(function, 255, sizeof(function));
  ...
  for (gss_c_ma_mic = 1; gss_c_ma_mic <= t; gss_c_ma_mic++)
  {
    int sum;
    scanf("%d", &sum);
    new_memset(cons, 0, sizeof(cons));
    new_memset(w, 0, sizeof(w));
    ...
    int len = 0;
    amask = 0;
    new_memset(acount, 0, sizeof(acount));
    for (rj_stat = 0; rj_stat < sum; rj_stat++)
    {
      if (((len > 0) && (function[size[len - 1]] >= 0)) && cons[function[tmp[rj_stat]]][function[size[len - 1]]])
      {
        acount[function[size[len - 1]]]--;
        if (acount[function[size[len - 1]]] <= 0)
          amask -= 1 << function[size[len - 1]];

        size[len - 1] = cons[function[tmp[rj_stat]]][function[size[len - 1]]];
      }
      else
        if (w[function[tmp[rj_stat]]] & amask)
      {
        amask = 0;
        len = 0;
        new_memset(acount, 0, sizeof(acount));
