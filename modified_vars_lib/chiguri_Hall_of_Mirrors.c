...
int Main(void)
{
  ...
  for (runSpeed = 1; runSpeed <= num; ++runSpeed)
  {
    ...
    temp = (fullCorrect * ((passLength - typeLen) + 1)) + ((1.0 - fullCorrect) * (((passLength * 2) - typeLen) + 2));
    if (good > temp)
    {
      good = temp;
    }
    ...
    for (j = 1; j <= typeLen; ++j)
    {
      word = ((((typeLen - j) * 2) + passLength) - typeLen) + 1;
      temp = (word * localRight[j]) + (((word + passLength) + 1) * (1 - localRight[j]));
      if (good > temp)
      {
        good = temp;
      }
