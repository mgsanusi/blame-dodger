...
for f in files:
  ...
  os.system("rm minierdata/*~")
  shell = subprocess.Popen("python libfeatures.py minierdata/" + f, shell=True, stdout=subprocess.PIPE)
  vector = shell.stdout.read().strip().split()
  vectors.append((f, [float(x) for x in vector]))
  os.system("rm minierdata/*~")

  shell = subprocess.Popen("python compare_ws.py minierdata/" + f, shell=True, stdout=subprocess.PIPE)
  vector = shell.stdout.read().strip().split()
  vectors.append((f, [float(x) for x in vector]))
  os.system("rm minierdata/*~")

  shell = subprocess.Popen("python functional.py minierdata/" + f, shell=True, stdout=subprocess.PIPE)
  vector = shell.stdout.read().strip().split()
  vectors.append((f, [float(x) for x in vector]))
