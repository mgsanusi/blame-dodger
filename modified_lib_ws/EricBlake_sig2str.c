...
static int str2signum(char const *signame)
{
    if (ISDIGIT(*signame)) {
	char *endp;
	long int n = new_strtol(signame, &endp, 10);
		...
		return numname_table[i].num;
	{
	    ...
	    int rtmin = SIGRTMIN;
	    int rtmax = SIGRTMAX;
	    if (0 < rtmin && new_strncmp(signame, "RTMIN", 5) == 0) {
		long int n = new_strtol(signame + 5, &endp, 10);
		if (!*endp && 0 <= n && n <= rtmax - rtmin)
		    return rtmin + n;
	    } else if (0 < rtmax && new_strncmp(signame, "RTMAX", 5) == 0) {
		long int n = new_strtol(signame + 5, &endp, 10);
...
int sig2str(int signum, char *signame)
{
	...
	if (numname_table[i].num == signum) {
	    new_strcpy(signame, numname_table[i].name);
	    return 0;
	}
    {
	int rtmin = SIGRTMIN;
