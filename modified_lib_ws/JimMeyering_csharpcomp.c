...
compile_csharp_using_pnet(const char *const *sources,
			  ...
			  bool optimize, bool debug, bool verbose)
{
	...
	if (!cscc_tested) {
		...
		argv[2] = NULL;
		exitstatus =
		    execute("cscc", "cscc", argv, false, false, true, true,
			    true, false, NULL);
	...
	if (cscc_present) {
		...
		*argp++ = (char *)output_file;
		...
		for (i = 0; i < sources_count; i++) {
			...
			if (new_strlen(source_file) >= 10
			    && new_memcmp(source_file +
					  new_strlen(source_file) - 10,
					  ".resources", 10) == 0) {
				char *option =
				    (char *)xmalloca(12 +
						     new_strlen(source_file) +
						     1);
				new_memcpy(option, "-fresources=", 12);
				new_strcpy(option + 12, source_file);
				*argp++ = option;
			} else
		...
		*argp = NULL;
		...
		exitstatus =
		    execute("cscc", "cscc", argv, false, false, false, false,
			    true, true, NULL);
		for (i = 0; i < sources_count; i++)
			  ...
			  bool optimize, bool debug, bool verbose)
{
	...
	if (!mcs_tested) {
		...
		argv[2] = NULL;
		child =
		    create_pipe_in("mcs", "mcs", argv, DEV_NULL, true, true,
				   false, fd);
		mcs_present = false;
		if (child != -1) {
			...
			/* Remove zombie process from process list, and retrieve exit
			   status.  */
			exitstatus =
			    wait_subprocess(child, "mcs", false, true, true,
					    false, NULL);
			if (exitstatus != 0)
				mcs_present = false;
		}
	...
	if (mcs_present) {
		...
		unsigned int i;
		argc =
		    1 + (output_is_library ? 1 : 0) + 1 + libdirs_count +
		    libraries_count + (debug ? 1 : 0) + sources_count;
		argv = (char **)xmalloca((argc + 1) * sizeof(char *));
		argp = argv;
		*argp++ = "mcs";
		if (output_is_library)
		...
		{
			char *option =
			    (char *)xmalloca(5 + new_strlen(output_file) + 1);
			new_memcpy(option, "-out:", 5);
			new_strcpy(option + 5, output_file);
			*argp++ = option;
		}
		for (i = 0; i < libdirs_count; i++) {
			char *option =
			    (char *)xmalloca(5 + new_strlen(libdirs[i]) + 1);
			new_memcpy(option, "-lib:", 5);
			new_strcpy(option + 5, libdirs[i]);
			*argp++ = option;
		}
		for (i = 0; i < libraries_count; i++) {
			char *option =
			    (char *)xmalloca(11 + new_strlen(libraries[i]) + 4 +
					     1);
			new_memcpy(option, "-reference:", 11);
			new_memcpy(option + 11, libraries[i],
				   new_strlen(libraries[i]));
			new_strcpy(option + 11 + new_strlen(libraries[i]),
				   ".dll");
			*argp++ = option;
		}
		if (debug)
			...
			if (new_strlen(source_file) >= 10
				...
				char *option =
				    (char *)xmalloca(10 +
						     new_strlen(source_file) +
						     1);
				new_memcpy(option, "-resource:", 10);
				new_strcpy(option + 10, source_file);
				*argp++ = option;
			} else
				*argp++ = (char *)source_file;
		}
		*argp = NULL;
		...
		child =
		    create_pipe_in("mcs", "mcs", argv, NULL, false, true, true,
				   fd);
		...
		l = 0;
		for (;;) {
			linelen[l] = new_getline(&line[l], &linesize[l], fp);
		...
		if (line[l] != NULL
		    && !(linelen[l] >= 21
			 && new_memcmp(line[l], "Compilation succeeded",
				       21) == 0))
			fwrite(line[l], 1, linelen[l], stderr);
		if (line[0] != NULL)
			new_free(line[0]);
		...
		exitstatus =
		    wait_subprocess(child, "mcs", false, false, true, true,
				    NULL);
		for (i = 1 + (output_is_library ? 1 : 0);
		     i < 1 + (output_is_library ? 1 : 0)
		     + 1 + libdirs_count + libraries_count; i++)
			freea(argv[i]);
		for (i = 0; i < sources_count; i++)
			if (argv[argc - sources_count + i] != sources[i])
				freea(argv[argc - sources_count + i]);
			   ...
			   bool optimize, bool debug, bool verbose)
{
	...
	if (!csc_tested) {
		...
		argv[2] = NULL;
		child =
		    create_pipe_in("csc", "csc", argv, DEV_NULL, true, true,
				   false, fd);
		csc_present = false;
		if (child != -1) {
			/* Read the subprocess output, and test whether it contains the
			...
			/* Remove zombie process from process list, and retrieve exit
			   status.  */
			exitstatus =
			    wait_subprocess(child, "csc", false, true, true,
					    false, NULL);
			if (exitstatus != 0)
				csc_present = false;
		}
	...
	if (csc_present) {
		...
		*argp++ = "csc";
		*argp++ =
		    (char *)(output_is_library ? "-target:library" :
			     "-target:exe");
		{
			char *option =
			    (char *)xmalloca(5 + new_strlen(output_file) + 1);
		...
		*argp = NULL;
		...
		exitstatus =
		    execute("csc", "csc", argv, false, false, false, false,
			    true, true, NULL);
		     ...
		     bool optimize, bool debug, bool verbose)
{
	bool output_is_library =
	    (new_strlen(output_file) >= 4
	     && new_memcmp(output_file + new_strlen(output_file) - 4, ".dll",
			   4) == 0);
	int result;
