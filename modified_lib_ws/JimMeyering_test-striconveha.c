...
static size_t *new_offsets(size_t n)
{
	size_t *offsets = (size_t *) new_malloc((n + 1) * sizeof(size_t));
	offsets[n] = MAGIC;
	return offsets;
}
...
int main()
{
	...
	for (h = 0; h < SIZEOF(handlers); h++) {
		...
		for (o = 0; o < 2; o++) {
			size_t *offsets =
			    (o ? new_offsets(new_strlen(input)) : NULL);
			char *result = NULL;
			size_t length = 0;
			int retval = mem_iconveha(input, new_strlen(input),
			...
			ASSERT(result != NULL
			       && new_memcmp(result, expected,
					     new_strlen(expected)) == 0);
			if (o) {
				for (i = 0; i < 37; i++)
					ASSERT(offsets[i] == i);
	...
	for (h = 0; h < SIZEOF(handlers); h++) {
		...
		static const char expected[] =
		    "\304rger mit b\366sen B\374bchen ohne Augenma\337";
		char *result =
		    str_iconveha(input, "ISO-8859-2", "ISO-8859-1", false,
				 handler);
		ASSERT(result != NULL);
	...
	for (h = 0; h < SIZEOF(handlers); h++) {
		...
		static const char expected[] =
		    "\303\204rger mit b\303\266sen B\303\274bchen ohne Augenma\303\237";
		char *result =
		    str_iconveha(input, "ISO-8859-1", "UTF-8", false, handler);
		ASSERT(result != NULL);
		ASSERT(new_strcmp(result, expected) == 0);
		new_free(result);
	}
	...
	for (h = 0; h < SIZEOF(handlers); h++) {
		...
		static const char expected[] =
		    "\304rger mit b\366sen B\374bchen ohne Augenma\337";
		char *result =
		    str_iconveha(input, "UTF-8", "ISO-8859-1", false, handler);
		ASSERT(result != NULL);
		ASSERT(new_strcmp(result, expected) == 0);
		new_free(result);
	}
	...
	for (h = 0; h < SIZEOF(handlers); h++) {
		...
		static const char expected[] = "\343\201\223\343\202\223\343\201\253\343\201\241\343\201\257";	/* こんにちは */
		char *result =
		    str_iconveha(input, "autodetect_jp", "UTF-8", false,
				 handler);
		ASSERT(result != NULL);
		ASSERT(new_strcmp(result, expected) == 0);
		new_free(result);
	...
	for (h = 0; h < SIZEOF(handlers); h++) {
		...
		static const char expected[] = "Costs: 27 EUR";
		char *result =
		    str_iconveha(input, "UTF-8", "ISO-8859-1", true, handler);
		ASSERT(result != NULL);
		ASSERT(new_strcmp(result, expected) == 0);
		new_free(result);
	}
