...
#define SET_ADDR(dst, src)                               \
  new_memcpy((dst)->sin6.sin6_addr.s6_addr, (src)->ip6.addr, \
...
static err_t mg_lwip_tcp_conn_cb(void *arg, struct tcp_pcb *tpcb,
				 err_t err)
{
    struct mg_connection *nc = (struct mg_connection *) arg;
    DBG(("%p connect to %s:%u = %d", nc,
	 IPADDR_NTOA(ipX_2_ip(&tpcb->remote_ip)), tpcb->remote_port, err));
    if (nc == NULL) {
	tcp_abort(tpcb);
	return ERR_ARG;
    }
...
{
    struct mg_connection *nc = (struct mg_connection *) arg;
    DBG(("%p %p %u %p %p", nc, tpcb, num_sent, tpcb->unsent,
	 tpcb->unacked));
    if (nc == NULL)
	return ERR_OK;
    if ((nc->flags & MG_F_SEND_AND_CLOSE) && !(nc->flags & MG_F_WANT_WRITE)
	&& nc->send_mbuf.len == 0 && tpcb->unsent == NULL
	&& tpcb->unacked == NULL) {
	mg_lwip_post_signal(MG_SIG_CLOSE_CONN, nc);
    }
    return ERR_OK;
 ...
 */
#if LWIP_VERSION >= 0x01050000
static void mg_lwip_udp_recv_cb(void *arg, struct udp_pcb *pcb,
...
static void mg_lwip_handle_recv_udp(struct mg_connection *nc)
{
	...
	if (data != NULL) {
	    ...
	    mg_if_recv_udp_cb(nc, data, data_len,
			      (union socket_address *) sap->payload,
			      sap->len);
	    pbuf_free(sap);
	} else {
	    pbuf_free(p);
...
static err_t mg_lwip_accept_cb(void *arg, struct tcp_pcb *newtpcb,
			       err_t err)
{
	...
	("%p conn %p from %s:%u", lc, newtpcb,
	 IPADDR_NTOA(ipX_2_ip(&newtpcb->remote_ip)),
	 newtpcb->remote_port));
    if (lc == NULL) {
	tcp_abort(newtpcb);
	return ERR_ABRT;
...
{
    struct mg_lwip_if_listen_ctx *ctx =
	(struct mg_lwip_if_listen_ctx *) arg;
    struct mg_connection *nc = ctx->nc;
...
{
    struct mg_lwip_tcp_write_ctx *ctx =
	(struct mg_lwip_tcp_write_ctx *) arg;
    struct mg_connection *nc = ctx->nc;
    struct mg_lwip_conn_state *cs = (struct mg_lwip_conn_state *) nc->sock;
    struct tcp_pcb *tpcb = cs->pcb.tcp;
    int len = MIN(tpcb->mss, MIN(ctx->len, tpcb->snd_buf));
    int unsent, unacked;
    if (len == 0) {
	DBG(("%p no buf avail %u %u %u %p %p", tpcb, tpcb->acked,
	     tpcb->snd_buf, tpcb->snd_queuelen, tpcb->unsent,
	     tpcb->unacked));
 ...
 */
    ...
    unsent = (tpcb->unsent != NULL ? tpcb->unsent->len : 0);
    unacked = (tpcb->unacked != NULL ? tpcb->unacked->len : 0);
    DBG(("%p tcp_write %u = %d, %u %u", tpcb, len, cs->err, unsent,
	 unacked));
    if (cs->err != ERR_OK) {
	/*
			    ...
			    uint16_t len)
{
    ...
    new_memcpy(p->payload, data, len);
    struct udp_sendto_ctx ctx = {.upcb = upcb,.p = p,.ip = ip,.port =
	    port };
...
static void mg_lwip_send_more(struct mg_connection *nc)
{
	...
	num_sent =
	    mg_lwip_udp_send(nc, nc->send_mbuf.buf, nc->send_mbuf.len);
	DBG(("%p mg_lwip_udp_send %u = %d", nc, nc->send_mbuf.len,
	     num_sent));
    } else {
	num_sent =
	    mg_lwip_tcp_write(nc, nc->send_mbuf.buf, nc->send_mbuf.len);
	DBG(("%p mg_lwip_tcp_write %u = %d", nc, nc->send_mbuf.len,
	     num_sent));
    }
    if (num_sent == 0)
	return;
...
void mg_lwip_if_recved(struct mg_connection *nc, int len)
{
    if (nc->flags & MG_F_UDP)
	return;
    struct mg_lwip_conn_state *cs = (struct mg_lwip_conn_state *) nc->sock;
...
void mg_lwip_if_get_conn_addr(struct mg_connection *nc, int remote,
			      union socket_address *sa)
{
    new_memset(sa, 0, sizeof(*sa));
    if (nc == NULL || nc->sock == INVALID_SOCKET)
	return;
    struct mg_lwip_conn_state *cs = (struct mg_lwip_conn_state *) nc->sock;
    if (nc->flags & MG_F_UDP) {
	struct udp_pcb *upcb = cs->pcb.udp;
	if (remote) {
	    new_memcpy(sa, &nc->sa, sizeof(*sa));
	} else if (upcb != NULL) {
