...
bool pem_to_der(const char *pem_file, const char *der_file)
{
    ...
    bool writing = false;
    pf = new_fopen(pem_file, "r");
    if (pf == NULL)
	goto clean;
    remove(der_file);
    fs_slfs_set_new_file_size(der_file +
			      MG_SSL_IF_SIMPLELINK_SLFS_PREFIX_LEN, 2048);
    df = new_fopen(der_file, "w");
    if (df == NULL)
	goto clean;
    while (1) {
	char pem_buf[70];
	char der_buf[48];
	if (!new_fgets(pem_buf, sizeof(pem_buf), pf))
	    break;
	if (writing) {
	    ...
	    int l = 0;
	    while (!new_isspace((unsigned int) pem_buf[l]))
		l++;
	    int der_len = 0;
...
int sl_set_ssl_opts(struct mg_connection *nc)
{
	...
	if (ctx->ssl_server_name != NULL) {
	    err = sl_SetSockOpt(nc->sock, SL_SOL_SOCKET,
				...
				ctx->ssl_server_name,
				new_strlen(ctx->ssl_server_name));
