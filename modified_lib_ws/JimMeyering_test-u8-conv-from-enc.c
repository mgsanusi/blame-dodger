...
int main()
{
	...
	for (h = 0; h < SIZEOF(handlers); h++) {
		...
		for (o = 0; o < 2; o++) {
			...
			size_t length;
			uint8_t *result =
			    u8_conv_from_encoding("ISO-8859-1", handler,
						  input, new_strlen(input),
						  offsets,
						  NULL, &length);
			ASSERT(result != NULL);
			ASSERT(length == u8_new_strlen(expected));
			ASSERT(u8_cmp(result, expected, u8_new_strlen(expected))
			       == 0);
			if (o) {
				for (i = 0; i < 37; i++)
	...
	for (h = 0; h < SIZEOF(handlers); h++) {
		...
		for (o = 0; o < 2; o++) {
			...
			size_t length;
			uint8_t *result =
			    u8_conv_from_encoding("ISO-8859-2", handler,
						  input, new_strlen(input),
						  offsets,
						  NULL, &length);
			ASSERT(result != NULL);
	...
	for (h = 0; h < SIZEOF(handlers); h++) {
		...
		for (o = 0; o < 2; o++) {
			...
			size_t length;
			uint8_t *result =
			    u8_conv_from_encoding("autodetect_jp", handler,
						  input, new_strlen(input),
						  offsets,
						  NULL, &length);
			ASSERT(result != NULL);
