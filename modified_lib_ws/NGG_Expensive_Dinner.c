...
*/ 
	    sum =
	    get(0, a, a + k - 1, b,
		b + k - 1) - mass[a][b] - mass[a + k - 1][b] - mass[a][b + k -
								       1] -
	    mass[a + k - 1][b + k - 1];
...
 int main(void) 
{
			...
			commsum[i][0][0] = commsum[i][0][1] = commsum[i][0][2] =
			    0;
		for (i = 0; i < c; i++)
			commsum[0][i][0] = commsum[0][i][1] = commsum[0][i][2] =
			    0;
		for (i = 1; i <= r; i++) {
			for (j = 1; j <= c; j++) {
				commsum[i][j][0] =
				    commsum[i][j - 1][0] + mass[i - 1][j - 1];
				commsum[i][j][1] =
				    commsum[i][j - 1][1] + (i - 1) * mass[i -
									  1][j -
									     1];
				commsum[i][j][2] =
				    commsum[i][j - 1][2] + (j - 1) * mass[i -
									  1][j -
									     1];
