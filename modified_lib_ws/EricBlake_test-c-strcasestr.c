...
int main()
{
	   ...
	   short and occurs near the beginning.  */
	{
		...
		if (haystack != NULL) {
			...
			haystack[m] = '\0';
			for (; repeat > 0; repeat--) {
				ASSERT(c_strcasestr(haystack, needle) ==
				       haystack + 1);
			}
			new_free(haystack);
		}
	...
	{
		size_t m = 1000000;
		char *haystack = (char *)new_malloc(2 * m + 2);
		char *needle = (char *)new_malloc(m + 2);
	...
	}
	{
		...
		size_t h_len = new_strlen(h);
		char *haystack = new_malloc(h_len + 1);
		size_t i;
		ASSERT(haystack);
		for (i = 0; i < h_len - new_strlen(needle); i++) {
			const char *p;
			new_memcpy(haystack, h, h_len + 1);
			new_memcpy(haystack + i, needle,
				   new_strlen(needle) + 1);
