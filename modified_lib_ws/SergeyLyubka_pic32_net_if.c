...
static void mg_handle_send(struct mg_connection *nc)
{
	...
	if (nc->flags & MG_F_UDP) {
		...
		bytes_written =
		    TCPIP_UDP_TxPutIsReady((UDP_SOCKET) nc->sock, 0);
...
static void mg_handle_recv(struct mg_connection *nc)
{
	...
	if (nc->flags & MG_F_UDP) {
		...
		if (bytes_read != 0 &&
			...
			if (TCPIP_UDP_ArrayGet
			    ((UDP_SOCKET) nc->sock, buf,
		...
		if (bytes_read != 0) {
			if (nc->recv_mbuf_limit != -1 &&
			    nc->recv_mbuf_limit - nc->recv_mbuf.len >
			    bytes_read) {
				bytes_read =
				    nc->recv_mbuf_limit - nc->recv_mbuf.len;
			}
			buf = (uint8_t *) MG_MALLOC(bytes_read);
			if (TCPIP_TCP_ArrayGet
			    ((TCP_SOCKET) nc->sock, buf,
			     bytes_read) != bytes_read) {
				nc->flags |= MG_F_CLOSE_IMMEDIATELY;
				MG_FREE(buf);
				bytes_read = 0;
