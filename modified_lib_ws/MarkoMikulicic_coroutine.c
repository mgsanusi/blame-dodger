...
enum cr_status cr_on_iter_begin(struct cr_ctx *p_ctx)
{
	...
	size_t locals_size =
	    p_ctx->p_func_descrs[p_ctx->called_fid].locals_size;
	/*
	 * increment stack pointers
	 */
	/* make sure this function has correct `struct cr_func_desc` entry */
	new_assert(locals_size == p_ctx->call_locals_size);
	/*
	 ...
	 */
	new_assert(locals_size < sizeof(cr_zero_size_type_t));
	...
	new_memcpy(p_ctx->stack_data.buf + p_ctx->stack_data.len -
		   locals_size, p_ctx->p_arg_retval, p_ctx->call_arg_size);
	/* set function id */
...
void cr_context_init(struct cr_ctx *p_ctx,
		     ...
		     const struct cr_func_desc *p_func_descrs)
{
     ...
     * By "zero-sized" I mean `cr_zero_size_type_t`.
     */
    new_assert(arg_retval_size < sizeof(cr_zero_size_type_t));
#ifdef NDEBUG
    ...
    mbuf_append(&p_ctx->stack_ret, NULL,
		1 /*starting byte for CR_FID__NONE */ );
    p_ctx->cur_fid_idx = p_ctx->stack_ret.len;
