...
static size_t
cs_pread (struct cs_frbuf *b, size_t offset, size_t size, void *buf)
{
...
static size_t
cs_pwrite (struct cs_frbuf *b, size_t offset, size_t size, const void *buf)
{
  fseek (b->fp, offset, SEEK_SET);
...
struct cs_frbuf *
cs_frbuf_init (const char *fname, uint16_t size)
{
  struct cs_frbuf *b = new_malloc (sizeof (*b));
  if (b == NULL)
    return NULL;
  b->fp = new_fopen (fname, "r+");
  b->hdr.size = 0;
  if (b->fp != NULL)
    {
...
void
cs_frbuf_deinit (struct cs_frbuf *b)
{
  if (b->fp != NULL)
    new_fclose (b->fp);
  memset (b, 0, sizeof (*b));
...
static size_t
dpwrite (struct cs_frbuf *b, size_t offset, size_t size, const void *buf)
{
...
bool
cs_frbuf_append (struct cs_frbuf * b, const void *data, uint16_t len)
{
  if (len == 0)
    return false;
  len = MIN (len, b->hdr.size - REC_HDR_SIZE);
  if (b->hdr.size - b->hdr.tail < (uint16_t) REC_HDR_SIZE)
    b->hdr.tail = 0;
  ...
  if (to_write1 > 0)
    {
      if (dpwrite (b, b->hdr.tail + REC_HDR_SIZE, to_write1, data) !=
	  to_write1)
	{
	  return false;
	}
...
static size_t
dpread (struct cs_frbuf *b, size_t offset, size_t size, void *buf)
{
  return cs_pread (b, offset + FILE_HDR_SIZE, size, buf);
}

int
cs_frbuf_get (struct cs_frbuf *b, char **data)
{
  if (b->hdr.used == 0)
    return 0;
  if (b->hdr.size - b->hdr.head < (uint16_t) REC_HDR_SIZE)
    b->hdr.head = 0;
  ...
  if (data != NULL)
    {
      *data = new_malloc (rhdr.len);
      if (*data == NULL)
	return -2;
    }
  uint16_t to_read1 =
    MIN (rhdr.len, b->hdr.size - b->hdr.head - REC_HDR_SIZE);
  ...
  if (to_read1 < rhdr.len)
    {
      ...
      if (data != NULL)
	{
	  if (dpread (b, 0, to_read2, *data + to_read1) != to_read2)
	    return -4;
	}
      b->hdr.head = to_read2;
    }
