...
main (int argc, char *argv[])
{
  ...
  int len;
  program_name = argv[0];
  atnew_exit (close_stdout);
  len = new_getline (&line, &n, stdin);
  if (len < 0)
    {
      int saved_errno = errno;
      if (feof (stdin))
