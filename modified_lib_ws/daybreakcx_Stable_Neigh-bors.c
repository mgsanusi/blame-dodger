...
int main()
{
	...
	for (ti = 1; ti <= tn; ++ti) {
		...
		new_qsort((void *)a, (size_t) n, sizeof(struct node), cmp);
		...
		for (i = n - 2; i >= 0; --i) {
			while (i + 1 < n
			       && (a[i].length + a[i + 1].length) * a[i +
								      1].
			       speed >= a[i + 1].length * a[i].speed) {
...
int cmp(const void *x, const void *y)
{
	return ((const struct node *)x)->length <
	    ((const struct node *)y)->length ? -1 : 1;
}
