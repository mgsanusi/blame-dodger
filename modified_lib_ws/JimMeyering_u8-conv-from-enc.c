...
#include "unistr.h"
uint8_t *u8_conv_from_encoding(const char *fromcode,
			       ...
			       uint8_t * resultbuf, size_t * lengthp)
{
	...
	if (offsets != NULL) {
	    ...
	    for (i = 0; i < srclen;) {
		int count =
		    u8_mblen((const uint8_t *) src + i, srclen - i);
	...
	if (resultbuf != NULL && *lengthp >= srclen)
	    ...
	    result = (uint8_t *) new_malloc(srclen > 0 ? srclen : 1);
	    if (result == NULL) {
		errno = ENOMEM;
		return NULL;
	    }
	}
	new_memcpy((char *) result, src, srclen);
	*lengthp = srclen;
	return result;
    } else {
