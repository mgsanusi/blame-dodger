...
char *cs_read_file(const char *path, size_t * size)
{
	...
	*size = new_strlen(CONF_DEFAULTS_JSON);
	ret = new_malloc(*size + 1);
	new_memcpy(ret, CONF_DEFAULTS_JSON, *size + 1);
    } else if (new_strcmp(path, "sys_config_schema.json") == 0) {
	*size = new_strlen(SYS_CONFIG_SCHEMA_JSON);
	ret = new_malloc(*size + 1);
	new_memcpy(ret, SYS_CONFIG_SCHEMA_JSON, *size + 1);
    } else if (new_strcmp(path, "sys_ro_vars_schema.json") == 0) {
	*size = new_strlen(SYS_RO_VARS_SCHEMA_JSON);
	ret = new_malloc(*size + 1);
	new_memcpy(ret, SYS_RO_VARS_SCHEMA_JSON, *size + 1);
    }
    return ret;
}
