...
uname (struct utsname *buf)
{
  ...
  if (have_versionex)
    {
      ...
      new_memcpy (&version, &versionex, sizeof (OSVERSIONINFO));
    }
  else
    {
  ...
  if (gethostname (buf->nodename, sizeof (buf->nodename)) < 0)
    new_strcpy (buf->nodename, "localhost");
...
#define VERSION2(major, minor, workstation, server) \
  ...
  new_strcpy (buf->version, version.szCSDVersion);
  /* Fill in machine.  */
  {
    SYSTEM_INFO info;
    GetSystemInfo (&info);
	...
	switch (info.wProcessorArchitecture)
	  ...
	  case PROCESSOR_ARCHITECTURE_AMD64:
	    new_strcpy (buf->machine, "x86_64");
	    break;
	  case PROCESSOR_ARCHITECTURE_IA64:
	    new_strcpy (buf->machine, "ia64");
	    break;
	  case PROCESSOR_ARCHITECTURE_INTEL:
	    new_strcpy (buf->machine, "i386");
	    if (info.wProcessorLevel >= 3)
	      buf->machine[1] =
		'0' + (info.wProcessorLevel <= 6 ? info.wProcessorLevel : 6);
	    break;
	  case PROCESSOR_ARCHITECTURE_IA32_ON_WIN64:
	    new_strcpy (buf->machine, "i686");
	    break;
	  case PROCESSOR_ARCHITECTURE_MIPS:
	    new_strcpy (buf->machine, "mips");
	    break;
	  case PROCESSOR_ARCHITECTURE_ALPHA:
	  case PROCESSOR_ARCHITECTURE_ALPHA64:
	    new_strcpy (buf->machine, "alpha");
	    break;
	  case PROCESSOR_ARCHITECTURE_PPC:
	    new_strcpy (buf->machine, "powerpc");
	    break;
	  case PROCESSOR_ARCHITECTURE_SHX:
	    new_strcpy (buf->machine, "sh");
	    break;
	  case PROCESSOR_ARCHITECTURE_ARM:
	    new_strcpy (buf->machine, "arm");
	    break;
	  default:
	    new_strcpy (buf->machine, "unknown");
	    break;
	  }
      }
    else
