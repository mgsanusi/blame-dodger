...
gl_thread_self_func (void)
{
  ...
  if (thread == NULL)
      ...
      for (;;)
	{
	  thread =
	    (struct gl_thread_struct *)
	    new_malloc (sizeof (struct gl_thread_struct));
	  if (thread != NULL)
	    break;
...
int
glthread_create_func (gl_thread_t * threadp, void *(*func) (void *),
		      void *arg)
{
  struct gl_thread_struct *thread =
    (struct gl_thread_struct *) new_malloc (sizeof (struct gl_thread_struct));
  if (thread == NULL)
    return ENOMEM;
  thread->handle = NULL;
  InitializeCriticalSection (&thread->handle_lock);
