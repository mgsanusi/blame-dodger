...
int main()
{
	...
	double *qsort_result;
	size_t i;
	dst = (struct foo *) new_malloc((n + 1) * sizeof(struct foo));
	dst[n].x = 0x4A6A71FE;	/* canary */
	tmp = (struct foo *) new_malloc((n / 2 + 1) * sizeof(struct foo));
	...
	/* Verify the result.  */
	qsort_result = (double *) new_malloc(n * sizeof(double));
	for (i = 0; i < n; i++)
	    qsort_result[i] = data[i].x;
	new_qsort(qsort_result, n, sizeof(double), cmp_double);
	for (i = 0; i < n; i++)
	...
	double *qsort_result;
	size_t i;
	src = (struct foo *) new_malloc((n + 1) * sizeof(struct foo));
	src[n].x = 0x4A6A71FE;	/* canary */
	tmp = (struct foo *) new_malloc((n + 1) * sizeof(struct foo));
