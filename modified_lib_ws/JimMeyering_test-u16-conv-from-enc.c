...
int main()
{
	...
	for (h = 0; h < SIZEOF(handlers); h++) {
		...
		{
			0xC4, 'r', 'g', 'e', 'r', ' ', 'm', 'i', 't', ' ', 'b',
			    0xF6, 's',
			'e', 'n', ' ', 'B', 0xFC, 'b', 'c', 'h', 'e', 'n', ' ',
			    'o', 'h',
		...
		for (o = 0; o < 2; o++) {
			...
			size_t length;
			uint16_t *result =
			    u16_conv_from_encoding("ISO-8859-1", handler,
						   input, new_strlen(input),
						   offsets,
						   NULL, &length);
			ASSERT(result != NULL);
			ASSERT(length == SIZEOF(expected));
			ASSERT(u16_cmp(result, expected, SIZEOF(expected)) ==
			       0);
			if (o) {
				for (i = 0; i < 37; i++)
					ASSERT(offsets[i] == i);
	...
	for (h = 0; h < SIZEOF(handlers); h++) {
		...
		for (o = 0; o < 2; o++) {
			...
			size_t length;
			uint16_t *result =
			    u16_conv_from_encoding("ISO-8859-2", handler,
						   input, new_strlen(input),
						   offsets,
						   NULL, &length);
			ASSERT(result != NULL);
	...
	for (h = 0; h < SIZEOF(handlers); h++) {
		...
		for (o = 0; o < 2; o++) {
			...
			size_t length;
			uint16_t *result =
			    u16_conv_from_encoding("autodetect_jp", handler,
						   input, new_strlen(input),
						   offsets,
						   NULL, &length);
			ASSERT(result != NULL);
