      ...
      const uint32_t * expected, size_t expected_length)
{
	...
	result =
	    u32_normalize(UNINORM_NFKD, input, input_length, NULL, &length);
	if (!(result != NULL))
		return 1;
	...
	if (expected_length > 0) {
		...
		preallocated =
		    (uint32_t *) new_malloc(length * sizeof(uint32_t));
		result =
		    u32_normalize(UNINORM_NFKD, input, input_length,
				  preallocated, &length);
		if (!(result != NULL))
			return 4;
...
0x0647, 0x0020,
			0x0639, 0x0644, 0x064A, 0x0647, 0x0020, 0x0648, 0x0633,
			    0x0644, 0x0645
		};
		ASSERT(check(input, SIZEOF(input), expected, SIZEOF(expected))
		       == 0);
		...
		for (pass = 0; pass < 3; pass++) {
			...
			if (input != NULL) {
				...
				for (i = 0; i < m2; i++)
					*p++ = 0x0300;
				for (; repeat > 0; repeat--)
					ASSERT(check(input, m, expected, m) ==
					       0);
				new_free(input);
			}
		}
