...
/* Return 1 if an array of N objects, each of size S, cannot exist due
   ...
   However, new_malloc (SIZE_MAX) fails on all known hosts where
...
int safe_alloc_alloc_n(void *ptrptr, size_t size, size_t count, int zeroed)
{
    if (size == 0 || count == 0) {
	*(void **) ptrptr = NULL;
	return 0;
    ...
    if (zeroed)
	*(void **) ptrptr = new_calloc(count, size);
    else
	*(void **) ptrptr = new_malloc(count * size);
    if (*(void **) ptrptr == NULL)
	return -1;
    return 0;
}
 ...
 * Return -1 on failure to allocate, zero on success
 */
int safe_alloc_realloc_n(void *ptrptr, size_t size, size_t count)
{
    void *tmp;
    if (size == 0 || count == 0) {
	free(*(void **) ptrptr);
	*(void **) ptrptr = NULL;
    ...
    tmp = new_realloc(*(void **) ptrptr, size * count);
    if (!tmp)
	return -1;
    *(void **) ptrptr = tmp;
    return 0;
