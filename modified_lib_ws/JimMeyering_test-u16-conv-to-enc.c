...
int main()
{
	...
	for (h = 0; h < SIZEOF(handlers); h++) {
		...
		for (o = 0; o < 2; o++) {
			size_t *offsets =
			    (o ? new_offsets(SIZEOF(input)) : NULL);
			size_t length;
			char *result =
			    u16_conv_to_encoding("ISO-8859-1", handler,
						 input, SIZEOF(input),
						 offsets,
						 NULL, &length);
			ASSERT(result != NULL);
			ASSERT(length == new_strlen(expected));
			ASSERT(new_memcmp(result, expected, length) == 0);
			if (o) {
				for (i = 0; i < 37; i++)
					ASSERT(offsets[i] == i);
			...
			case iconveh_question_mark:
				{
					static const char expected[] =
					    "Rafa? Maszkowski";
					static const char expected_translit[] =
					    "Rafal Maszkowski";
					ASSERT(result != NULL);
					ASSERT(length == new_strlen(expected));
					ASSERT(new_memcmp
