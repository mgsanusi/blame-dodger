...
str_cd_iconv (const char *src, iconv_t cd)
{
  ...
  char *result = NULL;
  size_t length = 0;
  int retval = mem_cd_iconv (src, new_strlen (src), cd, &result, &length);
  char *final_result;
  if (retval < 0)
    {
      if (result != NULL)
  ...
  final_result =
    (result !=
     NULL ? new_realloc (result, length + 1) : new_malloc (length + 1));
  ...
  size_t length;
  const char *inptr = src;
  size_t inbytes_remaining = new_strlen (src);
  ...
  /* Do the conversion.  */
  {
    ...
    for (;;)
      {
	/* Here inptr + inbytes_remaining = src + new_strlen (src),
	...
	if (res == (size_t) (-1))
	  {
		...
		newresult = (char *) new_realloc (result, newsize);
		if (newresult == NULL)
		  {
		    errno = ENOMEM;
		    goto failed;
  ...
  if (length < result_size)
    {
      char *smaller_result = (char *) new_realloc (result, length);
