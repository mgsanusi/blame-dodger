...
drop_dir (const char *fname)
{
   ...
   */
  if (fname[0] == '/' && new_strchr (fname + 1, '/') == NULL)
    {
      fname++;
    }
  if (fname != old_fname)
...
int
set_spiffs_errno (spiffs * fs, const char *op, int res)
{
...
int
spiffs_vfs_open (spiffs * fs, const char *path, int flags, int mode)
{
  (void) mode;
  spiffs_mode sm = 0;
  int rw = (flags & 3);
  ...
  if (fs->encrypted)
      ...
      if (fd >= 0 && (rw & O_WRONLY))
	{
	  ...
	  if (fm->plain_size == DEFAULT_PLAIN_SIZE || (flags & O_TRUNC))
		...
		{
		  LOG (LL_ERROR,
		       ("Corrupted encrypted file %s (es %u ps %u)", s.name,
...
#endif
    {
      if (flags & O_APPEND)
	sm |= SPIFFS_APPEND;
      return set_spiffs_errno (fs, path,
			       SPIFFS_open (fs, drop_dir (path), sm, 0));
    }
}

int
spiffs_vfs_close (spiffs * fs, int fd)
{
  return set_spiffs_errno (fs, "close", SPIFFS_close (fs, fd));
}

ssize_t
spiffs_vfs_read (spiffs * fs, int fd, void *dstv, size_t size)
{
#if CS_SPIFFS_ENABLE_ENCRYPTION
  spiffs_stat s;
  s32_t r = SPIFFS_fstat (fs, fd, &s);
  if (r < 0)
    return set_spiffs_errno (fs, "read", r);
  ...
  if (fs->encrypted && !fm->encryption_not_started)
	  ...
	  (s.size % CS_SPIFFS_ENCRYPTION_BLOCK_SIZE != 0))
	{
      ...
      if (plain_off < 0)
	return set_spiffs_errno (fs, "enc_read_tell", plain_off);
      s32_t block_off =
      ...
      while (num_read < size)
	{
	  ...
	  to_copy -= to_skip;
	  if (to_copy > (size - num_read))
	    to_copy = (size - num_read);
	  new_memcpy (dst, block + to_skip, to_copy);
    ...
    out_enc_err:
      LOG (LL_ERROR,
	   ("Corrupted encrypted file %s (es %u ps %u)", s.name, s.size,
	    fm->plain_size));
      errno = ENXIO;
      return -1;
    }
...
size_t
spiffs_vfs_write (spiffs * fs, int fd, const void *datav, size_t size)
{
#if CS_SPIFFS_ENABLE_ENCRYPTION
  spiffs_stat s;
  s32_t r = SPIFFS_fstat (fs, fd, &s);
  ...
  if (fs->encrypted && !fm->encryption_not_started)
	  ...
	  (s.size % CS_SPIFFS_ENCRYPTION_BLOCK_SIZE != 0))
	{
      ...
      s32_t plain_off = SPIFFS_tell (fs, fd);
      if (plain_off < 0)
	return set_spiffs_errno (fs, "enc_write_tell", plain_off);
      if (!(fm->not_o_append))
	plain_off = fm->plain_size;
      s32_t block_off =
	(plain_off - (plain_off % CS_SPIFFS_ENCRYPTION_BLOCK_SIZE));
      uint8_t block[CS_SPIFFS_ENCRYPTION_BLOCK_SIZE];
      ...
      if (prefix_len > 0)
	{
	  ...
	  if (r != prefix_len)
		...
		{
		  LOG (LL_ERROR,
		       ("Expected to read %d @ %d, got %d", prefix_len,
			block_off, r));
		  goto out_enc_err;
		}
	    }
      ...
      while (num_written < size)
	{
	  ...
	  if (to_copy > (size - num_written))
	    to_copy = size - num_written;
	  new_memcpy (block + to_skip, data, to_copy);
	  ...
	  if (r != sizeof (block))
		...
		{
		  LOG (LL_ERROR,
		       ("Expected to write %d @ %d, got %d", sizeof (block),
			block_off, r));
		  goto out_enc_err;
		}
	    }
	  LOG (LL_DEBUG,
	       ("enc_write s %d po %d bo %d ts %d tc %d nw %d", size,
      ...
      if (new_plain_off > fm->plain_size)
	{
      ...
      LOG (LL_DEBUG,
	   ("%d @ %d => %d, po %d ps %d", size, plain_off, num_written,
...
static void
spiffs_vfs_xlate_stat (spiffs * fs, spiffs_stat * ss, struct stat *st)
{
  st->st_ino = ss->obj_id;
...
int
spiffs_vfs_stat (spiffs * fs, const char *path, struct stat *st)
{
  int res;
  spiffs_stat ss;
  new_memset (st, 0, sizeof (*st));
...
int
spiffs_vfs_fstat (spiffs * fs, int fd, struct stat *st)
{
  int res;
  spiffs_stat ss;
  new_memset (st, 0, sizeof (*st));
...
off_t
spiffs_vfs_lseek (spiffs * fs, int fd, off_t offset, int whence)
{
#if CS_SPIFFS_ENABLE_ENCRYPTION
  spiffs_stat s;
  s32_t r = SPIFFS_fstat (fs, fd, &s);
  if (r < 0)
    return set_spiffs_errno (fs, "lseek", r);
  const struct file_meta *fm = (const struct file_meta *) s.meta;
  if (fm->plain_size != DEFAULT_PLAIN_SIZE)
    {
...
#endif /* CS_SPIFFS_ENABLE_ENCRYPTION */
  return set_spiffs_errno (fs, "lseek",
			   SPIFFS_lseek (fs, fd, offset, whence));
}

int
spiffs_vfs_rename (spiffs * fs, const char *src, const char *dst)
{
  int res;
  src = drop_dir (src);
  dst = drop_dir (dst);
...
int
spiffs_vfs_unlink (spiffs * fs, const char *path)
{
  path = drop_dir (path);
#if CS_SPIFFS_ENABLE_ENCRYPTION
  char enc_path[SPIFFS_OBJ_NAME_LEN];
...
DIR *
spiffs_vfs_opendir (spiffs * fs, const char *name)
{
  ...
  if ((sd = (struct spiffs_dir *) new_calloc (1, sizeof (*sd))) == NULL)
    {
      errno = ENOMEM;
      return NULL;
    }
...
struct dirent *
spiffs_vfs_readdir (spiffs * fs, DIR * dir)
{
...
#endif
    {
      new_memcpy (sd->de.d_name, sd->sde.name, SPIFFS_OBJ_NAME_LEN);
    }
  (void) fs;
  return &sd->de;
}
...
int
spiffs_vfs_closedir (spiffs * fs, DIR * dir)
{
  struct spiffs_dir *sd = (struct spiffs_dir *) dir;
  if (dir != NULL)
    {
  ...
  do
    {
      ...
      LOG (LL_DEBUG,
	   ("GC result %d del pages %u -> %u", r, (unsigned int) del_before,
	    (unsigned int) del_after));
    }
...
#if CS_SPIFFS_ENABLE_ENCRYPTION
bool
spiffs_vfs_enc_name (const char *name, char *enc_name, size_t enc_name_size)
{
  uint8_t tmp[SPIFFS_OBJ_NAME_LEN];
  char tmp2[SPIFFS_OBJ_NAME_LEN];
  int name_len = new_strlen (name);
  int enc_name_len = 0;
  ...
  new_memcpy (tmp, name, name_len);
  new_memset (tmp + name_len, 0, sizeof (tmp) - name_len);
  ...
  new_strncpy (enc_name, tmp2, enc_name_size);
  return true;
}

bool
spiffs_vfs_dec_name (const char *enc_name, char *name, size_t name_size)
{
  int i;
  char tmp[SPIFFS_OBJ_NAME_LEN];
  int enc_name_len = 0;
  cs_base64_decode ((const unsigned char *) enc_name, new_strlen (enc_name),
		    tmp, &enc_name_len);
  if (enc_name_len == 0
  ...
  for (i = 0; i < enc_name_len; i += CS_SPIFFS_ENCRYPTION_BLOCK_SIZE)
				     ...
				     CS_SPIFFS_ENCRYPTION_BLOCK_SIZE))
	{
      ...
      if (name_size - i < CS_SPIFFS_ENCRYPTION_BLOCK_SIZE)
	return false;
      new_memcpy (name + i, tmp + i, CS_SPIFFS_ENCRYPTION_BLOCK_SIZE);
    }
  while (i < name_size)
    {
      name[i++] = '\0';
...
spiffs_vfs_enc_fs (spiffs * fs)
{
  ...
  while (SPIFFS_readdir (&d, &e) != NULL)
	...
	{
	  LOG (LL_ERROR,
	       ("%s: update_meta failed: %d", e.name, SPIFFS_errno (fs)));
	  goto out;
	}
      int enc_size = 0;
      fm->plain_size = 0;
      while (true)
	{
	  ...
	  if (n < 0)
	    {
	      ...
	      if (err == SPIFFS_ERR_END_OF_OBJECT)
		break;
	  ...
	  if (SPIFFS_lseek (fs, fd, fm->plain_size, SPIFFS_SEEK_SET) !=
	    ...
	    {
	      LOG (LL_ERROR,
		   ("%s: seek failed: %d", e.name, SPIFFS_errno (fs)));
	      goto out;
	    }
	  ...
	  if (r != sizeof (block))
	    {
	      LOG (LL_ERROR,
		   ("%s: write failed: %d", e.name, SPIFFS_errno (fs)));
	      goto out;
	    }
	  fm->plain_size += n;
	...
	{
	  LOG (LL_ERROR,
	       ("%s: final update_meta failed: %d", e.name,
		SPIFFS_errno (fs)));
	  goto out;
	}
      SPIFFS_close (fs, fd);
	...
	{
	  LOG (LL_ERROR,
	       ("%s: rename failed: %d", e.name, SPIFFS_errno (fs)));
	  goto out;
	}
      /*
