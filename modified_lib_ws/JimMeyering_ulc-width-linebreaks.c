		      ...
		      const char *o, const char *encoding, char *p)
{
  if (n > 0)
	...
	return u8_width_linebreaks ((const uint8_t *) s, n, width,
				    start_column, at_end_columns, o, encoding,
				    p);
      else
	{
	  /* Convert the string to UTF-8 and build a translation table
	     from offsets into s to offsets into the translated string.  */
	  size_t *offsets = (size_t *) new_malloc (n * sizeof (size_t));
	  if (offsets != NULL)
	    {
	      uint8_t *t;
	      size_t m;
		  ...
		  if (m == 0 || memory != NULL)
		    {
		      ...
		      if (o != NULL)
			{
			  new_memset (o8, UC_BREAK_UNDEFINED, m);
			  for (i = 0; i < n; i++)
			...
			u8_width_linebreaks (t, m, width, start_column,
		      ...
		      new_memset (p, UC_BREAK_PROHIBITED, n);
		      for (i = 0; i < n; i++)
			if (offsets[i] != (size_t) (-1))
			  p[i] = q[offsets[i]];
		      new_free (memory);
  ...
  if (argc == 2)
    {
      ...
      int width = new_atoi (argv[1]);
      char *input = read_file (stdin);
      int length = strlen (input);
      char *breaks = new_malloc (length);
      int i;
      ulc_width_linebreaks (input, length, width, 0, 0, NULL,
