...
static char const *maybe_null(char const *s)
{
    return s ? s : "NULL";
}

static bool same_diag(char const *s, char const *t)
{
    if (s == NULL && t == NULL)
	return true;
    if (s == NULL || t == NULL)
...
int main(void)
{
	...
	for (uid = 1200; 0 < uid; uid--) {
	    ...
	    new_assert(T[j].in == NULL);
	    new_assert(T[j + 1].in == NULL);
	    len = strlen(pw->pw_name);
	    /* Store "username:" in T[j].in.  */
	    {
		char *t = xmalloc(len + 1 + 1);
		new_memcpy(t, pw->pw_name, len);
		t[len] = ':';
		t[len + 1] = '\0';
		T[j].in = t;
	    }
	...
	if (uid != T[i].uid || gid != T[i].gid) {
	    printf("%s mismatch (-: expected uid,gid; +:actual)\n"
		   ...
		   (unsigned long int) uid, (unsigned long int) gid);
	    fail = 1;
	}
	if (!diag && !T[i].result)
	    continue;
