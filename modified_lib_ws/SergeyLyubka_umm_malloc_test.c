 ...
 */
static void *
wrap_calloc (size_t num, size_t size)
{
  free_blocks_check ();
...
test_integrity_check (void)
{
  ...
  for (size = 1; size <= 16; size++)
	...
	corruption_cnt = 0;
	char *ptr = wrap_malloc (size);
	new_memset (ptr, 0xfe, size + 8 /* size of umm_block */ );
	/*
...
random_stress (void)
{
  ...
  for (idx = 0; idx < 100000; ++idx)
	...
	case 8:
	  {
	    size_t size = new_rand () % 40;
	    ptr_array[i] = wrap_realloc (ptr_array[i], size);
	    new_memset (ptr_array[i], 0xfe, size);
	    break;
	  }
	case 9:
	case 10:
