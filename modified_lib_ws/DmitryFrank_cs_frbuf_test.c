...
void
bin2hex (const char *src, int src_len, char *dst)
{
  int i = 0;
  for (i = 0; i < src_len; i++)
    {
...
#define ASSERT_FILE_EQ(expected_header, expected_data)                \
  do {                                                                \
    FILE *fp = new_fopen(TEST_FILE, "r");                                 \
    if (fp == NULL) FAIL("unable to open file", __LINE__);            \
    fseek(fp, 0, SEEK_END);                                           \
    long fsize = ftell(fp);                                           \
    fseek(fp, 0, SEEK_SET);                                           \
    char *buf = new_malloc(fsize);                                        \
    ...
    char *dbuf = new_calloc(fsize * 2 + 1, 1);                            \
...
static const char *
run_tests (const char *filter, double *total_elapsed)
{
  remove (TEST_FILE);
