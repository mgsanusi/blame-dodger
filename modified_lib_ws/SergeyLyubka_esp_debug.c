...
enum mgos_init_result mgos_debug_udp_init(const char *dst)
{
    ...
    if (upcb == NULL
	|| udp_bind(upcb, IP_ADDR_ANY, 0 /* any port */ ) != ERR_OK) {
	return MGOS_INIT_DEBUG_INIT_FAILED;
    }
    if (s_upcb != NULL)
	udp_remove(s_upcb);
...
void udp_flush_cb(void *arg)
{
    ...
    if (p != NULL) {
	new_memcpy(p->payload, s_to_send.buf, s_to_send.len);
...
void mgos_debug_udp_send(const struct mg_str prefix,
			 const struct mg_str data)
{
    if (s_upcb == NULL)
	return;
    if (s_to_send.len + prefix.len + data.len > 1500)
	return;
    mgos_lock();
