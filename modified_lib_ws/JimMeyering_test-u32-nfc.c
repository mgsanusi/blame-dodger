       ...
       const uint32_t * expected, size_t expected_length)
{
  ...
  if (expected_length > 0)
    {
      ...
      result =
	u32_normalize (UNINORM_NFC, input, input_length, preallocated,
		       &length);
      if (!(result != NULL))
	return 4;
  ...
  /* Test return conventions with resultbuf large enough.  */
  {
    ...
    preallocated = (uint32_t *) new_malloc (length * sizeof (uint32_t));
    result =
      u32_normalize (UNINORM_NFC, input, input_length, preallocated, &length);
    if (!(result != NULL))
      return 8;
...
test_u32_nfc (void)
{
  ...
  {				/* ANGSTROM SIGN */
    ...
    ASSERT (check
	    (decomposed, SIZEOF (decomposed), expected,
	     SIZEOF (expected)) == 0);
    ASSERT (check (expected, SIZEOF (expected), expected, SIZEOF (expected))
	    == 0);
  }
  ...
  /* Check that the sorting is not O(n²) but O(n log n).  */
  {
	...
	size_t repeat = 1;
	size_t m = 100000;
	uint32_t *input = (uint32_t *) new_malloc (2 * m * sizeof (uint32_t));
	if (input != NULL)
	  {
