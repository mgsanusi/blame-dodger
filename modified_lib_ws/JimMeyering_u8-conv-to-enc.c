...
char *u8_conv_to_encoding(const char *tocode,
			  enum iconv_ilseq_handler handler,
			  const uint8_t * src, size_t srclen,
			  size_t * offsets, char *resultbuf, size_t * lengthp)
{
	if (STRCASEEQ(tocode, "UTF-8", 'U', 'T', 'F', '-', '8', 0, 0, 0, 0)) {
		...
		if (resultbuf != NULL && *lengthp >= srclen)
			...
			result = (char *)new_malloc(srclen > 0 ? srclen : 1);
			if (result == NULL) {
				errno = ENOMEM;
				return NULL;
			}
		}
		new_memcpy(result, (const char *)src, srclen);
		*lengthp = srclen;
		return result;
	} else {
