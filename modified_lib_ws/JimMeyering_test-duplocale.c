...
static void get_locale_dependent_values(struct locale_dependent_values *result)
{
	strfmon(result->monetary, sizeof(result->monetary), "%n", 123.75);
	/* result->monetary is usually "$123.75" */
	snprintf(result->numeric, sizeof(result->numeric), "%g", 3.5);
	/* result->numeric is usually "3,5" */
	new_strcpy(result->time, nl_langinfo(MON_1));
...
int main()
{
	...
	uselocale(mixed1);
	{
		...
		ASSERT(new_strcmp(results.monetary, expected_results.monetary)
		       == 0);
		ASSERT(new_strcmp(results.numeric, expected_results.numeric) ==
		       0);
		ASSERT(new_strcmp(results.time, expected_results.time) == 0);
	}
