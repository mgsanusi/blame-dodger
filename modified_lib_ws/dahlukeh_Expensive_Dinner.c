...
 void handleCase(void)
{
	    ...
	    0.5 * ((lowerP[i].x - lowerP[i - 1].x) *
		   (absF(lowerP[i - 1].y - lowerP[i].y)));
    }
    for (i = 1; i < numU; i++) {
	h = 1000.0 - min(upperP[i - 1].y, upperP[i].y);
	area -= h * (upperP[i].x - upperP[i - 1].x);
	area +=
	    0.5 * ((upperP[i].x - upperP[i - 1].x) *
		   (absF(upperP[i - 1].y - upperP[i].y)));
...
  double calcAr(double place)
{
	...
	(upperP[i].y - upperP[i - 1].y) / (upperP[i].x - upperP[i - 1].x);
    y = upperP[i - 1].y + (dy * (place - upperP[i - 1].x));
    h = 1000.0 - min(upperP[i - 1].y, y);
    area -= h * (place - upperP[i - 1].x);
    area +=
	0.5 * ((place - upperP[i - 1].x) * (absF(upperP[i - 1].y - y)));
