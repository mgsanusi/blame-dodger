...
handleCase (void)
{
  ...
  scanf ("%d %d %d %d %d ", &length, &walkSpeed, &runSpeed, &maxTime,
	  &numBelts);
  double time = 0.0;
  int i;
  for (i = 0; i < numBelts; i++)
    {
      scanf ("%d %d %d ", &allBelts[i].start, &allBelts[i].end,
	      &allBelts[i].extra);
...
   FILE * new_freopen (const char *filename, const char *mode,
			   FILE * stream)
{
