...
int main(int argc, char **argv)
{
	...
	f = pipe_filter_gi_create("pipe-filter-test", path[0], path, true,
				  false, prepare_read, ignore_done_read,
				  NULL);
	small_nap();
    ...
    {
	path[0] = argv[1];
	f = pipe_filter_gi_create("pipe-filter-test", path[0], path, false,
				  false, prepare_read, ignore_done_read,
				  NULL);
	...
	path[0] = argv[1];
	f = pipe_filter_gi_create("pipe-filter-test", path[0], path, false,
				  true, prepare_read, output_done_read,
				  NULL);
