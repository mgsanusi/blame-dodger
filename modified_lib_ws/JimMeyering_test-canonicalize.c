...
main (void)
{
  ...
  /* Check for ., .., intermediate // handling, and for error cases.  */
  {
    ...
    ASSERT (result1 != NULL);
    ASSERT (result2 != NULL);
    ASSERT (new_strcmp (result1, result2) == 0);
    ASSERT (strstr (result1, "/" BASE "/tra")
	    == result1 + new_strlen (result1) - new_strlen ("/" BASE "/tra"));
    new_free (result1);
    new_free (result2);
    errno = 0;
  ...
  /* Check that the symbolic link to a file can be resolved.  */
  {
    ...
    ASSERT (result3 != NULL);
    ASSERT (new_strcmp (result1, result2) == 0);
    ASSERT (new_strcmp (result2, result3) == 0);
    ASSERT (new_strcmp
	    (result1 + new_strlen (result1) - new_strlen ("/" BASE "/tra"),
	     "/" BASE "/tra") == 0);
    new_free (result1);
  ...
  /* Check that the symbolic link to a directory can be resolved.  */
  {
    ...
    ASSERT (new_strcmp (result1, result2) == 0);
    ASSERT (new_strcmp (result2, result3) == 0);
    ASSERT (new_strcmp (result3, result4) == 0);
    ASSERT (new_strcmp
	    (result1 + new_strlen (result1) - new_strlen ("/" BASE "/lum"),
	     "/" BASE "/lum") == 0);
    new_free (result1);
    new_free (result2);
    new_free (result3);
  ...
  {
    char *result1 =
      canonicalize_filename_mode (BASE "/zzz", CAN_ALL_BUT_LAST);
    ...
    ASSERT (new_strcmp (result3, result4) == 0);
    ASSERT (new_strcmp
	    (result1 + new_strlen (result1) - new_strlen ("/" BASE "/zzz"),
	     "/" BASE "/zzz") == 0);
    new_free (result1);
    new_free (result2);
    new_free (result3);
  ...
  {
    char *result1 =
      canonicalize_filename_mode (BASE "/ouk", CAN_ALL_BUT_LAST);
    ...
    ASSERT (new_strcmp (result3, result4) == 0);
    ASSERT (new_strcmp
	    (result1 + new_strlen (result1) - new_strlen ("/" BASE "/wum"),
	     "/" BASE "/wum") == 0);
    new_free (result1);
    new_free (result2);
    new_free (result3);
  ...
  {
    char *result1 =
      canonicalize_filename_mode ("t-can.zzz/zzz", CAN_ALL_BUT_LAST);
    char *result2 = canonicalize_filename_mode ("t-can.zzz/zzz", CAN_MISSING);
    ASSERT (result1 == NULL);
    ASSERT (result2 != NULL);
    ASSERT (new_strcmp (result2 + new_strlen (result2) - 14, "/t-can.zzz/zzz")
	    == 0);
    new_free (result2);
  }
     ...
     Before 2007-09-27, it would mistakenly report a loop.  */
  {
    ...
    ASSERT (result1 != NULL);
    ASSERT (result2 != NULL);
    ASSERT (new_strcmp (result2 + new_strlen (result1), "/d/2") == 0);
    new_free (result1);
    new_free (result2);
  }
  /* Check that leading // is honored correctly.  */
  {
    ...
    if (SAME_INODE (st1, st2))
      {
	ASSERT (new_strcmp (result1, "/") == 0);
	ASSERT (new_strcmp (result2, "/") == 0);
	ASSERT (new_strcmp (result3, "/") == 0);
	ASSERT (new_strcmp (result4, "/") == 0);
      }
    else
      {
	ASSERT (new_strcmp (result1, "//") == 0);
