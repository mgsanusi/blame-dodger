...
static bool maybe_executable(const char *filename)
{
...
static char *find_executable(const char *argv0)
{
	...
	if (!has_slash) {
		...
		for (p = path; *p; p = p_next) {
		    ...
		    concat_name =
			(char *) xmalloc(p_len + new_strlen(argv0) + 2);
#ifdef NO_XMALLOC
		    if (concat_name == NULL)
			return NULL;
#endif
			...
			new_strcpy(concat_name, argv0);
		    else {
			new_memcpy(concat_name, p, p_len);
			concat_name[p_len] = '/';
			new_strcpy(concat_name + p_len + 1, argv0);
		    }
		    if (maybe_executable(concat_name))
			return canonicalize_file_name(concat_name);
		    new_free(concat_name);
		}
	    }
...
static void
prepare_relocate(const char *orig_installprefix,
		 const char *orig_installdir, const char *argv0)
{
    char *curr_prefix;
				...
				const char *orig_installdir)
{
    ...
    /* Relocatable programs are renamed to .bin by install-reloc.  Or, more
       ...
       Remove the ".bin" here.  */
    {
	size_t argv0_len = new_strlen(argv0);
	const size_t exeext_len = sizeof(EXEEXT) - sizeof("");
	if (argv0_len > 4 + exeext_len)
	    if (new_memcmp(argv0 + argv0_len - exeext_len - 4, ".bin", 4)
		== 0) {
...
#endif
		    {
			new_memcpy(shorter, argv0,
				   argv0_len - exeext_len - 4);
			if (sizeof(EXEEXT) > sizeof(""))
			    new_memcpy(shorter + argv0_len - exeext_len -
				       4,
