...
static struct visit *cur_visit(struct mbuf *stack)
{
    if (stack->len == 0)
	return NULL;
    return (struct visit *) (stack->buf + stack->len -
			     sizeof(struct visit));
}
...
{
    struct ub_ctx *ctx = (struct ub_ctx *) new_calloc(1, sizeof(*ctx));
    mbuf_init(&ctx->out, 0);
    mbuf_init(&ctx->stack, 0);
    return ctx;
}

void ub_ctx_new_free(struct ub_ctx *ctx)
{
    mbuf_new_free(&ctx->out);
    mbuf_new_free(&ctx->stack);
    struct link *l, *tmp;
    for (l = ctx->head; l != NULL; l = tmp) {
	tmp = l->next;
	if (l->is_array) {
	    /* l points to the beginning of the array object */
	    mbuf_new_free(&((struct ub_arr *) l)->elems);
	}
	new_free(l);
    }
    new_free(ctx);
...
{
    new_assert(obj.kind == UBJSON_TYPE_OBJECT);
...
static void ub_render_cont(struct ub_ctx *ctx)
{
	...
	if (obj.kind == UBJSON_TYPE_NULL) {
	    ...
	    cs_ubjson_emit_string(buf, obj.val.s->s,
				  new_strlen(obj.val.s->s));
		...
		s = cur->v.p->name->s;
		cs_ubjson_emit_object_key(buf, s, new_strlen(s));
...
void ub_render(struct ub_ctx *ctx, ub_val_t root, ub_cb_t cb,
	       void *user_data)
{
    ctx->cb = cb;
...
{
    struct link *res = new_calloc(1, size);
    res->next = ctx->head;
    ctx->head = res;
    return res;
}
...
struct ub_str *create_ub_str(struct ub_ctx *ctx, const struct mg_str s)
{
    ...
    new_memcpy((char *) res->s, s.p, s.len + 1);
    return res;
}
...
{
    new_assert(o.kind == UBJSON_TYPE_OBJECT);
    struct prop *p;
...
{
    new_assert(o.kind == UBJSON_TYPE_ARRAY);
    return o.val.a->elems.len / sizeof(ub_val_t);
}

static ub_val_t ub_array_get(ub_val_t a, size_t idx)
{
    new_assert(a.kind == UBJSON_TYPE_ARRAY);
    new_assert((idx * sizeof(ub_val_t)) < a.val.a->elems.len);
    ub_val_t res;
    new_memcpy(&res, a.val.a->elems.buf + (idx * sizeof(ub_val_t)),
	       sizeof(ub_val_t));
    return res;
}
