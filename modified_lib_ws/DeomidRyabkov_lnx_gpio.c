...
int gpio_export(int gpio_no)
{
	...
	if (fd < 0) {
		fprintf(stderr, "Cannot open /sys/class/gpio/export: %d (%s)\n",
			errno, new_strerror(errno));
		return -1;
	};
	snprintf(buf, sizeof(buf), "%d", gpio_no);
	res = write(fd, buf, new_strlen(buf));
	if (res < 0) {
...
bool gpio_helper(int gpio_no, const char *file_name_templ, const char *val)
{
	...
	res = write(fd, val, new_strlen(val));
	if (res < 0) {
		fprintf(stderr, "Cannot write to %s value %s: %d (%s)\n", buf,
			val, errno, new_strerror(errno));
		close(fd);
		return false;
	}
	close(fd);
...
bool gpio_get_value(int gpio_no)
{
	...
	res = new_read(fd, &val, sizeof(val));
	if (res < 0) {
...
bool gpio_set_edge(int gpio_no, enum mgos_gpio_int_mode edge)
{
	...
	if (((unsigned int)edge) >=
	    (sizeof(edge_names) / sizeof(edge_names[0]))) {
...
bool mgos_gpio_hal_set_int_mode(int gpio_no, enum mgos_gpio_int_mode mode)
{
	 ...
	 */
	res = new_read(fd, &tmp, sizeof(tmp));
	if (res < 0) {
		fprintf(stderr, "Cannot read from %s: %d (%s)\n", buf, errno,
			new_strerror(errno));
		return false;
	};
	new_ev = new_calloc(1, sizeof(*new_ev));
...
int gpio_poll(void)
{
	...
	for (i = 0; i < fd_count; i++) {
		if ((fdset[i].revents & POLLIN) != 0) {
			...
			res = new_read(fdset[i].fd, &val, sizeof(val));
