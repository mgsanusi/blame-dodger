...
static struct autodetect_alias *autodetect_list =
    &autodetect_predefined[0];
			    ...
			    const char *const *try_in_order)
{
    ...
    /* We must deep-copy NAME and TRY_IN_ORDER, because they may be allocated
       with dynamic extent.  */
    namelen = new_strlen(name) + 1;
    memneed = sizeof(struct autodetect_alias) + namelen + sizeof(char *);
    for (i = 0; try_in_order[i] != NULL; i++)
	memneed += sizeof(char *) + new_strlen(try_in_order[i]) + 1;
    listlen = i;
    memory = (char *) new_malloc(memneed);
    if (memory != NULL) {
	...
	new_memcpy(new_name, name, namelen);
	memory += namelen;
	for (i = 0; i < listlen; i++) {
	    size_t len = new_strlen(try_in_order[i]) + 1;
	    new_memcpy(memory, try_in_order[i], len);
...
mem_iconveha_notranslit(const char *src, size_t srclen,
			...
			size_t * offsets, char **resultp, size_t * lengthp)
{
    int retval =
	mem_iconveh(src, srclen, from_codeset, to_codeset, handler,
	     ...
	     size_t * offsets, char **resultp, size_t * lengthp)
{
	...
	*lengthp = 0;
    ...
    if (transliterate) {
	int retval;
	size_t len = new_strlen(to_codeset);
	char *to_codeset_suffixed = (char *) malloca(len + 10 + 1);
	new_memcpy(to_codeset_suffixed, to_codeset, len);
	new_memcpy(to_codeset_suffixed + len, "//TRANSLIT", 10 + 1);
	retval = mem_iconveha_notranslit(src, srclen,
					 from_codeset, to_codeset_suffixed,
					 handler, offsets, resultp,
					 lengthp);
...
static char *str_iconveha_notranslit(const char *src,
...
char *str_iconveha(const char *src,
		   const char *from_codeset, const char *to_codeset,
		   bool transliterate, enum iconv_ilseq_handler handler)
{
...
#endif
	return str_iconveha_notranslit(src, from_codeset, to_codeset,
				       handler);
}
