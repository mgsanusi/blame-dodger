...
static void *worker_thread(void *arg)
{
	...
	for (i = 0; i < KEYS_COUNT; i++) {
		values[i] =
		    (((unsigned int)new_rand() >> 3) % 1000000) * THREAD_COUNT +
		    id;
	...
	for (i = 0; i < KEYS_COUNT; i++) {
		unsigned int *ptr =
		    (unsigned int *)new_malloc(sizeof(unsigned int));
		*ptr = values[i];
	...
	for (repeat = REPEAT_COUNT; repeat > 0; repeat--) {
		dbgprintf("Worker %p doing value swapping\n",
			  gl_thread_self_pointer());
		i = ((unsigned int)new_rand() >> 3) % KEYS_COUNT;
		j = ((unsigned int)new_rand() >> 3) % KEYS_COUNT;
		if (i != j) {
	...
	for (i = 0; i < KEYS_COUNT; i++)
		if ((*(unsigned int *)gl_tls_get(mykeys[i]) % THREAD_COUNT) !=
		    id)
			abort();
