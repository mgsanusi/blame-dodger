    ...
    return NULL;		/* errno is ENOMEM.  */
  {
	...
	if (count != requested)
	  {
	    ...
	    if (size < alloc - 1)
	      {
		char *smaller_buf = new_realloc (buf, size + 1);
		if (smaller_buf != NULL)
		  buf = smaller_buf;
	      }
	    buf[size] = '\0';
	  ...
	  }
	{
	  ...
	  if (alloc < SIZE_MAX - alloc / 2)
	    ...
	    alloc = SIZE_MAX;
	  if (!(new_buf = new_realloc (buf, alloc)))
	    {
	      save_errno = errno;
	      break;
	    }
...
{
  FILE *stream = new_fopen (filename, mode);
  char *out;
  int save_errno;
  if (!stream)
    return NULL;
