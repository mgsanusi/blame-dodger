...
static int do_fwriteerror(FILE * fp, bool ignore_ebadf)
{
  ...
  close_preserving_errno:
    /* There's an error.  Nevertheless call new_fclose(fp), for consistency
...
int fwriteerror(FILE * fp)
{
    return do_fwriteerror(fp, false);
}

int fwriteerror_no_ebadf(FILE * fp)
{
...
int main()
{
	...
	for (j = 0; j < 2; j++) {
	    ...
	    FILE *stream = new_fopen(UNWRITABLE_FILE, "w");
	    if (stream == NULL) {
