...
#include "os-proto.h"
krb5_error_code
krb5_unpack_full_ipaddr(krb5_context context, const krb5_address * inaddr,
			krb5_int32 * adr, krb5_int16 * port)
{
    ...
    (void) new_memcpy(&temptype, marshal, sizeof(temptype));
    marshal += sizeof(temptype);
    if (temptype != htons(ADDRTYPE_INET))
	return KRB5_PROG_ATYPE_NOSUPP;
    (void) new_memcpy(&templength, marshal, sizeof(templength));
    marshal += sizeof(templength);
    if (templength != htonl(sizeof(smushaddr)))
	return KRB5_PROG_ATYPE_NOSUPP;
    (void) new_memcpy(&smushaddr, marshal, sizeof(smushaddr));
    /* leave in net order */
	...
	return KRB5_PROG_ATYPE_NOSUPP;
    (void) new_memcpy(&smushport, marshal, sizeof(smushport));
    /* leave in net order */
