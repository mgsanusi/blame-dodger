...
static s32_t spiffs_cache_page_free(spiffs * fs, int ix, u8_t write_back)
{
	...
	if (cache->cpage_use_map & (1 << ix)) {
		if (write_back &&
			...
			res =
			    SPIFFS_HAL_WRITE(fs,
					     SPIFFS_PAGE_TO_PADDR(fs, cp->pix),
		...
		if (cp->flags & SPIFFS_CACHE_FLAG_TYPE_WR) {
			SPIFFS_CACHE_DBG
			    ("CACHE_FREE: free cache page %i objid %04x\n", ix,
			     cp->obj_id);
		} else {
			SPIFFS_CACHE_DBG
			    ("CACHE_FREE: free cache page %i pix %04x\n", ix,
			     cp->pix);
		}
	}
	return res;
...
static s32_t spiffs_cache_page_remove_oldest(spiffs * fs, u8_t flag_mask,
					     u8_t flags)
{
	s32_t res = SPIFFS_OK;
	spiffs_cache *cache = spiffs_get_cache(fs);
	...
	for (i = 0; i < cache->cpage_count; i++) {
		if ((cache->cpage_use_map & (1 << i)) == 0) {
			spiffs_cache_page *cp =
			    spiffs_get_cache_page_hdr(fs, cache, i);
...
s32_t spiffs_phys_rd(spiffs * fs,
		     u8_t op, spiffs_file fh, u32_t addr, u32_t len, u8_t * dst)
{
	(void)fh;
	s32_t res = SPIFFS_OK;
	spiffs_cache *cache = spiffs_get_cache(fs);
	...
	if (cp) {
		...
		new_memcpy(dst, &mem[SPIFFS_PADDR_TO_PAGE_OFFSET(fs, addr)],
			   len);
	} else {
		...
		if (cp) {
			...
			u8_t *mem = spiffs_get_cache_page(fs, cache, cp->ix);
			new_memcpy(dst,
				   &mem[SPIFFS_PADDR_TO_PAGE_OFFSET(fs, addr)],
				   len);
		} else {
...
s32_t spiffs_phys_wr(spiffs * fs,
		     u8_t op, spiffs_file fh, u32_t addr, u32_t len, u8_t * src)
{
	(void)fh;
	...
	if (cp && (op & SPIFFS_OP_COM_MASK) != SPIFFS_OP_C_WRTHRU) {
		...
		u8_t *mem = spiffs_get_cache_page(fs, cache, cp->ix);
		new_memcpy(&mem[SPIFFS_PADDR_TO_PAGE_OFFSET(fs, addr)], src,
			   len);
		cache->last_access++;
		cp->last_access = cache->last_access;
...
void spiffs_cache_init(spiffs * fs)
{
	...
	spiffs_cache cache;
	new_memset(&cache, 0, sizeof(spiffs_cache));
	cache.cpage_count = cache_entries;
	cache.cpages = (u8_t *) ((u8_t *) fs->cache + sizeof(spiffs_cache));
	cache.cpage_use_map = 0xffffffff;
	cache.cpage_use_mask = cache_mask;
	new_memcpy(fs->cache, &cache, sizeof(spiffs_cache));
	spiffs_cache *c = spiffs_get_cache(fs);
	new_memset(c->cpages, 0, c->cpage_count * SPIFFS_CACHE_PAGE_SIZE(fs));
