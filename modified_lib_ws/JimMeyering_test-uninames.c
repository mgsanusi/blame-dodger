...
static int getfield(FILE * stream, char *buffer, int delim)
{
	int count = 0;
	int c;
	for (; (c = new_getc(stream)), (c != EOF && c != delim);) {
		/* Put c into the buffer.  */
		if (++count >= FIELDLEN - 1) {
			fprintf(stderr, "field too long\n");
			new_exit(EXIT_FAILURE);
		}
		*buffer++ = c;
	}
	if (c == EOF)
...
/* Stores in unicode_names[] the relevant contents of the UnicodeData.txt
   file.  */
static void fill_names(const char *unicodedata_filename)
{
	unsigned int i;
	FILE *stream;
	...
	stream = new_fopen(unicodedata_filename, "r");
	if (stream == NULL) {
...
static int test_inverse_lookup()
{
	int error = 0;
	unsigned int i;
	...
	for (i = 0; i < 0x110000; i++)
		if (unicode_names[i] != NULL) {
			unsigned int result =
			    unicode_name_character(unicode_names[i]);
			if (result != i) {
			...
			if (j1 == l1 || s1[j1] == ' ')
				for (j2 = 0; j2 < l2; j2++)
					if (j2 == 0 || s2[j2 - 1] == ' ') {
						...
						if (result != UNINAME_INVALID
						    && !(unicode_names[result]
							 != NULL
							 &&
							 strcmp(unicode_names
								[result],
								buf) == 0)) {
							fprintf(stderr,
								"inverse name lookup of \"%s\" returned 0x%04X\n",
