 ...
 *      For x in [0.625,1]
 *              asin(x) = pi/2-2*asin(new_sqrt((1-x)/2))
 *
 * Special cases:
  ...
  if (y >= 1.0L)		/* |x|>= 1 */
	...
	if (huge + y > one)
      ...
      p = (((((((((pS9 * t
		 ...
		 + pS6) * t
		+ pS5) * t
	       + pS4) * t + pS3) * t + pS2) * t + pS1) * t + pS0) * t;
      q = ((((((((t
		  + qS8) * t
		 + qS7) * t
		+ qS6) * t
	       + qS5) * t + qS4) * t + qS3) * t + qS2) * t + qS1) * t + qS0;
      return x + x * (p / q);
    }
  else if (y < 0.625)		/* 0.625 */
    {
      ...
      p = ((((((((((rS10 * t
		 ...
		 + rS6) * t
		+ rS5) * t
	       + rS4) * t + rS3) * t + rS2) * t + rS1) * t + rS0) * t;
      q = (((((((((t
		   + sS9) * t
		  + sS8) * t
		 + sS7) * t
		+ sS6) * t
	       + sS5) * t + sS4) * t + sS3) * t + sS2) * t + sS1) * t + sS0;
      t = asinr5625 + p / q;
    }
  else
    t = pio2_hi + pio2_lo - 2 * asinl (new_sqrtl ((1 - y) / 2));
  return t * sign;
}
...
{
  printf ("%.18Lg %.18Lg\n",
	  asinl (1.0L), 1.5707963267948966192313216916397514420984L);
  printf ("%.18Lg %.18Lg\n",
	  asinl (0.7071067811865475244008443621048490392848L),
	  0.7853981633974483096156608458198757210492L);
  printf ("%.18Lg %.18Lg\n",
	  asinl (0.5L), 0.5235987755982988730771072305465838140328L);
  printf ("%.18Lg %.18Lg\n",
