...
int FUNC(DOUBLE x)
{
	...
	return (((m.word[0] ^ 0x8000U) << 16) | m.
		word[1] | (m.word[2] >> 16)) != 0;
    else
	return 1 & ~(m.word[0] >> 15);
#else
	...
	if (((m.word[EXPBIT0_WORD] ^ nan.word[EXPBIT0_WORD])
	    ...
	    == 0)
	    return (new_memcmp(&m.value, &plus_inf, SIZE) != 0
		    && new_memcmp(&m.value, &minus_inf, SIZE) != 0);
	else
	    return 0;
    }
#endif
	...
	memory_double m1;
	memory_double m2;
	new_memset(&m1.value, 0, SIZE);
	new_memset(&m2.value, 0, SIZE);
	m1.value = x;
	m2.value = x + (x ? 0.0L : -0.0L);
	if (new_memcmp(&m1.value, &m2.value, SIZE) != 0)
	    return 1;
#endif
	return 0;
    } else
