	 ...
	 int (*func) (char const *file1, char const *file2))
{
	...
	char *proc_file1 =
	    ((fd1 == AT_FDCWD || IS_ABSOLUTE_FILE_NAME(file1))
	     ? (char *) file1 : openat_proc_name(proc_buf1, fd1, file1));
	if (proc_file1) {
	    char proc_buf2[OPENAT_BUFFER_SIZE];
	    char *proc_file2 =
		((fd2 == AT_FDCWD || IS_ABSOLUTE_FILE_NAME(file2))
		 ? (char *) file2 : openat_proc_name(proc_buf2, fd2,
						     file2));
	    if (proc_file2) {
	...
	new_free(cwd);		/* Reduced to case 3.  */
	...
	new_free(cwd);		/* Reduced to case 12.  */
    }
