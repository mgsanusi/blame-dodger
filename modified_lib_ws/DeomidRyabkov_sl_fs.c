...
drop_dir (const char *fname, bool * is_slfs)
{
  if (is_slfs != NULL)
    {
      *is_slfs = (new_strncmp (fname, "SL:", 3) == 0);
      if (*is_slfs)
	fname += 3;
    }
  /* Drop "./", if any */
...
#if MG_TI_NO_HOST_INTERFACE
int
new_read (int fd, char *buf, unsigned count)
{
#else
int
_new_read (int fd, void *buf, int count)
{
#endif
  int r = -1;
  switch (fd_type (fd))
...
#ifdef CC3200_FS_SPIFFS
    case FD_SPIFFS:
      r = fs_spiffs_new_read (fd - SPIFFS_FD_BASE, buf, count);
      break;
#endif
#ifdef MG_FS_SLFS
    case FD_SLFS:
      r = fs_slfs_new_read (fd - SLFS_FD_BASE, buf, count);
      break;
#endif
    }
  DBG (("new_read(%d, %u) = %d", fd, count, r));
  return r;
}
...
#else
int
_write (int fd, const void *buf, int count)
{
#endif
  int r = -1;
  switch (fd_type (fd))
