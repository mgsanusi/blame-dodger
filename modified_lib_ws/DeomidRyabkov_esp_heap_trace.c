...
extern void *__real_umm_new_realloc(void *ptr, size_t size);
extern
void *new_realloc(void *ptr, size_t size);
void __real_umm_free(void *ptr);
...
/*
 * malloc/free log item. Does not support new_realloc (since reallocs don't happen
 ...
 */
NOINSTR static void echo_log_malloc_req(size_t size, int shim)
{
    esp_exc_printf("hl{m,%u,%d,", (unsigned int) size, shim);
}

NOINSTR static void echo_log_zalloc_req(size_t size, int shim)
{
    esp_exc_printf("hl{z,%u,%d,", (unsigned int) size, shim);
}

NOINSTR static void echo_log_calloc_req(size_t size, int shim)
{
...
NOINSTR static void echo_log_alloc_res(void *ptr)
{
...
NOINSTR static void echo_log_free(void *ptr, int shim)
{
#if MGOS_ENABLE_CALL_TRACE
 ...
 */
NOINSTR static void echo_log_item(struct log_item *item)
{
    switch (item->type) {
	...
	break;
    case ITEM_TYPE_FREE:
	echo_log_free(PTR_INFLATE((unsigned int) item->ptr),
		      item->our_shim);
	break;
    default:
	abort();
 ...
 */
void *__wrap_umm_new_realloc(void *ptr, size_t size)
{
    void *ret = NULL;
    if (uart_initialized) {
	flush_log_items();
    ...
    ret = __real_umm_new_realloc(ptr, size);
    if (uart_initialized) {
	mgos_wdt_feed();
