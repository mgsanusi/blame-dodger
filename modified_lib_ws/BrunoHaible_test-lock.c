...
random_account (void)
{
  return ((unsigned int) new_rand () >> 3) % ACCOUNT_COUNT;
}
  ...
  for (repeat = REPEAT_COUNT; repeat > 0; repeat--)
    {
      ...
      i1 = random_account ();
      i2 = random_account ();
      value = ((unsigned int) new_rand () >> 3) % 10;
      account[i1] += value;
      account[i2] -= value;
...
test_once (void)
{
  ...
  for (repeat = 0; repeat <= REPEAT_COUNT; repeat++)
	...
	break;
      ...
      new_memcpy (&once_control, &fresh_once, sizeof (gl_once_t));
