 ...
 */
IRAM NOINSTR static uint8_t
read_unaligned_byte (uint8_t * addr)
{
  ...
  if (addr >= (uint8_t *) MMAP_BASE && addr < (uint8_t *) MMAP_END)
	...
	{
	  printf
	    ("MMAP invalid address %p: block %d, off %d, pages %d, desc %d\n",
...
IRAM NOINSTR void *new_memset (void *ptr, int value, size_t num);
void *new_calloc (size_t num, size_t size);
int
esp_mmap_exception_handler (uint32_t vaddr, uint8_t * pc, long *pa2)
{
  int pc_inc = 0;
  uint32_t instr =
    (uint32_t) read_unaligned_byte (pc) | ((uint32_t)
					   read_unaligned_byte (pc + 1) << 8);
...
alloc_mmap_desc (void)
{
  ...
  mbuf_append (&mmap_descs_mbuf, NULL,
	       MMAP_DESCS_ADD_SIZE * sizeof (struct mmap_desc));
  mmap_descs = (struct mmap_desc *) mmap_descs_mbuf.buf;
  new_memset (mmap_descs_mbuf.buf + old_len, 0,
	      mmap_descs_mbuf.len - old_len);
...
mmap (void *addr, size_t len, int prot, int flags, int fd, off_t offset)
{
  ...
  cur_mmap_desc = desc;
  desc->pages = 0;
  desc->blocks = (uint32_t *) new_calloc (sizeof (uint32_t), pages);
  if (desc->blocks == NULL)
    {
  ...
  desc->base = MMAP_ADDR_FROM_DESC (desc);
  {
    ...
    if (t != (int32_t) len)
      {
	LOG (LL_ERROR,
	     ("Spiffs dummy read failed: expected len: %d, actual: %d", len,
	      t));
	return NULL;
      }
    /*
	  ...
	  if (mmap_descs[i].base == addr)
	    {
	      ...
	      new_memset (&mmap_descs[i], 0, sizeof (mmap_descs[i]));
	      return 0;
	    }
	}
    }
 ...
 */
void
esp_spiffs_on_page_move_hook (spiffs * fs, spiffs_file fh,
...
int
esp_spiffs_dummy_read (spiffs * fs, u32_t addr, u32_t size, u8_t * dst)
{
  (void) size;
  if (dst >= DUMMY_MMAP_BUFFER_START && dst < DUMMY_MMAP_BUFFER_END)
	...
	{
	  addr &= MMAP_ADDR_MASK;
	  cur_mmap_desc->blocks[cur_mmap_desc->pages++] =
	    FLASH_BASE (fs) + addr;
	}
      return 1;
    }
