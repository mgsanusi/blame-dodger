...
test_function (char *(*my_asnprintf) (char *, size_t *, const char *, ...))
{
  /* Test the support of the 'U' conversion specifier for Unicode strings.  */
  {
	...
	my_asnprintf (NULL, &length, "%U %d", unicode_string, 33, 44, 55);
      ASSERT (result != NULL);
      ASSERT (new_strcmp (result, "Rafa? Maszkowski 33") == 0
	      || new_strcmp (result, "Rafal Maszkowski 33") == 0);
  ...
  }
  {
    ...
    {				/* FLAG_LEFT.  */
      size_t length;
      char *result =
	my_asnprintf (NULL, &length, "%-20llU %d", unicode_string, 33, 44,
		      55);
      ASSERT (result != NULL);
      ASSERT (new_strcmp (result, "Rafa? Maszkowski     33") == 0
	      || new_strcmp (result, "Rafal Maszkowski     33") == 0);
    ...
    {				/* FLAG_ZERO: no effect.  */
      size_t length;
      char *result =
	my_asnprintf (NULL, &length, "%020llU %d", unicode_string, 33, 44,
		      55);
      ASSERT (result != NULL);
      ASSERT (new_strcmp (result, "    Rafa? Maszkowski 33") == 0
	      || new_strcmp (result, "    Rafal Maszkowski 33") == 0);
  ...
  /* Test the support of the 's' conversion specifier for strings.  */
  {
	...
	my_asnprintf (NULL, &length, "%s %d", locale_string, 33, 44, 55);
      ASSERT (result != NULL);
      ASSERT (new_strcmp (result, "\304rger 33") == 0);
      ASSERT (length == new_strlen (result));
      new_free (result);
    }
    {				/* Width.  */
