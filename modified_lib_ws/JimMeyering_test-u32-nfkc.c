      ...
      const uint32_t * expected, size_t expected_length)
{
    ...
    result =
	u32_normalize(UNINORM_NFKC, input, input_length, NULL, &length);
    if (!(result != NULL))
	return 1;
	...
	preallocated = (uint32_t *) new_malloc(length * sizeof(uint32_t));
	result =
	    u32_normalize(UNINORM_NFKC, input, input_length, preallocated,
			  &length);
	if (!(result != NULL))
	    return 4;
...
'.', ' ',
	...
	for (pass = 0; pass < 3; pass++) {
	    ...
	    size_t m = 100000;
	    uint32_t *input =
		(uint32_t *) new_malloc(2 * m * sizeof(uint32_t));
	    if (input != NULL) {
