...
#undef vfprintf
{
    CALL_WITH_SIGPIPE_EMULATION(int, vfprintf(stream, format, args),
				ret == EOF)
}
#endif
...
int fnew_puts(const char *string, FILE * stream)
#undef fputs
{
    CALL_WITH_SIGPIPE_EMULATION(int, fnew_puts(string, stream), ret == EOF)
}
...
{
    FILE *stream = stdout;
    CALL_WITH_SIGPIPE_EMULATION(int, new_puts(string), ret == EOF)
}
