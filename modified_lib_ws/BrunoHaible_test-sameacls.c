...
main (int argc, char *argv[])
{
  ...
  /* Compare the contents of the two files.  */
  {
    ...
    if (new_memcmp (contents1, contents2, size1) != 0)
      {
  ...
  }
  {
	...
	for (i = 0; i < count1; i++)
	  {
		...
		fprintf (stderr,
			 ...
			 file1, file2, i,
			 (unsigned int) entries1[i].a_access_mask,
			 (unsigned int) entries2[i].a_access_mask);
		return 1;
	      }
	...
	for (i = 0; i < count1; i++)
	  {
		...
		fprintf (stderr,
			 ...
			 file1, file2, i, (int) entries1[i].uid,
			 (int) entries2[i].uid);
		return 1;
	      }
		...
		fprintf (stderr,
			 ...
			 file1, file2, i, (int) entries1[i].gid,
			 (int) entries2[i].gid);
		return 1;
	      }
		...
		fprintf (stderr,
			 "files %s and %s: different ACL entry #%d: different permissions %03o and %03o\n",
			 file1, file2, i, (unsigned int) entries1[i].mode,
	...
	if (errno == ENOSYS)
      ...
      if (aclx_printStr (text1, &textsize1, acl1, aclsize1, type1, file1, 0) <
	  0)
      {
	...
	if (errno == ENOSYS)
      ...
      if (aclx_printStr (text2, &textsize2, acl2, aclsize2, type2, file2, 0) <
	  0)
      {
	fprintf (stderr, "cannot convert the ACLs of file %s to text\n",
		 file2);
	...
	fprintf (stderr,
		 ...
		 acl1.a.o_access, acl2.a.u_access, acl2.a.g_access,
		 acl2.a.o_access);
	return 1;
      }
    if (new_memcmp (acl1.a.acl_ext, acl2.a.acl_ext, acl1.a.acl_len) != 0)
      {
