...
int main()
{
	...
	for (h = 0; h < SIZEOF(handlers); h++) {
		...
		uint32_t *result =
		    u32_strconv_from_encoding(input, "ISO-8859-1", handler);
		ASSERT(result != NULL);
	...
	for (h = 0; h < SIZEOF(handlers); h++) {
		...
		uint32_t *result =
		    u32_strconv_from_encoding(input, "ISO-8859-2", handler);
		ASSERT(result != NULL);
		ASSERT(u32_strcmp(result, expected) == 0);
		new_free(result);
	}
	...
	for (h = 0; h < SIZEOF(handlers); h++) {
		...
		uint32_t *result =
		    u32_strconv_from_encoding(input, "autodetect_jp", handler);
		ASSERT(result != NULL);
		ASSERT(u32_strcmp(result, expected) == 0);
		new_free(result);
	}
