...
static void test_locale_name(void)
{
	...
	unsetenv("LANG");
	setlocale(LC_ALL, "");
	ASSERT(new_strcmp(gl_locale_name(LC_MESSAGES, "LC_MESSAGES"),
			  gl_locale_name_default()) == 0);
	ASSERT(new_strcmp(gl_locale_name(LC_NUMERIC, "LC_NUMERIC"),
			  gl_locale_name_default()) == 0);
	...
	unsetenv("LANG");
	setlocale(LC_ALL, "");
	ASSERT(new_strcmp(gl_locale_name(LC_MESSAGES, "LC_MESSAGES"), "C") ==
	       0);
	unsetenv("LC_ALL");
	setenv("LC_CTYPE", "C", 1);
	...
	if (setlocale(LC_ALL, "") != NULL) {
		...
		ASSERT(new_strcmp(name, "de_DE.UTF-8") == 0);
		name = gl_locale_name(LC_MESSAGES, "LC_MESSAGES");
		ASSERT(new_strcmp(name, "fr_FR.UTF-8") == 0);
	}
	unsetenv("LC_ALL");
	unsetenv("LC_CTYPE");
	setenv("LC_MESSAGES", "fr_FR.UTF-8", 1);
	...
	if (setlocale(LC_ALL, "") != NULL) {
		name = gl_locale_name(LC_CTYPE, "LC_CTYPE");
		ASSERT(new_strcmp(name, gl_locale_name_default()) == 0);
		name = gl_locale_name(LC_MESSAGES, "LC_MESSAGES");
		ASSERT(new_strcmp(name, "fr_FR.UTF-8") == 0);
	}
#if HAVE_NEWLOCALE
	   ...
	   thread locale, and that the name is the right one for each.  */
	{
		...
		for (i = 0; i < SIZEOF(categories); i++) {
			...
			if (locale != NULL) {
				locale =
				    newlocale(category_mask, "de_DE.UTF-8",
					      locale);
				if (locale != NULL) {
					unsigned int j;
					uselocale(locale);
...
static void test_locale_name_thread(void)
{
	/* Get into a defined state,  */
	setlocale(LC_ALL, "en_US.UTF-8");
#if HAVE_NEWLOCALE
	...
	/* Check that gl_locale_name_thread considers the thread locale.  */
	{
		...
		if (locale != NULL) {
			...
			ASSERT(new_strcmp(name, "fr_FR.UTF-8") == 0);
			name =
			    gl_locale_name_thread(LC_MESSAGES, "LC_MESSAGES");
			ASSERT(new_strcmp(name, "fr_FR.UTF-8") == 0);
		}
	}
	   ...
	   indefinite extent.  */
	{
		...
		for (j = 0; j < SIZEOF(choices); j++) {
			locale_t locale =
			    newlocale(LC_ALL_MASK, choices[j], NULL);
			...
			if (locale != NULL) {
				...
				for (i = 0; i < SIZEOF(categories); i++) {
					unsaved_names[j][i] =
					    gl_locale_name_thread(categories[i].
								  cat,
								  categories[i].
								  string);
					saved_names[j][i] =
					    strdup(unsaved_names[j][i]);
				}
				uselocale(LC_GLOBAL_LOCALE);
				freelocale(locale);
			}
...
static void test_locale_name_posix(void)
{
	const char *name;
	/* Get into a defined state,  */
	setlocale(LC_ALL, "en_US.UTF-8");
	...
	ASSERT(name == NULL || new_strcmp(name, gl_locale_name_default()) == 0);
	...
	if (setlocale(LC_ALL, "") != NULL) {
		name = gl_locale_name_posix(LC_CTYPE, "LC_CTYPE");
		ASSERT(name == NULL
		       || new_strcmp(name, gl_locale_name_default()) == 0);
		name = gl_locale_name_posix(LC_MESSAGES, "LC_MESSAGES");
		ASSERT(new_strcmp(name, "fr_FR.UTF-8") == 0);
	}
#if HAVE_NEWLOCALE
...
static void test_locale_name_environ(void)
{
	const char *name;
	/* Get into a defined state,  */
	setlocale(LC_ALL, "en_US.UTF-8");
	...
	/* Check that gl_locale_name_environ ignores the thread locale.  */
	{
		...
		if (locale != NULL) {
			...
			uselocale(locale);
			name =
			    gl_locale_name_environ(LC_MESSAGES, "LC_MESSAGES");
			ASSERT(new_strcmp(name, "C") == 0);
		}
	}
#endif
...
static void test_locale_name_default(void)
{
	...
	/* Check that gl_locale_name_default ignores the thread locale.  */
	{
		...
		if (locale != NULL) {
			uselocale(locale);
			ASSERT(new_strcmp(gl_locale_name_default(), name) == 0);
		}
	}
#endif
}
