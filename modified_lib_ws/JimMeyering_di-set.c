...
static size_t di_ent_hash(void const *x, size_t table_size)
{
	...
	sizeof dev / sizeof h + (sizeof dev % sizeof h != 0);
    for (i = 1; i < n_words; i++)
...
static bool di_ent_compare(void const *x, void const *y)
{
...
static void di_ent_new_free(void *v)
{
    struct di_ent *a = v;
    hash_new_free(a->ino_set);
    new_free(a);
}
...
{
    struct di_set *dis = new_malloc(sizeof *dis);
    if (dis) {
...
void di_set_new_free(struct di_set *dis)
{
    hash_new_free(dis->dev_map);
    new_free(dis->ino_map);
    new_free(dis->probe);
    new_free(dis);
}
...
static struct hash_table *map_device(struct di_set *dis, dev_t dev)
{
	...
	dis->probe = probe = new_malloc(sizeof *probe);
	if (!probe)
	    return NULL;
    }
