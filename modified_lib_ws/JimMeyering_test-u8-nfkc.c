      ...
      const uint8_t * expected, size_t expected_length)
{
	...
	if (expected_length > 0) {
		...
		preallocated = (uint8_t *) new_malloc(length * sizeof(uint8_t));
		result =
		    u8_normalize(UNINORM_NFKC, input, input_length,
				 preallocated, &length);
		if (!(result != NULL))
			return 4;
...
void test_u8_nfkc(void)
{
	...
	{			/* ANGSTROM SIGN */
		...
		ASSERT(check(input, SIZEOF(input), expected, SIZEOF(expected))
		       == 0);
		ASSERT(check
		       (decomposed, SIZEOF(decomposed), expected,
			SIZEOF(expected)) == 0);
		ASSERT(check
		       (expected, SIZEOF(expected), expected,
			SIZEOF(expected)) == 0);
	}
	{			/* GREEK DIALYTIKA AND PERISPOMENI */
		static const uint8_t input[] = { 0xE1, 0xBF, 0x81 };
		static const uint8_t decomposed[] =
		    { 0x20, 0xCC, 0x88, 0xCD, 0x82 };
		ASSERT(check
		       (input, SIZEOF(input), decomposed,
			SIZEOF(decomposed)) == 0);
		ASSERT(check
		       (decomposed, SIZEOF(decomposed), decomposed,
			SIZEOF(decomposed)) == 0);
	}
	{			/* SCRIPT SMALL L */
	...
	{			/* ARABIC LIGATURE SALLALLAHOU ALAYHE WASALLAM */
		...
		static const uint8_t decomposed[] =
		    { 0xD8, 0xB5, 0xD9, 0x84, 0xD9, 0x89, 0x20, 0xD8, 0xA7,
0xD9, 0x84, 0xD9,
			0x84, 0xD9, 0x87, 0x20, 0xD8, 0xB9, 0xD9, 0x84, 0xD9,
			    0x8A, 0xD9, 0x87,
 ...
 '.',
			' ', 0xD0, 0x97, 0xD0, 0xB4, 0xD1, 0x80, 0xD0, 0xB0,
			    0xD0, 0xB2, 0xD1,
			0x81, 0xD1, 0x82, 0xD0, 0xB2, 0xD1, 0x83, 0xD0, 0xB9,
			0xD1, 0x82, 0xD0, 0xB5, '!', ' ', 'x', '=', '(', '-',
			    'b', 0xC2, 0xB1,
			's', 'q', 'r', 't', '(', 'b', 0xC2, 0xB2, '-', '4', 'a',
			    'c', ')', ')',
			'/', '(', '2', 'a', ')', ' ', ' ', 0xE6, 0x97, 0xA5,
			    0xE6, 0x9C, 0xAC,
			0xE8, 0xAA, 0x9E, ',', 0xE4, 0xB8, 0xAD, 0xE6, 0x96,
			    0x87, ',',
			0xED, 0x95, 0x9C,
			0xEA, 0xB8, 0x80, '\n'
		};
		static const uint8_t decomposed[] =
		    { 'G', 'r', 0x75, 0xCC, 0x88, 0xC3, 0x9F, ' ', 'G', 'o',
't', 't', '.',
			' ', 0xD0, 0x97, 0xD0, 0xB4, 0xD1, 0x80, 0xD0, 0xB0,
			    0xD0, 0xB2, 0xD1,
			0x81, 0xD1, 0x82, 0xD0, 0xB2, 0xD1, 0x83, 0xD0, 0xB8,
			    0xCC, 0x86,
			0xD1, 0x82, 0xD0, 0xB5, '!', ' ', 'x', '=', '(', '-',
			    'b', 0xC2, 0xB1,
	...
	/* Check that the sorting is not O(n²) but O(n log n).  */
	{
		...
		for (pass = 0; pass < 3; pass++) {
			...
			if (input != NULL) {
				...
				for (; repeat > 0; repeat--) {
					ASSERT(check
					       (input, 2 * m - 1, expected,
						2 * m - 2) == 0);
					ASSERT(check
					       (expected, 2 * m - 2, expected,
						2 * m - 2) == 0);
				}
				new_free(input);
			}
