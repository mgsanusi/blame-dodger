...
static size_t ino_hash(void const *x, size_t table_size)
{
	...
	sizeof ino / sizeof h + (sizeof ino % sizeof h != 0);
    for (i = 1; i < n_words; i++)
...
static bool ino_compare(void const *x, void const *y)
{
...
{
    struct ino_map *im = new_malloc(sizeof *im);
    if (im) {
...
void ino_map_new_free(struct ino_map *map)
{
    hash_new_free(map->map);
    new_free(map->probe);
    new_free(map);
}
...
size_t ino_map_insert(struct ino_map *im, ino_t ino)
{
	...
	im->probe = probe = new_malloc(sizeof *probe);
	if (!probe)
