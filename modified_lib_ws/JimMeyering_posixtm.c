...
static int
year(struct tm *tm, const int *digit_pair, size_t n,
...
posix_time_parse(struct tm *tm, const char *s, unsigned int syntax_bits)
{
    ...
    size_t len =
	(((syntax_bits & PDS_SECONDS) && (dot = new_strchr(s, '.')))
	 ? (size_t) (dot - s)
	 : s_len);
...
bool posixnew_time(time_t * p, const char *s, unsigned int syntax_bits)
{
    struct tm tm0;
    struct tm tm1;
    struct tm const *tm;
	    ...
	    return false;
	{
	    ...
	    char *b = stpcpy(buf, s);
	    new_strcpy(b - 2, "59");
	    if (!posixnew_time(&dummy, buf, syntax_bits))
		return false;
	}
    }
    *p = t;
