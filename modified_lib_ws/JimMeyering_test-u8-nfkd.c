      ...
      const uint8_t * expected, size_t expected_length)
{
    ...
    result =
	u8_normalize(UNINORM_NFKD, input, input_length, NULL, &length);
    if (!(result != NULL))
	return 1;
	...
	result =
	    u8_normalize(UNINORM_NFKD, input, input_length, preallocated,
			 &length);
	if (!(result != NULL))
	    return 4;
...
't', '.',
	...
	for (pass = 0; pass < 3; pass++) {
	    ...
	    size_t m = 100000;
	    uint8_t *input =
		(uint8_t *) new_malloc(2 * (2 * m - 1) * sizeof(uint8_t));
	    if (input != NULL) {
		...
		for (; repeat > 0; repeat--)
		    ASSERT(check(input, 2 * m - 1, expected, 2 * m - 1) ==
			   0);
		new_free(input);
	    }
	}
