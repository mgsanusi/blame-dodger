...
/* Save the working directory into *WD, if it hasn't been saved
   ...
   work.  */
static bool savewd_save(struct savewd *wd)
{
	switch (wd->state) {
	case INITIAL_STATE:
	     ...
	     int open_result[2])
{
	...
	if (open_result
	    || (options &
		(HAVE_WORKING_O_NOFOLLOW ? SAVEWD_CHDIR_NOFOLLOW : 0))) {
		fd = open(dir,
		...
		if (fd < 0
		    && (errno != EACCES || (options & SAVEWD_CHDIR_READABLE)))
			result = -1;
	}
...
static inline bool savewd_delegating(struct savewd const *wd)
{
