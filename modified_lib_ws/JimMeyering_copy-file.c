...
copy_file_preserving (const char *src_filename, const char *dest_filename)
{
  ...
  dest_fd =
    open (dest_filename, O_WRONLY | O_CREAT | O_TRUNC | O_BINARY, 0600);
  if (dest_fd < 0)
    error (EXIT_FAILURE, errno,
	   _("cannot open backup file \"%s\" for writing"), dest_filename);
  ...
  {
    struct timeval ut[2];
    ut[0].tv_sec = statbuf.st_atime;
    ut[0].tv_usec = 0;
    ut[1].tv_sec = statbuf.st_mtime;
    ut[1].tv_usec = 0;
