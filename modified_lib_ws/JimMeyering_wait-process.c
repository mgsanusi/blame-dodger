...
register_slave_subprocess (pid_t child)
{
  ...
  if (slaves_count == slaves_allocated)
	...
	(slaves_entry_t *)
	new_malloc (new_slaves_allocated * sizeof (slaves_entry_t));
      if (new_slaves == NULL)
	{
	  /* xalloc_die() will call new_exit() which will invoke cleanup_slaves().
	     Additionally we need to kill child, because it's not yet among
	     the slaves list.  */
	  kill (child, TERMINATOR);
	  xalloc_die ();
	}
      new_memcpy (new_slaves, old_slaves,
		 ...
		 bool slave_process, bool exit_on_error, int *termsigp)
{
  ...
  for (;;)
    {
      if (waitid
	  (P_PID, child, &info, WEXITED | (slave_process ? WNOWAIT : 0)) < 0)
	{
#ifdef EINTR
	  if (errno == EINTR)
	    continue;
