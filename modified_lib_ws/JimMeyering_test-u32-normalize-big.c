...
static int cmp_ucs4_t(const void *a, const void *b)
{
			     ...
			     struct normalization_test_file *file)
{
	...
	for (;;) {
		...
		*ptr = '\0';
		...
		if (buf[0] == '@') {
			...
			if (part_index >= 0) {
				lines = (struct normalization_test_line *)
				    xnrealloc(lines, lines_length,
					      sizeof(struct
						     normalization_test_line));
				file->parts[part_index].lines = lines;
				file->parts[part_index].lines_length =
				    lines_length;
			}
			part_index++;
			lines = NULL;
		...
		for (sequence_index = 0; sequence_index < 5; sequence_index++) {
			...
			for (;;) {
				...
				sequence = (uint32_t *)
				    xnrealloc(sequence, sequence_length + 2,
					      sizeof(uint32_t));
		...
		if (lines_length == lines_allocated) {
			...
			if (lines_allocated < 7)
				lines_allocated = 7;
			lines = (struct normalization_test_line *)
			    xnrealloc(lines, lines_allocated,
	...
	}
	{
		...
		for (line_index = 0; line_index < p->lines_length; line_index++) {
			const uint32_t *sequence =
			    p->lines[line_index].sequences[0];
		...
		/* Sort this array.  */
		new_qsort(c1_array, p->lines_length, sizeof(ucs4_t),
			  cmp_ucs4_t);
	...
	if (ferror(stream) || new_fclose(stream)) {
			    ...
			    const uint32_t * c5, size_t c5_length))
{
	size_t part_index;
	for (part_index = 0; part_index < 4; part_index++) {
		const struct normalization_test_part *p =
		    &file->parts[part_index];
		size_t line_index;
		for (line_index = 0; line_index < p->lines_length; line_index++) {
			const struct normalization_test_line *l =
			    &p->lines[line_index];
			ASSERT_WITH_LINE(check
					 (l->sequences[0],
					  u32_strlen(l->sequences[0]),
					  l->sequences[1],
					  u32_strlen(l->sequences[1]),
					  l->sequences[2],
					  u32_strlen(l->sequences[2]),
					  l->sequences[3],
					  u32_strlen(l->sequences[3]),
					  l->sequences[4],
					  u32_strlen(l->sequences[4]))
					 == 0, file->filename, l->lineno);
		}
	}
}

void test_other(const struct normalization_test_file *file, uninorm_t nf)
{
