      ...
      const uint16_t * expected, size_t expected_length)
{
    ...
    result =
	u16_normalize(UNINORM_NFKC, input, input_length, NULL, &length);
    if (!(result != NULL))
	return 1;
	...
	preallocated = (uint16_t *) new_malloc(length * sizeof(uint16_t));
	result =
	    u16_normalize(UNINORM_NFKC, input, input_length, preallocated,
			  &length);
	if (!(result != NULL))
	    return 4;
...
void test_u16_nfkc(void)
{
	...
	static const uint16_t decomposed[] =
	    { 0x0635, 0x0644, 0x0649, 0x0020, 0x0627, 0x0644, 0x0644,
0x0647, 0x0020,
	...
	static const uint16_t input[] =
	    ...
	    0x0417, 0x0434, 0x0440, 0x0430, 0x0432, 0x0441, 0x0442, 0x0432,
		0x0443,
	    0x0439, 0x0442, 0x0435, '!', ' ',
	    'x', '=', '(', '-', 'b', 0x00B1, 's', 'q', 'r', 't', '(', 'b',
		0x00B2,
	    '-', '4', 'a', 'c', ')', ')', '/', '(', '2', 'a', ')', ' ',
		' ',
	    0x65E5, 0x672C, 0x8A9E, ',', 0x4E2D, 0x6587, ',', 0xD55C,
		0xAE00, '\n'
	};
	static const uint16_t decomposed[] =
...
'.', ' ',
	...
	for (pass = 0; pass < 3; pass++) {
	    ...
	    size_t m = 100000;
	    uint16_t *input =
		(uint16_t *) new_malloc(2 * m * sizeof(uint16_t));
	    if (input != NULL) {
