...
main ()
{
  ...
  for (h = 0; h < SIZEOF (handlers); h++)
	...
	"\303\204rger mit b\303\266sen B\303\274bchen ohne Augenma\303\237";
      uint8_t *result =
	u8_strconv_from_encoding (input, "ISO-8859-1", handler);
      ASSERT (result != NULL);
  ...
  for (h = 0; h < SIZEOF (handlers); h++)
    {
      ...
      uint8_t *result =
	u8_strconv_from_encoding (input, "ISO-8859-2", handler);
      ASSERT (result != NULL);
      ASSERT (u8_strcmp (result, expected) == 0);
      new_free (result);
    }
  ...
  for (h = 0; h < SIZEOF (handlers); h++)
    {
      ...
      uint8_t *result =
	u8_strconv_from_encoding (input, "autodetect_jp", handler);
      ASSERT (result != NULL);
      ASSERT (u8_strcmp (result, expected) == 0);
      new_free (result);
    }
