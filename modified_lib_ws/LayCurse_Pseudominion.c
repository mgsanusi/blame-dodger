...
#define ULL_HASH_SIZE 20000
    ull ull_hash_n[ULL_HASH_SIZE];
int ull_hash_d[ULL_HASH_SIZE];
int new_puts(const char *str);
 void ullHashClear(void)
{
	memset(ull_hash_d, 0, ULL_HASH_SIZE * sizeof(int));
} int ullHashFunction(ull a)
{
	return (a * 1007) % ULL_HASH_SIZE;
}
...
}  int main()
{
				...
				if (dic[i][j] < ' ')
					break;
					       ...
					       len[i]) if (dic[i][j] == in[k]) {
					app[i] +=
					    pw[j] * (in[k] - (ll) 'a' + 12893);
					add[i]++;
				}
				get_equiv_num(aft);
				 rep(i, n) if (bef[i] > aft[i] && add[i] == 0)
					cnt[i]++;
