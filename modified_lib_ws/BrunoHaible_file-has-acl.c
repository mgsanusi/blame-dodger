...
int acl_extended_nontrivial(acl_t acl)
{
...
int acl_access_nontrivial(acl_t acl)
{
	...
	for (got_one = acl_get_entry(acl, ACL_FIRST_ENTRY, &ace);
	     got_one > 0; got_one = acl_get_entry(acl, ACL_NEXT_ENTRY, &ace)) {
		acl_tag_t tag;
		if (acl_get_tag_type(ace, &tag) < 0)
			return -1;
		if (!
		    (tag == ACL_USER_OBJ || tag == ACL_GROUP_OBJ
		     || tag == ACL_OTHER))
			return 1;
	}
	return got_one;
...
/* Test an ACL retrieved with GETACL.
   ...
   Return 0 if it is trivial, i.e. equivalent to a simple stat() mode.  */
int acl_nontrivial(int count, aclent_t * entries)
{
	int i;
	for (i = 0; i < count; i++) {
		...
		if (!(ace->a_type == USER_OBJ
		      || ace->a_type == GROUP_OBJ || ace->a_type == OTHER_OBJ
...
/* Test an ACL retrieved with ACE_GETACL.
   Return 1 if the given ACL, consisting of COUNT entries, is non-trivial.
   Return 0 if it is trivial, i.e. equivalent to a simple stat() mode.  */
int acl_ace_nontrivial(int count, ace_t * entries)
{
	int i;
	...
	for (i = 0; i < count; i++)
		if (entries[i].
		    a_flags & (OLD_ACE_OWNER | OLD_ACE_GROUP | OLD_ACE_OTHER)) {
			old_convention = 1;
			break;
		}
	if (old_convention)
		...
		if ((NEW_ACE_WRITE_NAMED_ATTRS
		     ...
		     | NEW_ACE_WRITE_ACL | NEW_ACE_WRITE_OWNER)
		    & ~access_masks[1])
			return 1;
		...
		if ((NEW_ACE_READ_NAMED_ATTRS
		     ...
		     | NEW_ACE_READ_ACL | NEW_ACE_SYNCHRONIZE)
		    & ~access_masks[5])
			return 1;
...
/* Return 1 if the given ACL is non-trivial.
   Return 0 if it is trivial, i.e. equivalent to a simple stat() mode.  */
int acl_nontrivial(int count, struct acl_entry *entries, struct stat *sb)
{
	int i;
	for (i = 0; i < count; i++) {
...
/* Return 1 if the given ACL is non-trivial.
   Return 0 if it is trivial, i.e. equivalent to a simple stat() mode.  */
int aclv_nontrivial(int count, struct acl *entries)
{
	int i;
	for (i = 0; i < count; i++) {
		...
		if (!(ace->a_type == USER_OBJ	/* no need to check ace->a_id here */
		      ...
		      || ace->a_type == CLASS_OBJ || ace->a_type == OTHER_OBJ))
			return 1;
	}
	return 0;
}
...
/* Return 1 if the given ACL is non-trivial.
   Return 0 if it is trivial, i.e. equivalent to a simple stat() mode.  */
int acl_nontrivial(struct acl *a)
{
...
/* Return 1 if the given ACL is non-trivial.
   Return 0 if it is trivial, i.e. equivalent to a simple stat() mode.  */
int acl_nfs4_nontrivial(nfs4_acl_int_t * a)
{
...
/* Test an ACL retrieved with ACL_GET.
   Return 1 if the given ACL, consisting of COUNT entries, is non-trivial.
   Return 0 if it is trivial, i.e. equivalent to a simple stat() mode.  */
int acl_nontrivial(int count, struct acl *entries)
{
	int i;
	for (i = 0; i < count; i++) {
		struct acl *ace = &entries[i];
...
int file_has_acl(char const *name, struct stat const *sb)
{
	...
	if (!S_ISLNK(sb->st_mode)) {
		   ...
		   file systems (whereas the other ones are used in UFS file systems).  */
		{
			...
			for (;;) {
				...
				if (count > 6)
					return 1;
				entries =
				    (ace_t *) new_malloc(count * sizeof(ace_t));
				if (entries == NULL) {
					errno = ENOMEM;
					return -1;
				}
				if (acl(name, ACE_GETACL, count, entries) ==
				    count) {
		...
		for (;;) {
			...
			count =
			    acl((char *)name, ACL_CNT, NACLVENTRIES, entries);
			if (count < 0) {
		...
		for (;;) {
			...
			type.u64 = ACL_ANY;
			if (aclx_get(name, 0, &type, aclbuf, &aclsize, &mode) >=
			    0)
				break;
			if (errno == ENOSYS)
				return 0;
		...
		if (type.u64 == ACL_AIXC) {
			...
			int result =
			    acl_nfs4_nontrivial((nfs4_acl_int_t *) acl);
			if (acl != aclbuf)
				new_free(acl);
			return result;
		} else {
