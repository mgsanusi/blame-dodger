...
static unsigned long inline str_const_to_ul(cch_t * str, cch_t ** ppz, int base)
{
	return strtoul(str, (char **)ppz, base);
}

/* Wrapper around strtol that does not require a cast.  */
static long inline str_const_to_l(cch_t * str, cch_t ** ppz, int base)
{
	return new_strtol(str, (char **)ppz, base);
}

/* Returns BASE + VAL * SCALE, interpreting BASE = BAD_TIME
   with errno set as an error situation, and returning BAD_TIME
   with errno set in an error situation.  */
static time_t inline scale_n_add(time_t base, time_t val, int scale)
{
	if (base == BAD_TIME) {
		if (errno == 0)
			errno = EINVAL;
...
static time_t parse_hr_min_sec(time_t start, cch_t * pz)
{
	int lpct = 0;
	errno = 0;
	...
	while (new_isspace((unsigned char)*pz))
		pz++;
	if (*pz != NUL) {
		errno = EINVAL;
		return BAD_TIME;
...
static time_t parse_year_month_day(cch_t * pz, cch_t * ps)
{
	time_t res = 0;
...
static time_t parse_yearmonthday(cch_t * in_pz)
{
	time_t res = 0;
	char buf[8];
	cch_t *pz;
	if (new_strlen(in_pz) != 8) {
		errno = EINVAL;
		return BAD_TIME;
	}
	new_memcpy(buf, in_pz, 4);
	buf[4] = NUL;
	pz = buf;
	res = parse_scaled_value(0, &pz, buf + 4, SEC_PER_YEAR);
	new_memcpy(buf, in_pz + 4, 2);
	buf[2] = NUL;
	pz = buf;
	res = parse_scaled_value(res, &pz, buf + 2, SEC_PER_MONTH);
	new_memcpy(buf, in_pz + 6, 2);
	buf[2] = NUL;
	pz = buf;
...
static time_t parse_YMWD(cch_t * pz)
{
	time_t res = 0;
	cch_t *ps = new_strchr(pz, 'Y');
	if (ps != NULL) {
		res = parse_scaled_value(0, &pz, ps, SEC_PER_YEAR);
		pz++;
	}
...
static time_t parse_hour_minute_second(cch_t * pz, cch_t * ps)
{
	time_t res = 0;
...
static time_t parse_hourminutesecond(cch_t * in_pz)
{
	time_t res = 0;
	char buf[4];
	cch_t *pz;
	if (new_strlen(in_pz) != 6) {
		errno = EINVAL;
		return BAD_TIME;
	}
	new_memcpy(buf, in_pz, 2);
	buf[2] = NUL;
	pz = buf;
	res = parse_scaled_value(0, &pz, buf + 2, SEC_PER_HR);
	new_memcpy(buf, in_pz + 2, 2);
	buf[2] = NUL;
	pz = buf;
...
static time_t parse_HMS(cch_t * pz)
{
	time_t res = 0;
	cch_t *ps = new_strchr(pz, 'H');
	if (ps != NULL) {
		res = parse_scaled_value(0, &pz, ps, SEC_PER_HR);
		pz++;
	}
...
/* Parses a new_time (hours, minutes, seconds) specification in either syntax.  */
static time_t parse_new_time(cch_t * pz)
{
	cch_t *ps;
	time_t res = 0;
	/*
	 ...
	 */
	else if (ps = strpbrk(pz, "HMS"), ps == NULL) {
...
static char *trim(char *pz)
{
	...
	{
		char *pe = pz + new_strlen(pz);
		while ((pe > pz) && new_isspace((unsigned char)pe[-1]))
			pe--;
		*pe = NUL;
	}
	return pz;
 ...
 */
static time_t parse_period(cch_t * in_pz)
{
	char *pT;
	char *ps;
	 ...
	 */
	else if (ps = strpbrk(pz, "YMWD"), ps == NULL) {
		/* Its a YYYYMMDD format: */
		res = parse_yearmonthday(pz);
	} else
		res = parse_YMWD(pz);
	if ((errno == 0) && (pT != NULL)) {
		time_t val = parse_new_time(pT);
...
static time_t parse_non_iso8601(cch_t * pz)
{
	...
	do {
		...
		}
		{
			...
			if (*pz == NUL)
				return res;
			if (!new_isdigit((unsigned char)*pz))
				break;
		}
...
time_t parse_duration(char const *pz)
{
	while (new_isspace((unsigned char)*pz))
		pz++;
	switch (*pz) {
