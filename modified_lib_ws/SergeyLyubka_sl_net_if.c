...
void mg_set_non_blocking_mode(sock_t sock)
{
    ...
    sl_SetSockOpt(sock, SL_SOL_SOCKET, SL_SO_NONBLOCKING, &opt,
		  sizeof(opt));
}
...
void mg_sl_if_tcp_send(struct mg_connection *nc, const void *buf,
		       size_t len)
{
    mbuf_append(&nc->send_mbuf, buf, len);
}

void mg_sl_if_udp_send(struct mg_connection *nc, const void *buf,
		       size_t len)
{
    mbuf_append(&nc->send_mbuf, buf, len);
}
...
time_t mg_sl_if_poll(struct mg_iface *iface, int timeout_ms)
{
    ...
    if (num_fds > 0) {
	num_ev =
	    sl_Select((int) max_fd + 1, &read_set, &write_set, &err_set,
		      &tv);
    }
    now = mg_time();
	...
	if (nc->sock != INVALID_SOCKET) {
		...
		fd_flags =
		    ...
		    (SL_FD_ISSET(nc->sock, &write_set) ? _MG_F_FD_CAN_WRITE
		     : 0) | (SL_FD_ISSET(nc->sock,
					 &err_set) ? _MG_F_FD_ERROR : 0);
	    }
...
void sl_restart_cb(struct mg_mgr *mgr)
{
	...
	if (nc->flags & MG_F_LISTENING) {
	    DBG(("restarting %p %s:%d", nc, inet_ntoa(nc->sa.sin.sin_addr),
		 ntohs(nc->sa.sin.sin_port)));
	    int res =
		(nc->flags & MG_F_UDP ? mg_sl_if_listen_udp(nc, &nc->sa)
