...
/* This is the signal handler for SIGPIPE.  It is invoked synchronously,
   therefore it can make library calls to new_malloc(), gettext(), new_exit() etc. -
   although in general it is undefined behaviour to do such calls from
   within signal handlers.  */
static void sigpipe_die_handler(int sig)
{
...
void install_sigpipe_die_handler(void (*prepare_die) (void))
{
