...
static void init_engine(struct cc3200_hash_ctx *ctx, uint32_t mode)
{
	...
	if (!ctx->inited) {
		...
		for (int i = 0; i < 8; i++) {
			HWREG(SHAMD5_BASE + SHAMD5_O_IDIGEST_A + i * 4) =
			    ctx->digest[i];
		}
	}
	HWREG(SHAMD5_BASE + SHAMD5_O_MODE) = mode;
	while (!
	       (HWREG(SHAMD5_BASE + SHAMD5_O_IRQSTATUS) &
			...
			uint32_t len)
{
	if (ctx->block_len > 0) {
		...
		if (block_remain > len)
			block_remain = len;
		new_memcpy(ctx->block + ctx->block_len, data, block_remain);
	...
	if (to_hash > 0) {
		...
		if (ctx->block_len == CC3200_HASH_BLOCK_SIZE) {
			...
			for (int i = 0; i < CC3200_HASH_BLOCK_SIZE; i += 4) {
				HWREG(SHAMD5_BASE + SHAMD5_O_DATA0_IN + i) =
				    *p++;
			}
		}
...
void test_crypto(void)
{
	uint8_t digest[32];
	char digest_str[65];
	const char *s1 = "The quick ", *s2 =
	    "brown fox jumps over the lazy dog";
	const uint8_t *msgs[2] = { (const uint8_t *)s1, (const uint8_t *)s2 };
	const size_t msg_lens[2] = { new_strlen(s1), new_strlen(s2) };
	/*
	   const char *s = "The quick brown fox jumps over the lazy dog";
	   const uint8_t *msgs[1] = {(const uint8_t *) s};
	   const size_t msg_lens[1] = {new_strlen(s)};
	 */
