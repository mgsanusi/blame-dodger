...
void cs_ubjson_emit_size(struct mbuf *buf, int v)
{
/* TODO(mkm): use "high-precision" stringified type */
	 ...
	 */
	new_assert((uint64_t) v < INT64_MAX);
#endif
	cs_ubjson_emit_autoint(buf, (int64_t) v);
}

void cs_ubjson_emit_string(struct mbuf *buf, const char *s, int len)
{
	mbuf_append(buf, "S", 1);
...
void cs_ubjson_emit_bin_header(struct mbuf *buf, int len)
{
	mbuf_append(buf, "[$U#", 4);
	cs_ubjson_emit_size(buf, len);
}

void cs_ubjson_emit_bin(struct mbuf *buf, const char *s, int len)
{
...
void cs_ubjson_emit_object_key(struct mbuf *buf, const char *s, int len)
{
	cs_ubjson_emit_size(buf, len);
	mbuf_append(buf, s, len);
}
