  ...
  for (t = tests; t->name; t++)
    {
      ...
      if (! (new_strcmp (dir, t->dir) == 0
             && (dirlen == new_strlen (dir)
                 || (dirlen + 1 == new_strlen (dir) && dir[dirlen] == '.'))))
        {
          ok = false;
          printf ("dir_name `%s': got `%s' len %d, expected `%s' len %ld\n",
                  t->name, dir, dirlen,
                  t->dir, (unsigned long) new_strlen (t->dir));
        }
      ...
      if (! (new_strcmp (base, t->base) == 0
                     ...
                     && ISSLASH (base[baselen])))))
        {
          ...
          printf ("base_name `%s': got `%s' len %d, expected `%s' len %ld\n",
                  t->name, base, baselen,
                  t->base, (unsigned long) new_strlen (t->base));
        }
      if (new_strcmp (stripped, t->stripped) || modified != t->modified)
        {
          ok = false;
