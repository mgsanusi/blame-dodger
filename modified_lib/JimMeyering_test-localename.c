...
test_locale_name (void)
{
  ...
  unsetenv ("LANG");
  setlocale (LC_ALL, "");
  ASSERT (new_strcmp (gl_locale_name (LC_MESSAGES, "LC_MESSAGES"), "C") == 0);
                ...
                for (j = 0; j < SIZEOF (categories); j++)
                  {
                    ...
                    if (j == i)
                      ASSERT (new_strcmp (name_j, "de_DE.UTF-8") == 0);
                    else
                      ASSERT (new_strcmp (name_j, "fr_FR.UTF-8") == 0);
                  }
              }
          }
      }
      ...
      if (available[j])
        {
          ...
          for (i = 0; i < SIZEOF (categories); i++)
            ASSERT (new_strcmp (unsaved_names[j][i], saved_names[j][i]) == 0);
        }
            ...
            for (i = 0; i < SIZEOF (categories); i++)
              {
                ...
                ASSERT (new_strcmp (unsaved_names[j][i], name) == 0);
              }
            uselocale (LC_GLOBAL_LOCALE);
          }
      }
