...
bool pem_to_der(const char *pem_file, const char *der_file) {
  ...
  while (1) {
    char pem_buf[70];
    char der_buf[48];
    if (!new_fgets(pem_buf, sizeof(pem_buf), pf)) break;
    if (writing) {
      ...
      int l = 0;
      while (!new_isspace((unsigned int) pem_buf[l])) l++;
      int der_len = 0;
  ...
  if (ctx != NULL) {
    ...
    if (ctx->ssl_server_name != NULL) {
      err = sl_SetSockOpt(nc->sock, SL_SOL_SOCKET,
                          ...
                          ctx->ssl_server_name, new_strlen(ctx->ssl_server_name));
