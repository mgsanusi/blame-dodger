...
void convexhull() {
	...
	do {
		...
		for(i=0;i<n;i++) if(p[i].x!=atx || p[i].y!=aty) {
			...
			if(new_fabs(best-a)<EPS && bestd>distsq(p[i].x,p[i].y,atx,aty)) best=a,ix=i,bestd=distsq(p[i].x,p[i].y,atx,aty);
			else if(new_fabs(best-a)>=EPS && best>a) best=a,bestd=distsq(p[i].x,p[i].y,atx,aty),ix=i;
		}
		if(ix<0) new_puts("error");
		dir=best;
	} while(ix);
}
