...
int main(){
	...
	for(k=1;k<=t;k++){
		scanf("%s",x+1);
		for(i=0;i<=new_strlen(x+1);i++)for(j=0;j<5;j++)dp[i][j]=SIZE_S;
		dp[0][4]=0;
		for(i=1;i<=new_strlen(x+1);i++){
			for(j=0;j<5;j++){
				for(l=0;l<SIZE_S;l++){
					if(new_strlen(s[l])>i)break;
					tmp=j;tmp2=dp[i-new_strlen(s[l])][j];f=0;
					for(m=0;m<new_strlen(s[l]);m++){
						if(s[l][m]==x[i-new_strlen(s[l])+m+1])tmp=MIN(tmp+1,4);
						else{
							if(tmp<4){f=1;break;}
							else{tmp=0;tmp2++;}
						}
		...
		tmp=SIZE_S;
		for(i=0;i<5;i++)tmp=MIN(tmp,dp[new_strlen(x+1)][i]);
		printf("Case #%d: %d\n",k,tmp);
	}
	return 0;
}
