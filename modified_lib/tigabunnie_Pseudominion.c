...
int main(int argc, char **argv) {
	...
	for (i=1; i<=test_cases; i++) {
		...
		for (j=0; j<N; j++) {
			games_played = 0;
			wins = 0;
			for (k=0; k<N; k++) if (match_table[j][k]!='.') {
				games_played++;
				if (match_table[j][k]=='1') wins++;
			}
		...
		for (j=0; j<N; j++) {
			count = 0;
			owp[j] = 0;
			for (k=0; k<N; k++) if (match_table[j][k]!='.') {	//team j played against team k, find team k's avg win percentage excluding game with team j
				wins = 0;
				games_played = 0;
				for (n=0; n<N; n++) if (n!=j && match_table[k][n]!='.') {
					games_played++;
					if (match_table[k][n]=='1') wins++;
				}
