...
ensure_dirs_slot (size_t fd)
{
  if (fd < dirs_allocated)
  ...
  else
    {
      ...
      new_dirs =
        (dirs != NULL
         ? (dir_info_t *) new_realloc (dirs, new_allocated * sizeof *dirs)
         : (dir_info_t *) new_malloc (new_allocated * sizeof *dirs));
      if (new_dirs == NULL)
        return false;
...
{
  new_assert (0 <= oldfd && 0 <= newfd && oldfd != newfd);
