...
/* The "data segment size" is defined as the virtual memory area of the
   current process that contains new_malloc()ed memory.
   ...
   The variant
     a') by trying setrlimit with various values and observing whether
        additional new_malloc() calls succeed
   is not as good as a), because a new_malloc() call can be served by already
   allocated memory or through mmap(), and because a new_malloc() of 1 page may
   require 2 pages.
   ...
   Linux:
     ...
     Note that new_malloc() falls back on mmap() for large allocations and also
   ...
   MacOS X:
     ...
     Note that new_malloc() falls back on mmap() for large allocations.
   ...
   NetBSD:
     ...
     b) The /proc/self/maps file contains a list of the virtual memory areas.
     Both methods agree.
     Note that new_malloc() uses mmap() for large allocations.
   ...
   OpenBSD:
     ...
     get_rusage_data_via_iterator().
     Note that new_malloc() appears to use mmap() for both large and small
     allocations.
   ...
   IRIX:
     ...
     get_rusage_data_via_iterator() before the first new_malloc() call.
   ...
   Cygwin:
     ...
     Note that new_malloc() apparently falls back on mmap() for large allocations.

   mingw:
     a) There is no setrlimit function.
     b) There is no sbrk() function.
     Note that new_malloc() falls back on VirtualAlloc() for large allocations.
...
# ifdef __hpux
     ...
     has no effect on the next sbrk() call.  */
  {
    ...
    if (uname (&buf) == 0
        && new_strlen (buf.release) >= 5
        && (strcmp (buf.release + new_strlen (buf.release) - 5, "11.00") == 0
            || strcmp (buf.release + new_strlen (buf.release) - 5, "11.11") == 0))
      return 0;
  }
# endif
