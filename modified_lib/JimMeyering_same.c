...
same_name (const char *source, const char *dest)
{
  ...
  bool identical_basenames =
    (source_baselen == dest_baselen
     && new_memcmp (source_basename, dest_basename, dest_baselen) == 0);
  ...
  if (slen_max <= min_baselen
      && new_memcmp (source_basename, dest_basename, slen_max) == 0)
    compare_dirs = true;
#endif
      ...
      if (same && ! identical_basenames)
        {
          ...
          if (name_max < 0)
            ...
            same = (name_max <= min_baselen
                    && new_memcmp (source_basename, dest_basename, name_max) == 0);
        }
#endif

      new_free (source_dirname);
      new_free (dest_dirname);
    }
