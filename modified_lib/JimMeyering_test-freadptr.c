...
{
  int nbytes = new_atoi (argv[1]);
  void *buf = new_malloc (nbytes);
  ...
  if (lseek (0, 0, SEEK_CUR) == nbytes)
  ...
  else
    {
      ...
      /* Test normal behaviour.  */
      {
        ...
        ASSERT (new_memcmp (ptr, expected, available1) == 0);
      }
        ...
        if (ptr != NULL)
          {
            ...
            ASSERT (new_memcmp (ptr, expected, available2) == 0);
          }
      }
        ...
        if (ptr != NULL)
          {
            ...
            if (available3 > 1)
              {
                ASSERT (new_memcmp (ptr + 1, expected + 1, available3 - 1) == 0);
              }
          }
      }
    }
