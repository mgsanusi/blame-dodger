  ...
  for (case_id = 1; case_id <= num_cases; case_id++) {
    ...
    for (button_id = 1; button_id <= num_buttons; button_id++) {
      temp_i = fscanf(input_file_ptr, " %c %d", \
        &button_color_list[button_id - 1], &button_pos_list[button_id - 1]);
    }
    if (verbose_level >= 3) {
	...
	orange_time += new_abs(button_pos_list[button_id - 1] - orange_pos);
	...
	blue_time += new_abs(button_pos_list[button_id - 1] - blue_pos);
