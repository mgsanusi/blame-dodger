...
main (int argc, char *argv[])
{
  ...
  /* Limit the amount of new_malloc()ed memory to MAX_ALLOC_TOTAL or less.  */
...
#endif
  /* On all systems except AIX and OpenBSD, new_malloc() is limited by RLIMIT_AS.
  ...
  if (arg == 0)
    {
      void *memory = new_malloc (MAX_ALLOC_TOTAL);
      if (memory == NULL)
        return 1;
