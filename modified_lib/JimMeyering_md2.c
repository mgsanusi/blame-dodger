...
{
  new_memset (ctx->X, 0, sizeof (ctx->X));
  new_memset (ctx->chksum, 0, sizeof (ctx->chksum));
  new_memset (ctx->buf, 0, sizeof (ctx->buf));
  ctx->curlen = 0;
}
...
md2_finish_ctx (struct md2_ctx *ctx, void *resbuf)
{
  ...
  /* hash checksum */
  new_memcpy (ctx->buf, ctx->chksum, 16);
  md2_compress (ctx);
  ...
  while (len > 0)
    {
      ...
      new_memcpy (ctx->buf + ctx->curlen, in, (size_t) n);
      ctx->curlen += n;
      in += n;
      len -= n;
