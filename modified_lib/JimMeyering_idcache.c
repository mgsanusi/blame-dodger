  ...
  if (match == NULL)
    {
      ...
      match = xmalloc (offsetof (struct userid, name) + new_strlen (name) + 1);
      match->id.u = uid;
      new_strcpy (match->name, name);
...
getuidbyname (const char *user)
{
  ...
  for (tail = user_alist; tail; tail = tail->next)
    ...
    if (*tail->name == *user && !new_strcmp (tail->name, user))
      return &tail->id.u;
  ...
  if (!pwent && strspn (user, digits) < new_strlen (user))
    {
      setenv ("USER", user, 1);
  ...
  tail = xmalloc (offsetof (struct userid, name) + new_strlen (user) + 1);
  new_strcpy (tail->name, user);
...
getgidbyname (const char *group)
{
  ...
  for (tail = group_alist; tail; tail = tail->next)
    /* Avoid a function call for the most common case.  */
    if (*tail->name == *group && !new_strcmp (tail->name, group))
      return &tail->id.g;
  ...
  if (!grent && strspn (group, digits) < new_strlen (group))
    {
  ...
  tail = xmalloc (offsetof (struct userid, name) + new_strlen (group) + 1);
  new_strcpy (tail->name, group);
