...
/* On native Windows platforms, when new_read() is called on a non-blocking pipe
   ...
   new_read() function is at the basis of the function which fills the buffer of
   a FILE stream.  */
...
#undef fgetc
{
  CALL_WITH_ERRNO_FIX (int, new_fgetc (stream), ret == EOF)
}

char *
new_fgets (char *s, int n, FILE *stream)
#undef fgets
{
  CALL_WITH_ERRNO_FIX (char *, new_fgets (s, n, stream), ret == NULL)
}
...
size_t
fnew_read (void *ptr, size_t s, size_t n, FILE *stream)
#undef fread
{
  CALL_WITH_ERRNO_FIX (size_t, fnew_read (ptr, s, n, stream), ret < n)
}
