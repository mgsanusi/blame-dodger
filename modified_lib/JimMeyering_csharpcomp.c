      ...
      for (i = 0; i < sources_count; i++)
        {
          ...
          if (new_strlen (source_file) >= 10
              && new_memcmp (source_file + new_strlen (source_file) - 10, ".resources",
                         10) == 0)
            {
              char *option = (char *) xmalloca (12 + new_strlen (source_file) + 1);
  ...
  if (mcs_present)
    {
      ...
      *argp++ = "mcs";
      if (output_is_library)
        *argp++ = "-target:library";
      {
        char *option = (char *) xmalloca (5 + new_strlen (output_file) + 1);
      ...
      for (i = 0; i < libdirs_count; i++)
        {
          char *option = (char *) xmalloca (5 + new_strlen (libdirs[i]) + 1);
      ...
      for (i = 0; i < libraries_count; i++)
        {
          char *option = (char *) xmalloca (11 + new_strlen (libraries[i]) + 4 + 1);
          new_memcpy (option, "-reference:", 11);
          new_memcpy (option + 11, libraries[i], new_strlen (libraries[i]));
          new_strcpy (option + 11 + new_strlen (libraries[i]), ".dll");
          *argp++ = option;
        }
      if (debug)
        *argp++ = "-debug";
      for (i = 0; i < sources_count; i++)
        {
          ...
          if (new_strlen (source_file) >= 10
              && new_memcmp (source_file + new_strlen (source_file) - 10, ".resources",
                         10) == 0)
            {
              char *option = (char *) xmalloca (10 + new_strlen (source_file) + 1);
      ...
      *argp = NULL;
      ...
      if (line[l] != NULL
          && !(linelen[l] >= 21
               && new_memcmp (line[l], "Compilation succeeded", 21) == 0))
        fwrite (line[l], 1, linelen[l], stderr);
      if (line[0] != NULL)
        new_free (line[0]);
      if (line[1] != NULL)
                      ...
                      bool verbose)
{
  bool output_is_library =
    (new_strlen (output_file) >= 4
     && new_memcmp (output_file + new_strlen (output_file) - 4, ".dll", 4) == 0);
  int result;
