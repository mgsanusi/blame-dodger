...
main ()
{
  ...
  /* Test conversion from ISO-8859-1 to UTF-8 with no errors.  */
  {
    ...
    size_t inbytesleft = new_strlen (input);
    char *outptr = buf;
    ...
    ASSERT (outptr == buf + new_strlen (expected));
    ASSERT (new_memcmp (buf, expected, new_strlen (expected)) == 0);
  }
  ...
  /* Test conversion from UTF-8 to ISO-8859-1 with EILSEQ.  */
  {
    ...
    if (res == (size_t)(-1))
      {
        ASSERT (errno == EILSEQ);
        ASSERT (inbytesleft == new_strlen (input) && outptr == buf);
      }
    else
      {
        ASSERT (res == 1);
