                      ...
                      /*krb5_int16*/int port, krb5_address **outaddr)
{
    ...
    (void) new_memcpy(marshal, &temptype, sizeof(temptype));
    marshal += sizeof(temptype);

    templength = htonl(sizeof(smushaddr));
    (void) new_memcpy(marshal, &templength, sizeof(templength));
    marshal += sizeof(templength);

    (void) new_memcpy(marshal, &smushaddr, sizeof(smushaddr));
    ...
    (void) new_memcpy(marshal, &smushport, sizeof(smushport));
