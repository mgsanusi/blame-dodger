...
void dfs(int i, int j){
    ...
    for (d = 0; d < 8; d++){
        k = i + dx[d], l = j + dy[d];
        if (valid(k, l) && str[k][l] != '*' && !visited[k][l]) dfs(k, l);
    }
}

int main(){
    int i, j, k, l, a, b, c, d, x, bitmask;
    for (bitmask = 0; bitmask < (1 << 25); bitmask++) Popcount[bitmask] = __builtin_popcount(bitmask);
