            ...
            for (p = path; *p; p = p_next)
              {
                ...
                concat_name = (char *) xmalloc (p_len + new_strlen (argv0) + 2);
# ifdef NO_XMALLOC
                if (concat_name == NULL)
                  return NULL;
# endif
                                 ...
                                 const char *orig_installdir)
{
     ...
     Remove the ".bin" here.  */
  {
    ...
    if (argv0_len > 4 + exeext_len)
      if (new_memcmp (argv0 + argv0_len - exeext_len - 4, ".bin", 4) == 0)
        {
...
#endif
              {
                new_memcpy (shorter, argv0, argv0_len - exeext_len - 4);
                if (sizeof (EXEEXT) > sizeof (""))
                  new_memcpy (shorter + argv0_len - exeext_len - 4,
