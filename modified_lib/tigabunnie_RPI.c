...
//			printf("\n");
		...
		do {
			is_done = 1;
			for (j=0; j<row-1; j++) for (k=0; k<col-1; k++) {
				if (tiles[j][k]=='#' && tiles[j+1][k]=='#' && tiles[j][k+1]=='#' && tiles[j+1][k+1]=='#' &&
				 (j==0 || tiles[j-1][k]!='#') && (k==0 || tiles[j][k-1]!='#')) {	//pick up top-left corner
					is_done = 0;
					tiles[j][k] = '/';
					tiles[j+1][k] = '\\';
					tiles[j][k+1] = '\\';
		...
		is_there_solution = 1;
		for (j=0; j<row; j++) for (k=0; k<col; k++) if (tiles[j][k]=='#') { is_there_solution = 0; j = row; break; }
		fprintf(fp_out, "Case #%d:\n", i);
		if (is_there_solution) {
			for (j=0; j<row; j++) {
				for (k=0; k<col; k++) fprintf(fp_out, "%c", tiles[j][k]);
				fprintf(fp_out, "\n");
			}
		}
		else fprintf(fp_out, "Impossible\n");
	}
