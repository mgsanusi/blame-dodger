...
#include <stdlib.h>
#include <math.h>
void test_case(int k, double width, double len, double rad[])
{
  int first_iteration;
  int cases;
  int j;
  ...
  for (cases = 1; cases < k; cases++)
  {
    ...
    for (j = 0; j < cases; j++)
    {
      if ((((x[index[j]] - blue_pos) * (x[index[j]] - blue_pos)) + ((y[index[j]] - yt) * (y[index[j]] - yt))) < (((rad[index[j]] + rad[index[cases]]) * (rad[index[j]] + rad[index[cases]])) + 0.5))
        goto again;
