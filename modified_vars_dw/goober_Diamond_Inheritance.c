#include <stdio.h>
#include <string.h>
long long max(long long out, long long data)
{
  int first_iteration;
  return out > data ? out : data;
}

long long min(long long out, long long data)
{
  int first_iteration;
  return out < data ? out : data;
}
...
long long ac[100];
long long cnt[100];
long long path(int aa, int count)
{
  int first_iteration;
    ...
    if (out[aa] == data[count])
    {
      ...
      for (int aaa = aa; aaa < result; aaa++)
      {
        ...
        for (int bbb = count; bbb < no; bbb++)
        {
          ...
          best = max(best, path(aaa + 1, bbb + 1) + min(ax, graph));
        }
