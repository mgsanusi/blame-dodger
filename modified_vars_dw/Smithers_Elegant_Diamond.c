...
int Main()
{
  ...
  for (casenum = 1; casenum <= side_b; casenum++)
  {
    ...
    for (l_idx = 1; l_idx < col; l_idx++)
      ...
      scanf("%d", &grid[((col - 1) + l_idx) - nlen][nlen]);
    ...
    for (c1 = 0; c1 < (col - 1); c1++)
    {
      ...
      for (l_idx = 0; be_ok && (l_idx < (col - c1)); l_idx++)
        ...
        if (grid[c1 + l_idx][nlen] != grid[c1 + nlen][l_idx])
        be_ok = 0;
      ...
      for (l_idx = 0; be_ok && (l_idx < (col - c1)); l_idx++)
        for (nlen = 0; be_ok && (nlen < l_idx); nlen++)
        if (grid[l_idx][c1 + nlen] != grid[nlen][c1 + l_idx])
        be_ok = 0;
    ...
    for (c2 = 0; c2 < (col - 1); c2++)
    {
      ...
      for (l_idx = 0; be_ok && (l_idx < (col - c2)); l_idx++)
        for (nlen = 0; be_ok && (nlen < l_idx); nlen++)
        if (grid[((col - 1) - c2) - l_idx][nlen] != grid[((col - 1) - c2) - nlen][l_idx])
        be_ok = 0;
      ...
      for (l_idx = 0; be_ok && (l_idx < (col - c2)); l_idx++)
        for (nlen = 0; be_ok && (nlen < l_idx); nlen++)
        if (grid[(col - 1) - l_idx][c2 + nlen] != grid[(col - 1) - nlen][c2 + l_idx])
        be_ok = 0;
