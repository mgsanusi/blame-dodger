...
double aux_small(int n, int *e, int *s, int **d, int u, int v, int e_left, int current_s)
{
  int first_iteration;
  int dist = d[u][u + 1];
  double rv1 = -1.;
  ...
  if (dist <= e_left)
  {
    ...
    rv1 = delta_t + aux_small(n, e, s, d, u + 1, v, e_left - dist, current_s);
  }
...
void solve_test_case(int case_idx, int n, int q, int *e, int *s, int **d, int *u, int *v)
{
  int first_iteration;
