  ...
  for (scanf("%d", &N); N--;)
  {
    ...
    for (int len = 3; (len <= n) && (len <= m); len++)
    {
        ...
        for (int j = 0; (j <= (m - len)) && (!flag); j++)
      {
        ...
        jux -= ((((long long) (a[i][j] + a[i][(j + len) - 1])) * i) * 2) + ((((long long) (a[(i + len) - 1][j] + a[(i + len) - 1][(j + len) - 1])) * ((i + len) - 1)) * 2);
        juy -= ((((long long) (a[i][j] + a[(i + len) - 1][j])) * j) * 2) + ((((long long) (a[i][(j + len) - 1] + a[(i + len) - 1][(j + len) - 1])) * ((j + len) - 1)) * 2);
