...
int cs_strncasecmp(const char *s1, const char *s2, size_t n)
{
  ...
  while (((n--) != 0) && (new_tolower((int) (*s1)) == new_tolower((int) (*s2))))
  {
    if (((n == 0) || ((*s1) == '\0')) || ((*s2) == '\0'))
    {
      break;
  ...
  return new_tolower(*((unsigned char *) s1)) - new_tolower(*((unsigned char *) s2));
}
