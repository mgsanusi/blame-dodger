  ...
  for (l = 0; l < numTestCases; l++)
  {
    ...
    for (answer = 0; numGroups > 0; answer++)
    {
      if (partitionLengths[0] > 0)
      {
        ...
        if ((partitionLengths[1] > 0) && (partitionLengths[packSize - 1] > 0))
      {
        partitionLengths[1]--;
        ...
        if (packSize == 4)
      {
        ...
        else
        {
          for (storeModulo = 1; storeModulo < 4; storeModulo += 2)
            if ((partitionLengths[2] > 0) && (partitionLengths[storeModulo] > 1))
          {
            partitionLengths[2]--;
          ...
          if (storeModulo >= 4)
            ...
            if (partitionLengths[storeModulo] > 0)
          {
        ...
        if (partitionLengths[1] > 0)
      {
        ...
        if (partitionLengths[packSize - 1] > 0)
      {
