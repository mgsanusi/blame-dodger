...
double computeang(double x, double y, double x0, double y0)
{
  int first_iteration;
  double s = (x * y0) - (y * x0);
  double ang = ((x * x0) + (y * y0)) / (new_sqrt((x * x) + (y * y)) * new_sqrt((x0 * x0) + (y0 * y0)));
  if (s < 0)
    ang = (-1) - ang;
  else
    ang += 1;
  ...
  for (d = 1; d <= e; d++)
  {
    ...
    for (a = 0; a < n; a++)
    {
      ...
      for (b = 0; b < n; b++)
      {
        ...
        p[c].coor = p_[b];
        p[c].ang = computeang(p_[b][0] - pa[0], p_[b][1] - pa[1], 0, 1);
        c++;
      }
      ...
      for (b = 0; b < c; b++)
      {
        ...
        while (next < (b + c))
        {
          if ((((p[next].coor[0] - pa[0]) * (p[b].coor[1] - pa[1])) - ((p[next].coor[1] - pa[1]) * (p[b].coor[0] - pa[0]))) < 0)
            break;
        ...
        new_assert((((p[next - 1].coor[0] - pa[0]) * (p[b].coor[1] - pa[1])) - ((p[next - 1].coor[1] - pa[1]) * (p[b].coor[0] - pa[0]))) >= 0);
