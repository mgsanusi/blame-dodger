...
int calc(int n, int t)
{
	...
	if ((n % 2) == 1) {
	    if (((dataNeighborsShtml > 0)
		 && (d[dataNeighborsShtml - 1] == 1))
		|| (d[dataNeighborsShtml + 1] == 1)) {
		conf[n][t] = 2;
		return 2;
	    }
...
{
    int dataNeighborsShtml;
    for (dataNeighborsShtml = 0; dataNeighborsShtml < max[num];
	 dataNeighborsShtml++) {
	if (r == 0) {
	    s[r][deskid] = hasdesk;
	} else
	    if ((calc(deskid, dataNeighborsShtml) == 1)
		&& (s[r][deskid] <
		    (s[r - 1][dataNeighborsShtml] + hasdesk))) {
...
int storeHorse()
{
    ...
    int answer = 0;
    scanf("%d%d", &r, &num);
    for (dataNeighborsShtml = 0; dataNeighborsShtml < r;
	 dataNeighborsShtml++)
...
int Main()
{
    ...
    for (dataNeighborsShtml = 1; dataNeighborsShtml <= count;
	 dataNeighborsShtml++) {
