	...
	for (ans = 1; ans <= data2; ans++) {
		...
		while (dc) {
			...
			if ((tx == xpStat) && (nc == m))
				break;
			if ((f[tx][nc] >
			     f[tx - 1][nc] ? f[tx][nc] : f[tx - 1][nc]) <=
			    ((c[tx][nc] <
			      c[tx - 1][nc] ? c[tx][nc] : c[tx - 1][nc]) -
			     50)) {
				tmp.x = tx - 1;
				tmp.y = nc;
				tmp.z =
			...
			if ((f[tx][nc] >
			     f[tx][nc - 1] ? f[tx][nc] : f[tx][nc - 1]) <=
			    ((c[tx][nc] <
			      c[tx][nc - 1] ? c[tx][nc] : c[tx][nc - 1]) -
			     50)) {
				tmp.x = tx;
				tmp.y = nc - 1;
				tmp.z =
				    pn < (d <
					  (c[tx][nc - 1] - 50) ? d : c[tx][nc -
									   1] -
					  50) ? pn : d <
