...
Main (int argc, char *argv[])
{
  ...
  for (t = 1; t <= cx; t++)
      ...
      for (a = 1; a < (4 * length); a++)
	{
      ...
      int ans = 0;
      for (a = 1; ((a < (length * k)) && (a < (length * 4))) && (ans == 0);
	   a++)
	{
	  ...
	  if (i != I)
	    continue;
	  for (l = 1;
	       ((l < (length * 4)) && ((a + l) < (length * k))) && (ans == 0);
	       l++)
	    {
	      int i =
		low (divt, rslt[(length * k) % (4 * length)],
		     rslt[((length * k) - l) % (4 * length)]);
	      if (i != K)
		continue;
	      int z =
		low (divt, rslt[((length * k) - l) % (4 * length)],
		     rslt[a % (4 * length)]);
	      if (z != J)
		continue;
	      ans = 1;
