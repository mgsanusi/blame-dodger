  ...
  for (ishappy = 1; ishappy <= c; ishappy++)
      ...
      for (cf_stat = 0; cf_stat < mutex_buf; cf_stat++)
	{
	  scanf ("%d %d %d %d %d %d %d %d", &b[cf_stat].p,
		 &b[cf_stat].mutex_buf, &b[cf_stat].c, &b[cf_stat].p,
		 &b[cf_stat].l, &b[cf_stat].dd, &b[cf_stat].y,
		 &b[cf_stat].cases);
	}
      sort ();
      b = 0;
      while (1)
	{
	  for (cf_stat = 0;;)
	    {
	      for (length2 = 2 * b[cf_stat].c; length2 <= (2 * b[cf_stat].p);
		   length2++)
		{
		  if (length2 < 0)
		    {
	      ...
	      if ((((cf_stat + 1) < mutex_buf) && b[cf_stat + 1].mutex_buf)
