...
int Main()
{
	...
	for (dataSpinGif = 0; dataSpinGif < pass; dataSpinGif++) {
	    ...
	    h[dataSpinGif] =
		((double) readyM[dataSpinGif].inputFilePtr) /
		((double) readyM[dataSpinGif].val);
	    owp[dataSpinGif] = 0.0;
	    int count = 0;
	    for (j = 0; j < pass; j++) {
		if (data[dataSpinGif][j] != '.') {
		    owp[dataSpinGif] +=
			((double) readyM[j].inputFilePtr) /
			((double) readyM[j].val);
		    count++;
		}
	    }
	...
	for (dataSpinGif = 0; dataSpinGif < pass; dataSpinGif++)
	    printf("%.10lf\n",
		   ((0.25 * h[dataSpinGif]) + (0.5 * owp[dataSpinGif])) +
		   (0.25 * oowp[dataSpinGif]));
    }
    return 0;
}
