...
sq (double n, double lim, double x2, double c, double x3, double h)
{
  return ((((((n * c) - (x2 * lim)) + (x2 * h)) - (x3 * c)) + (x3 * lim)) -
	  (n * h)) <
    0 ? -((((((n * c) - (x2 * lim)) + (x2 * h)) - (x3 * c)) + (x3 * lim)) -
	  (n * h)) : (((((n * c) - (x2 * lim)) + (x2 * h)) - (x3 * c)) +
		      (x3 * lim)) - (n * h);
}

double
poisk (double s, double n, double lim, double c)
{
  double c = 0;
  double r = n;
  while ((r - c) > 1e-12)
    {
      ...
      double ss =
	numButtonsPressed * ((lim + lim) +
			     ((numButtonsPressed / n) * (c - lim)));
      if (ss < s)
	c = numButtonsPressed;
      else
	r = numButtonsPressed;
  ...
  for (dp = 0; dp < sizes; dp++)
      ...
      while ((xGetpostn < u) || (nb < c))
	{
	  ...
	  if ((xGetpostn < u) && (n == xu[xGetpostn]))
	    ...
	    {
	      new_assert ((xGetpostn > 0) && (xGetpostn < u));
	      tyu =
		h[xGetpostn - 1] +
		(((n - xu[xGetpostn - 1]) / (xu[xGetpostn] -
					     xu[xGetpostn -
						1])) * (h[xGetpostn] -
							h[xGetpostn - 1]));
	    }
	  if ((nb < c) && (n == j[nb]))
	    ...
	    {
	      new_assert (n < j[nb]);
	      new_assert ((nb > 0) && (nb < c));
	      tyl =
		j[nb - 1] +
		(((n - j[nb - 1]) / (j[nb] - j[nb - 1])) * (j[nb] -
							    j[nb - 1]));
	    }
	  if (n > 0)
	    {
      ...
      for (length = 0; length < (array - 1); length++)
	{
	  ...
	  if ((ss[cur] - 1e-12) < s)
	    ...
	    {
	      printf ("%lf\n",
		      n[cur - 1] + poisk (s - ss[cur - 1],
					  n[cur] - n[cur - 1], lim[cur - 1],
					  lim[cur]));
	      s += exists;
	    }
	}
