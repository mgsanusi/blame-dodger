...
int Main()
{
	...
	for (index = 0; index < altitude; index++)
	    for (j = 0; j < width; j++)
		scanf("%d %d %d", &light_grid[index][j].l,
		      &light_grid[index][j].k, &light_grid[index][j].r);
	...
	while ((next_index != 0) || (next_j != ((2 * width) - 1))) {
	    ...
	    next_j = j % 2 ? j + 1 : j - 1;
	    temp = time[index][j] + 2;
	    if (((next_index >= 0) && (next_index < (2 * altitude)))
		&& (temp < time[next_index][j]))
		time[next_index][j] = temp;
	    if (((next_j >= 0) && (next_j < (2 * width)))
		&& (temp < time[index][next_j]))
		...
		for (j = 0; j < (2 * width); j++)
			...
			next_index = index, next_j = j, temp =
			    time[index][j];
	}
