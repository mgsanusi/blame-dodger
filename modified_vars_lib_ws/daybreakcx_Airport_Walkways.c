...
int
cmp (const void *cmps, const void *cmpt)
{
  struct node *l = cmps;
  struct node *y = cmpt;
	...
	if ((x2 > 0) && (sum > v))
	  {
	    if ((x2 * (sum + d[cases].pattern)) >= (d[cases].x2 - d[cases].d))
	      {
		envp =
		  ((double) (d[cases].x2 - d[cases].d)) / (sum +
							   d[cases].pattern);
		x2 -= envp;
		result += envp;
	      }
		...
		result +=
		  (((double)
		    ((d[cases].x2 - d[cases].d) -
		     (x2 * (sum + d[cases].pattern)))) / (v +
							  d[cases].pattern)) +
		  x2;
		x2 = -1;
	      }
	  ...
	  result +=
	    ((double) (d[cases].x2 - d[cases].d)) / (v + d[cases].pattern);
      printf ("%.6lf\n", result);
    }
  return 0;
}
