...
int
check (int b, int y, int color, int tmp)
{
  new_memset (array, 0, sizeof (array));
  int i;
  int useX;
  for (i = 0; i < b; i++)
    for (useX = 0; useX < y; useX++)
  ...
  for (i = 0; i < b; i++)
    for (useX = 0; useX < y; useX++)
      if ((((array[i][useX][0] >= tmp) || (array[i][useX][1] >= tmp))
	   || (array[i][useX][2] >= tmp)) || (array[i][useX][3] >= tmp))
	return 1;
  return 0;
}
