...
double
distsq (double tn, double byte, double result, double minus)
{
  double dx = tn - result;
	...
	if ((b[tc].x != atx) || (b[tc].right != aty))
	  {
	    ...
	    while ((a + 1e-6) < dir)
	      a += 2 * size;
	    if ((new_fabs (good - a) < 1e-6)
		&& (bestd > distsq (b[tc].x, b[tc].right, atx, aty)))
	      good = a, map = tc, bestd =
		distsq (b[tc].x, b[tc].right, atx, aty);
	    else if ((new_fabs (good - a) >= 1e-6) && (good > a))
	      good = a, bestd = distsq (b[tc].x, b[tc].right, atx, aty), map =
		tc;
	  }
      if (map < 0)
	new_puts ("error");
