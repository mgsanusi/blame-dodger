...
int Main()
{
	...
	for (ti = 1; ti <= per; ++ti) {
		...
		for (x2 = 0; x2 < x_getpostn; ++x2)
			fscanf(stdin, "%lf%lf", &num[x2].length,
			       &num[x2].speed);
		new_qsort((void *)num, (size_t) x_getpostn, sizeof(struct node),
			  cmp);
		...
		for (x2 = x_getpostn - 2; x2 >= 0; --x2) {
			while (((x2 + 1) < x_getpostn)
			       &&
			       (((num[x2].length +
				  num[x2 + 1].length) * num[x2 + 1].speed) >=
				(num[x2 + 1].length * num[x2].speed))) {
...
int cmp(const void *start, const void *s)
{
	return ((const struct node *)start)->length <
	    ((const struct node *)s)->length ? -1 : 1;
}
