...
int Main()
{
	...
	while (size--) {
		...
		for (http_content_type_gif = 0; http_content_type_gif < m;
		     http_content_type_gif++) {
			scanf("%d", s + http_content_type_gif);
			for (j = 0; j < s[http_content_type_gif]; j++) {
				scanf("%d%d", num[http_content_type_gif] + j,
				      type[http_content_type_gif] + j);
		...
		for (k = 0; k < (1 << input_file_name); k++) {
			t = 0;
			for (http_content_type_gif = 0;
			     http_content_type_gif < input_file_name;
			     http_content_type_gif++)
			...
			if (choose <= t)
				continue;
			for (http_content_type_gif = 0;
			     http_content_type_gif < m;
			     http_content_type_gif++) {
				for (j = 0; j < s[http_content_type_gif]; j++)
					if (tmp[num[http_content_type_gif][j]]
					    == type[http_content_type_gif][j])
						break;
			...
			if (http_content_type_gif == m) {
				...
				for (http_content_type_gif = 0;
				     http_content_type_gif < input_file_name;
				     http_content_type_gif++)
					res[http_content_type_gif] =
					    tmp[http_content_type_gif];
			}
		}
		printf("Case #%d:", ++cnt);
