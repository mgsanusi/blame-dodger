...
int Main()
{
	...
	for (in = 0; in < noTestCase; in++) {
		...
		while (numUsed < numPeople) {
			for (datasetNum = byLevel; datasetNum < curLevel;
			     datasetNum++) {
				subdivision newDistance =
				    divideSpace(distance[datasetNum]);
				addSpace(newDistance.max,
					 frequency[datasetNum]);
				if (numUsed >= numPeople)
					break;
				addSpace(newDistance.min,
					 frequency[datasetNum]);
				if (numUsed >= numPeople)
					break;
			}
