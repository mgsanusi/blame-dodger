  ...
  while (size--)
	...
	for (altitude = 1; altitude < r; altitude++)
	  if (caseno[altitude - 1] > caseno[altitude])
	    {
	      k = caseno[altitude - 1], caseno[altitude - 1] =
		caseno[altitude], caseno[altitude] = k;
	      k = min[altitude - 1], min[altitude - 1] =
		min[altitude], min[altitude] = k;
	    }
      rslt = 0;
      for (k = 0; k < (1 << r); k++)
	{
