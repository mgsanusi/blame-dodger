...
int
viagcmp1 (const void *b, const void *e2)
{
  Viag *ein;
  Viag *v2;
  ein = (Viag *) b;
...
int
viagcmp2 (const void *b, const void *e2)
{
  Viag *ein;
  Viag *v2;
  ein = (Viag *) b;
...
Main (void)
{
  ...
  for (scanf ("%d", &cs), h = 1; h <= cs; h++)
	...
	{
	  d = 1 - order;
	  new_qsort (r[order], n[order], sizeof (r[order][0]), viagcmp1);
	  new_qsort (r[d], n[d], sizeof (r[d][0]), viagcmp2);
	  for (x = 0; x < n[d]; x++)
	    {
	      r[d][x].x = 0;
	    }
