...
char
check (int p[505][505], int fileIndexHtml, int p, int xpDestroy)
{
  int casenum;
  int k;
  int r;
...
Main ()
{
  ...
  for (length = 0; length < q; length++)
      ...
      for (am0 = 0; am0 < 505; am0++)
	{
      ...
      for (am0 = 0; am0 < fileIndexHtml; am0++)
	{
	  for (up = (fileIndexHtml - am0) - 1; up <= (fileIndexHtml + am0);
	       up += 2)
	    {
	      scanf ("%d", &p[am0][up]);
	    }
	...
	{
	  for (xpDestroy = 0; xpDestroy < ((2 * fileIndexHtml) - 1);
	       xpDestroy++)
	    {
	      int ns =
		(fileIndexHtml + module ((fileIndexHtml - 1) - p)) +
