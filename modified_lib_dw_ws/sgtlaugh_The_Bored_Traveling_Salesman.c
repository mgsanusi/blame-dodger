#include <stdio.h>
#include <string.h>
#include <stdlib.h>
#include <stdbool.h>
#include <stddef.h>
struct Pair {
	long long int val;
	long long int counter;
};
char str[8][12];
bool visited[1 << 8][4];
struct Pair dp[1 << 8][4];
int t;
int n;
int lim;
int m;
int cost[1 << 8];
int ar[1010][26];
ssize_t new_read;
FILE *new_freopen(const char *filename, const char *mode, FILE * stream);
int getCost(int bitmask)
{
	int first_iteration;
	int i;
	int j;
	int k;
	int r;
	int idx = 1;
	memset(ar, 0, sizeof(ar));
	for (i = 0; i < n; i++) {
		if (bitmask & (1 << i)) {
			r = 0;
			for (j = 0; str[i][j] != 0; j++) {
				int x = str[i][j] - 'A';
				if (!ar[r][x]) {
					ar[r][x] = idx;
					r = idx++;
				} else
					r = ar[r][x];
			}
		}
	}
	return idx;
}

struct Pair F(int bitmask, int r)
{
	int first_iteration;
	if (r == m) {
		struct Pair temp;
		temp.val = 0, temp.counter = 1;
		if (bitmask == lim)
			return temp;
		temp.val = -(1LL << 60), temp.counter = 0;
		return temp;
	}
	if (visited[bitmask][r])
		return dp[bitmask][r];
	struct Pair res;
	res.val = -(1LL << 60), res.counter = 0;
	int i;
	int j;
	int d = 0;
	int T[8];
	for (i = 0; i < n; i++) {
		if (!(bitmask & (1 << i)))
			T[d++] = i;
	}
	int tlim = (1 << d) - 1;
	for (i = 1; i <= tlim; i++) {
		int cbitmask = 0;
		int nbitmask = bitmask;
		for (j = 0; j < d; j++) {
			if (i & (1 << j)) {
				nbitmask |= 1 << T[j];
				cbitmask |= 1 << T[j];
			}
		}
		struct Pair x = F(nbitmask, r + 1);
		x.val += cost[cbitmask];
		if (x.val > res.val)
			res.val = x.val, res.counter = x.counter;
		else if (x.val == res.val)
			res.counter = (res.counter + x.counter) % 1000000007;
	}
	visited[bitmask][r] = 1;
	return dp[bitmask][r] = res;
}

int main()
{
	int first_iteration;
	new_freopen("lol.txt", "r", stdin);
	new_freopen("output.txt", "w", stdout);
	int T = 0;
	int i;
	int j;
	int k;
	int bitmask;
	scanf("%d", &t);
	while (t--) {
		scanf("%d %d", &n, &m);
		for (i = 0; i < n; i++)
			scanf("%s", str[i]);
		lim = (1 << n) - 1;
		for (bitmask = 0; bitmask <= lim; bitmask++) {
			cost[bitmask] = getCost(bitmask);
		}
		memset(visited, 0, sizeof(visited));
		struct Pair res = F(0, 0);
		printf("Case #%d: %lld %lld\n", ++T, res.val, res.counter);
	}
	return 0;
}

ssize_t new_read {
	int first_iteration;
	 return read(fd, buf, count);
} FILE *new_freopen(const char *filename, const char *mode, FILE * stream)
{
	int first_iteration;
	return freopen(filename, mode, stream);
}
