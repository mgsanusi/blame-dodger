...
int main(void)
{
	...
	for (i = 0; i < tot_test_cases; i++) {
		...
		for (; num_combine > 0; num_combine--) {
			scanf(" %s", combine_str);
			reaction_array[combine_str[0] - 'A'][combine_str[1] -
							     'A'] =
			    combine_str[2] - 'A';
			reaction_array[combine_str[1] - 'A'][combine_str[0] -
							     'A'] =
			    combine_str[2] - 'A';
		}
		scanf("%d", &num_destroy);
		for (; num_destroy > 0; num_destroy--) {
			scanf(" %s", destroy_str);
			destruction_array[destroy_str[0] - 'A'][destroy_str[1] -
								'A'] = 1;
			destruction_array[destroy_str[1] - 'A'][destroy_str[0] -
								'A'] = 1;
		}
		scanf("%d", &num_string);
		scanf(" %s", input_str);
		for (j = 0; j < num_string; j++) {
			...
			do {
				...
				if (output_len != 0) {
					previous_element =
					    output_str[output_len - 1] - 'A';
					if (reaction_array[new_element]
					    [previous_element] != (-1)) {
						flag = 1;
						new_element =
						    reaction_array[new_element]
						    [previous_element];
						output_len--;
						element_seen
						    [previous_element]--;
