...
int Main()
{
	...
	for (answer = 0; no_group > 0; answer++) {
		...
		if ((partition_length[1] > 0)
		    && (partition_length[carry_size - 1] > 0)) {
		partition_length[1]--;
		partition_length[carry_size - 1]--;
		no_group -= 2;
	    } else if (partition_length[1] > 0) {
		no_group -= min(partition_length[1], carry_size);
		partition_length[1] -=
		    min(partition_length[1], carry_size);
	    } else if (partition_length[carry_size - 1] > 0) {
		no_group -=
		    min(partition_length[carry_size - 1], carry_size);
		partition_length[carry_size - 1] -=
		    min(partition_length[carry_size - 1], carry_size);
	    }
	}
