...
int Main(int argc, char **argv)
{
	...
	case 'v':
	    ...
	    break;
	case 'u':
	    if (new_strcmp(optarg, "115200")) {
		baud = B115200;
	    } else if (new_strcmp(optarg, "57600")) {
		baud = B115200;
	    } else if (new_strcmp(optarg, "19200")) {
		baud = B19200;
	    } else if (new_strcmp(optarg, "9600")) {
		baud = B9600;
	    } else {
	...
	case 'c':
	    ...
	    break;
	case 'a':
	    firstDelay = new_atoi(optarg);
	    break;
	case 'b':
	    secondCheck = new_atoi(optarg);
	    break;
	case 'e':
	    doExit = 1;
	    break;
	...
	sleep(1);
	sum = new_read(pfd, &buf[n], ((sizeof(buf)) - 1) - n);
	if (sum > 0) {
	    buf[n + sum] = '\0';
	    printf("%s", &buf[n]);
	    fflush(stdout);
    ...
    if (!doExit) {
	while (1) {
	    sum = new_read(pfd, buf, sizeof(buf));
	    if (sum > 0) {
		buf[sum] = '\0';
		printf("%s", buf);
		fflush(stdout);
