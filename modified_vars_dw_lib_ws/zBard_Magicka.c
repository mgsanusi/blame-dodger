...
int Main(void)
{
	...
	for (base = 0; base < tot_screen_case; base++) {
		...
		for (; no_combine > 0; no_combine--) {
			scanf(" %s", combine_str);
			reaction_arr[combine_str[0] - 'A'][combine_str[1] -
							   'A'] =
			    combine_str[2] - 'A';
			reaction_arr[combine_str[1] - 'A'][combine_str[0] -
							   'A'] =
			    combine_str[2] - 'A';
		}
		scanf("%d", &num_destroy);
		for (; num_destroy > 0; num_destroy--) {
			...
			destruction_array[destroy_string[0] -
					  'A'][destroy_string[1] - 'A'] = 1;
			destruction_array[destroy_string[1] -
					  'A'][destroy_string[0] - 'A'] = 1;
		}
		scanf("%d", &no_str);
		scanf(" %s", input_str);
		for (bx = 0; bx < no_str; bx++) {
			...
			do {
				...
				if (output_len != 0) {
					former_element =
					    output_string[output_len - 1] - 'A';
					if (reaction_arr[new_element]
					    [former_element] != (-1)) {
						flag = 1;
						new_element =
						    reaction_arr[new_element]
						    [former_element];
						output_len--;
