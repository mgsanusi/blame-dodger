...
int cmppan(const void *x, const void *t)
{
	...
	if (apan.lateralArea > bpan.lateralArea) {
		return -1;
	} else if (apan.lateralArea < bpan.lateralArea) {
		return 1;
	} else
		return 0;
}
...
int Main(int argc, char **argv)
{
	...
	for (trialCase = 0; trialCase < t; trialCase++) {
		...
		Pancake pancakes[n2];
		for (l = 0; l < n2; l++) {
			scanf(" %llu %llu", &pancakes[l].sizeCount,
			      &pancakes[l].t2);
			pancakes[l].lateralArea =
			    ((2 * M_PI) * pancakes[l].sizeCount) *
			    pancakes[l].t2;
			pancakes[l].topArea =
			    (M_PI * pancakes[l].sizeCount) *
			    pancakes[l].sizeCount;
		}
		new_qsort(pancakes, n2, sizeof(Pancake), cmppan);
		int base;
		double maxArea = 0;
		for (base = 0; base < n2; base++) {
			double area =
			    pancakes[base].topArea + pancakes[base].lateralArea;
			int selected = 0;
			for (l = 0; (l < n2) && (selected < (dy - 1)); l++) {
				if ((l != base)
				    && (pancakes[l].sizeCount <=
					pancakes[base].sizeCount)) {
					selected++;
