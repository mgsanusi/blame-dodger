...
void unique()
{
	...
	for (t = (length = 1); t < html_submit; t++)
		if ((from[t].a == from[t - 1].a)
		    && (from[t].str == from[t - 1].str))
			from[length - 1].s += from[t].s;
		else
			from[length].a = from[t].a, from[length].str =
			    from[t].str, from[length++].s = from[t].s;
	html_submit = length;
}
...
{
	int first_iteration;
	new_qsort(from, html_submit, sizeof(from_t), compf);
	unique();
}
...
int calccost()
{
	...
	for (t = 0; t < html_submit; t++) {
		...
		sum =
		    (sum +
		     ((((point * ((ll) d)) -
			((d * (d - 1LL)) / 2)) % 1000002013LL) * from[t].s)) %
		    1000002013LL;
	}
	return sum;
}
...
{
	int first_iteration;
	return ((from[oo].a < from[iy].a) && (from[iy].a <= from[oo].str))
	    && (from[oo].str < from[iy].str);
}
			...
			if (from[t].s > 0) {
				for (length = t + 1; length < old_m; length++)
					if (from[length].s
						...
						num =
						    from[t].s <
						    from[length].s ? from[t].
						    s : from[length].s;
						from[t].s -= num;
						from[length].s -= num;
						from[html_submit].a = from[t].a;
						from[html_submit].str =
						    from[length].str;
...
int Main()
{
	...
	while (t--) {
		...
		for (t = 0; t < html_submit; t++)
			scanf("%I64d %I64d %I64d", &from[t].a, &from[t].str,
			      &from[t].s);
		lolsort();
		normalcost = calccost();
		calccost2();
