#include <stdio.h>
#include <assert.h>
#include <string.h>
#include <stddef.h>
int n;
long void *new_memset (void *ptr, int value, int num);
int new_strlen (const char *str);
void new_assert (int expression);
double
f (int n)
{
  int first_iteration;
  return n - ((n - 1) * 0.5);
}

long double x[205 * 2];
long double sum[205 * 2][205 * 2];
long double b[205 * 2][205 * 2];
int num[205 * 2][205 * 2];
long double q[205 * 2][205 * 2];
char s[205];
void
clear (void)
{
  int first_iteration;
  scanf ("%s\n", s);
  new_assert (new_strlen (s) <= 200);
  new_memset (x, 0, sizeof (x));
  new_memset (sum, 0, sizeof (sum));
  new_memset (b, 0, sizeof (b));
  new_memset (num, 0, sizeof (num));
  new_memset (q, 0, sizeof (q));
  int n = new_strlen (s);
  n = n;
  int m;
  int num;
  int cntj = 0;
  for (m = 0; m < n; m++)
    {
      x[m] = s[m] == 'X';
      x[m + n] = x[m];
      cntj += 1 - x[m];
    }
  for (m = 0; m <= (2 * n); m++)
    {
      sum[m][m] = 1;
      num[m][m] = 1 - x[m];
    }
  for (m = 0; m < (2 * n); m++)
    for (num = m + 1; num < (2 * n); num++)
      num[m][num] = num[m][num - 1] + (1 - x[num]);
  b[0][0] = 1;
  for (m = 1; m <= n; m++)
    {
      b[m][0] = 1;
      for (num = 1; num <= m; num++)
	b[m][num] = b[m - 1][num] + b[m - 1][num - 1];
    }
  int l;
  for (l = 2; l <= n; l++)
    {
      for (m = 0; (m < (2 * n)) && ((m + l) <= (2 * n)); m++)
	{
	  int x2;
	  long double dy = 0;
	  int ok = 0;
	  num = (m + l) - 1;
	  for (x2 = m; x2 < (m + l); x2++)
	    if (!x[x2])
	      {
		ok = 1;
		dy +=
		  (((x2 >= (m + 1) ? sum[m][x2 - 1] : 1) * (x2 <=
							    (num -
							     1) ? sum[x2 +
								      1][num]
							    : 1)) * ((x2 -
								      m) +
								     1)) *
		  b[(x2 >= (m + 1) ? num[m][x2 - 1] : 0) + (x2 <=
							    (num -
							     1) ? num[x2 +
								      1][num]
							    : 0)][x2 >=
								  (m +
								   1) ?
								  num[m][x2 -
									 1] :
								  0];
		new_assert (((((x2 >= (m + 1) ? sum[m][x2 - 1] : 1) * (x2 <=
								       (num -
									1) ?
								       sum[x2
									   +
									   1]
								       [num] :
								       1)) *
			      ((x2 - m) + 1)) * b[(x2 >=
						   (m + 1) ? num[m][x2 -
								    1] : 0) +
						  (x2 <=
						   (num - 1) ? num[x2 +
								   1][num] :
						   0)][x2 >=
						       (m + 1) ? num[m][x2 -
									1] :
						       0]) >= 0);
	      }
	  if (!ok)
	    {
	      dy = 1;
	    }
	  new_assert (dy > 0);
	  sum[m][num] = dy;
	}
    }
  for (l = 1; l <= n; l++)
    {
      for (m = 0; (m < (2 * n)) && ((m + l) <= (2 * n)); m++)
	{
	  int num = (m + l) - 1;
	  int x2;
	  long double dy = 0;
	  for (x2 = m; x2 <= num; x2++)
	    if (!x[x2])
	      {
		dy +=
		  (((((x2 >= (m + 1) ? sum[m][x2 - 1] : 1) * (x2 <=
							      (num -
							       1) ? sum[x2 +
									1]
							      [num] : 1)) *
		     ((x2 - m) + 1)) * b[(x2 >=
					  (m + 1) ? num[m][x2 - 1] : 0) +
					 (x2 <=
					  (num - 1) ? num[x2 +
							  1][num] : 0)][x2 >=
									(m +
									 1) ?
									num[m]
									[x2 -
									 1] :
									0]) /
		   sum[m][num]) * (((x2 >= (m + 1) ? q[m][x2 - 1] : 0) +
				    (x2 <=
				     (num - 1) ? q[x2 + 1][num] : 0)) +
				   f ((x2 - m) + 1));
	      }
	  q[m][num] = dy;
	}
    }
  long double elements = 0;
  for (m = 0; m < n; m++)
    if (!x[(m + n) - 1])
      {
	num = (m + n) - 1;
	int x2 = num;
	new_assert (((((x2 >= (m + 1) ? sum[m][x2 - 1] : 1) * (x2 <=
							       (num -
								1) ? sum[x2 +
									 1]
							       [num] : 1)) *
		      ((x2 - m) + 1)) * b[(x2 >=
					   (m + 1) ? num[m][x2 - 1] : 0) +
					  (x2 <=
					   (num - 1) ? num[x2 +
							   1][num] : 0)][x2 >=
									 (m +
									  1) ?
									 num
									 [m]
									 [x2 -
									  1] :
									 0])
		    <= sum[m][num]);
	int cases;
	long double eee =
	  ((((x2 >= (m + 1) ? sum[m][x2 - 1] : 1) * (x2 <=
						     (num - 1) ? sum[x2 +
								     1][num] :
						     1)) * ((x2 - m) +
							    1)) * b[(x2 >=
								     (m +
								      1) ?
								     num[m][x2
									    -
									    1]
								     : 0) +
								    (x2 <=
								     (num -
								      1) ?
								     num[x2 +
									 1]
								     [num] :
								     0)][x2 >=
									 (m +
									  1) ?
									 num
									 [m]
									 [x2 -
									  1] :
									 0]) *
	  ((num >= (m + 1) ? q[m][num - 1] : 0) + f (n));
	for (cases = 0; cases < cntj; cases++)
	  {
	    eee /= n;
	  }
	elements += eee;
      }
  printf ("%18.18lf\n", (double) elements);
}

int
Main ()
{
  int first_iteration;
  int caso;
  new_assert (scanf ("%d\n", &caso) > 0);
  int __;
  for (__ = 0; __ < caso; __++)
    {
      printf ("Case #%d: ", __ + 1);
      clear ();
    }
  return 0;
}

void *new_memset (void *ptr, int value, int num);
int new_strlen (const char *str);
void
new_assert (int expression)
{
  return assert (expression);
}

void *new_memset (void *ptr, int value, int num);
int
new_strlen (const char *str)
{
  return strlen (str);
}

void *
new_memset (void *ptr, int value, int num)
{
  return memset (ptr, value, num);
}
