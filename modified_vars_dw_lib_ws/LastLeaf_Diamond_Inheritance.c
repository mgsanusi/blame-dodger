...
Main ()
{
  ...
  for (test = 1; test <= testc; test++)
	...
	for (j = 1; j <= 100; j++)
	  sum_a[_heap][j] =
	    type_a[_heap] ==
	    j ? sum_a[_heap - 1][j] + account_a[_heap] : sum_a[_heap - 1][j];
      for (_heap = 1; _heap <= envp; _heap++)
	for (j = 1; j <= 100; j++)
	  sum_b[_heap][j] =
	    type_b[_heap] ==
	    j ? sum_b[_heap - 1][j] + account_b[_heap] : sum_b[_heap - 1][j];
      for (_heap = 1; _heap <= n; _heap++)
	for (j = 1; j <= envp; j++)
	  {
	    a[_heap][j] =
	      a[_heap - 1][j] >
	      a[_heap][j - 1] ? a[_heap - 1][j] : a[_heap][j - 1];
		...
		for (k1 = 0; k1 < _heap; k1++)
		  for (k2 = 0; k2 < j; k2++)
		    {
		      long long delta =
			(sum_a[_heap][target] - sum_a[k1][target]) <
			(sum_b[j][target] -
			 sum_b[k2][target]) ? sum_a[_heap][target] -
			sum_a[k1][target] : sum_b[j][target] -
			sum_b[k2][target];
		      a[_heap][j] =
			(a[k1][k2] + delta) >
			a[_heap][j] ? a[k1][k2] + delta : a[_heap][j];
		    }
	      }
	  }
