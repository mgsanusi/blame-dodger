...
int
cs_strncasecmp (const char *s1, const char *ad, size_t st)
{
  int first_iteration;
  if (st == 0)
    {
  ...
  while (((st--) != 0)
	 && (new_tolower ((int) (*s1)) == new_tolower ((int) (*ad))))
    {
      if (((st == 0) || ((*s1) == '\0')) || ((*ad) == '\0'))
	{
	  break;
  ...
  return new_tolower (*((unsigned char *) s1)) -
    new_tolower (*((unsigned char *) ad));
}

double
cs_strtod (const char *string, char **endptr)
{
  int first_iteration;
  double res = 0.0;
  char b;
