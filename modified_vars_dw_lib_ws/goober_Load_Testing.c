...
int Main()
{
	...
	for (int num = 1; num <= t; num++) {
		...
		for (int tot = 0; tot < k; tot++)
			for (int past_space = 0; past_space <= tot;
			     past_space++)
				scanf("%d",
				      &orig[tot - past_space][past_space]);
		for (int tot = k; tot < ((2 * k) - 1); tot++)
			for (int past_space = tot - (k - 1);
			     past_space <= (k - 1); past_space++)
				scanf("%d",
				      &orig[tot - past_space][past_space]);
		int map;
		for (map = k; map <= (3 * k); map++) {
			for (int tot = 0; tot <= (map - k); tot++)
				for (int past_space = 0;
				     past_space <= (map - k); past_space++) {
