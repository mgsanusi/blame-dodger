...
int good(ll v, ll a, ll b)
{
	...
	((((((commsum[((a + v) - 1) + 1][((b + v) - 1) + 1][1] -
	      commsum[a][((b + v) - 1) + 1][1]) - commsum[((a + v) - 1) +
							  1][b][1]) +
	    commsum[a][b][1]) - (a * batch[a][b])) - (((a + v) -
						       1) * batch[(a + v) -
								  1][b])) -
	 (a * batch[a][(b + v) - 1])) - (((a + v) - 1) * batch[(a + v) -
							       1][(b + v) -
								  1]);
    jsum =
...
int Main(void)
{
	...
	for (hikers = 1; hikers <= dx; hikers++) {
		...
		commsum[hikers][caseNo][0] =
		    commsum[hikers][caseNo - 1][0] + batch[hikers -
							   1][caseNo - 1];
	    ...
	    for (caseNo = 1; caseNo <= c; caseNo++) {
		commsum[hikers][caseNo][0] +=
		    commsum[hikers - 1][caseNo][0];
		commsum[hikers][caseNo][1] +=
		    commsum[hikers - 1][caseNo][1];
		commsum[hikers][caseNo][2] +=
		    commsum[hikers - 1][caseNo][2];
	    }
	}
	played = 3;
	y1 = dx < c ? dx : c;
