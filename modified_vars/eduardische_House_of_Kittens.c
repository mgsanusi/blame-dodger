...
char check(long long int word)
{
  ...
  {
    min = last + re_vers;
    max = last + (re_vers * ((long long int) ret[space_frequency].cnt));
    ...
    else
    {
      ...
      if (s > word)
      {
        ...
        max = min + (re_vers * ((long long int) (ret[space_frequency].cnt - 1)));
      }
...
int Main()
{
  ...
  for (j = 0; j < word; j++)
  {
    ...
    for (space_frequency = 0; space_frequency < best; space_frequency++)
    {
      scanf("%I64i %d", &ret[space_frequency].pos, &ret[space_frequency].cnt);
