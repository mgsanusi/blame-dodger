  ...
  for (size = 0; size < numScreenCase; size++)
  {
    ...
    for (result = 0; numGroup > 0; result++)
    {
      if (partitionLength[0] > 0)
      {
        ...
        if ((partitionLength[1] > 0) && (partitionLength[packSize - 1] > 0))
      {
        partitionLength[1]--;
        ...
        if (packSize == 4)
      {
        ...
        else
        {
          for (storeModulo = 1; storeModulo < 4; storeModulo += 2)
            if ((partitionLength[2] > 0) && (partitionLength[storeModulo] > 1))
          {
            partitionLength[2]--;
            ...
            if (partitionLength[storeModulo] > 0)
          {
            ...
            partitionLength[storeModulo] -= min(partitionLength[storeModulo], 4);
          }
        ...
        if (partitionLength[1] > 0)
      {
        ...
        if (partitionLength[packSize - 1] > 0)
      {
        numGroup -= min(partitionLength[packSize - 1], packSize);
        partitionLength[packSize - 1] -= min(partitionLength[packSize - 1], packSize);
      }
