  ...
  for (subjectI = 1; subjectI <= caseCnt; subjectI++)
  {
    ...
    for (orga = 0; orga < cases; orga++)
    {
      for (j = 0; j < t; j++)
      {
        ...
        {
          dir = 0, drop = 0;
          if ((tmp > 0) && ((elevation[tmp][f] - elevation[tmp - 1][f]) > drop))
            dir = 1, drop = elevation[tmp][f] - elevation[tmp - 1][f];

          if ((f > 0) && ((elevation[tmp][f] - elevation[tmp][f - 1]) > drop))
            dir = 2, drop = elevation[tmp][f] - elevation[tmp][f - 1];

          if ((f < (t - 1)) && ((elevation[tmp][f] - elevation[tmp][f + 1]) > drop))
            dir = 3, drop = elevation[tmp][f] - elevation[tmp][f + 1];

          if ((tmp < (cases - 1)) && ((elevation[tmp][f] - elevation[tmp + 1][f]) > drop))
