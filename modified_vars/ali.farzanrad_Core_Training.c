int Main()
{
  ...
  for (itest = 0; (++itest) <= ntest;)
  {
    ...
    for (casos = 0; casos < n; ++casos)
      ...
      if (((!mark[casos]) && (!mark[root])) && (((b[casos] + b[root]) % x) == 0))
    {
    ...
    for (casos = 0; casos < n; ++casos)
      for (root = casos; (++root) < n;)
      for (k = root; (++k) < n;)
      if ((((!mark[casos]) && (!mark[root])) && (!mark[k])) && ((((b[casos] + b[root]) + b[k]) % x) == 0))
    {
    ...
    for (casos = 0; casos < n; ++casos)
      ...
      for (k = root; (++k) < n;)
      for (b = k; (++b) < n;)
      if (((((!mark[casos]) && (!mark[root])) && (!mark[k])) && (!mark[b])) && (((((b[casos] + b[root]) + b[k]) + b[b]) % x) == 0))
    {
