        ...
        for (j = 0; j < per; j++)
        {
          ...
          if (j >= (n - 1))
          {
            ...
            for (ind = 0; ind < 2; ind++)
              for (jj = 0; jj < 2; jj++)
            {
              curm -= t[cur - ((n - 1) * ind)][j - ((n - 1) * jj)] - '0';
              curmx -= (cur - ((n - 1) * ind)) * (t[cur - ((n - 1) * ind)][j - ((n - 1) * jj)] - '0');
              curmy -= (j - ((n - 1) * jj)) * (t[cur - ((n - 1) * ind)][j - ((n - 1) * jj)] - '0');
            }


            if (((curmx * 2) == (curm * ((2 * cur) - (n - 1)))) && ((curmy * 2) == (curm * ((2 * j) - (n - 1)))))
            {
              good = n;
              break;
            }
