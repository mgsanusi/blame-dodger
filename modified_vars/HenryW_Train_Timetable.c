...
void sort()
{
  ...
  {
    if ((depH[wp] > depH[foundx]) || ((depH[wp] == depH[foundx]) && (depM[wp] > depM[foundx])))
    {
      {
        temp = depH[foundx];
        depH[foundx] = depH[wp];
  ...
  printf("\n");
  for (wp = 0; wp < (inputFileName + line); wp++)
    printf("Train %d->%d, %02d:%02d ~ %02d:%02d\n", depAb[wp], arrayAb[wp], depH[wp], depM[wp], arrH[wp], arrM[wp]);
...
void process()
{
  ...
  for (wp = 0; wp < (inputFileName + line); wp++)
  {
    ...
    {
      if ((((readyH[foundx] < mH) || ((readyM[foundx] <= r) && (readyH[foundx] == mH))) && ((readyH[foundx] < depH[wp]) || ((readyM[foundx] <= depM[wp]) && (readyH[foundx] == depH[wp])))) && (readyAt[foundx] == depAb[wp]))
      {
        mH = readyH[foundx];
        r = readyM[foundx];
        mIdx = foundx;
