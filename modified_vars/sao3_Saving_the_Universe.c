...
{
  int t_case;
  for (t_case = 0; fscanf(flipper, "%c", &string[t_case]), string[t_case] != '\n'; t_case++)
    ;
...
int Main()
{
  ...
  while ((n--) > 0)
  {
    ...
    {
      readstring(now);
      for (gss_c_nt_export_name = 0; (gss_c_nt_export_name < s) && (strcmp(now, buf[gss_c_nt_export_name]) != 0); gss_c_nt_export_name++)
        ;
