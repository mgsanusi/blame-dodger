...
#include <string.h>
#include <stdlib.h>
double auxSmall(int t, int *b, int *xdrs, int **l, int k, int p, int eLeft, int currS)
{
  int dist = l[k][k + 1];
  double rv1 = -1.;
  double rv2 = -1.;
  ...
  if (dist <= eLeft)
  {
    ...
    rv1 = deltaT + auxSmall(t, b, xdrs, l, k + 1, p, eLeft - dist, currS);
  }
  ...
  if (dist <= b[k])
  {
    ...
    eLeft -= dist;
    rv2 = deltaT + auxSmall(t, b, xdrs, l, k + 1, p, eLeft, xdrs[k]);
  }
...
void clearTestCase(int caseIdx, int t, int inputFileName, int *b, int *xdrs, int **l, int *k, int *p)
{
