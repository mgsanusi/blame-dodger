  ...
  def visit_TypeDecl(self, node):
    ...
    if not node.declname is None:
      if node.declname in varnames:
        ...
        if current_filename not in file_varnames or new_name in file_varnames[current_filename] or len(new_name) == 0 or new_name in keywords or new_name == "case": # if it's already being used, switch it back
...
# changes var names
def populate_and_transform(filenames, folder, new_folder):
  global dictionary
  global current_filename
  global dict_objects
  global varnames
  ...
  for f in filenames:
    ...
    current_filename = f.strip()
    transform(folder + "/" + f)
    with open(new_folder + "/" + f, 'w') as write_file:
...
if __name__ == "__main__":
  ...
  if len(sys.argv) > 1:
    ...
    # for each file
    populate_and_transform(folder_contents, sys.argv[1], sys.argv[2])
